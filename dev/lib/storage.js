...
// 회원(본인) 탈퇴 신청 RPC 호출 (현재 원본 마이그레이션 354).
...
//          'admin_requested' }
//   ⚠️ 'admin_requested' — 관리자가 대신 신청한(자격 상실 등) 탈퇴는 본인이
//          취소할 수 없다. 'already_done' — 이미 확정된 탈퇴는 되돌릴 수 없다.
//   네트워크/예외 실패: { ok:false, error: <메시지> } — reason 이 아니라
//          error 키로 온다. 호출 측에서 반드시 구분해서 분기할 것.
...
//   업무 실패(정상 흐름): { ok:false, reason:'not_authenticated'|'not_found'|
//           'forbidden'|'audit_account_blocked', mode:'locked_support' }
