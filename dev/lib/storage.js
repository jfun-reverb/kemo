...
// 신청별·제품별 메모 카운트 + latest 텍스트 (목록 셀 표시용, migration 123 이후)
//   반환 키: `${application_id}_${product_idx}` (페어 키)
//   값: {count, latest} — 최신 메모 텍스트 + 누적 카운트
async function fetchBrandAppMemoSummaries() {
  if (!db) return {};
  try {
    const {data, error} = await db?.from('brand_application_memos')
      .select('application_id, product_idx, text, created_at')
      .order('created_at', {ascending: false})
      .limit(100000);
    if (error) throw error;
    const summary = {};
    (data || []).forEach(r => {
      ...
      if (!summary[key]) summary[key] = {count: 0, latest: null};
      summary[key].count++;
      if (!summary[key].latest) summary[key].latest = r.text;
    });
    return summary;
