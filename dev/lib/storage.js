...
// ── 인플루언서 위반 기록 (관리자 전용, migration 060) ──
async function recordInfluencerViolation(targetId, reasonCode, note = null) {
  if (!db) throw new Error('DB 미연결');
  await retryWithRefresh(async () => {
...
// ── 위반 이력 수정 (관리자 전용, migration 061) ──
// violation 행의 reason_code / note 사후 수정.
// verify/blacklist 등 비-violation 행에 호출하면 DB에서 EXCEPTION 발생.
async function updateInfluencerViolation(flagId, reasonCode, note = null) {
  if (!db) throw new Error('DB 미연결');
  await retryWithRefresh(async () => {
