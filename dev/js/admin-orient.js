...
function osRowMemoCell(s) {
  const n = osSheetUnreadCount(s.id);
  if (!n) return '<span style="color:#ddd">-</span>';
  // ⚠️ 스타일을 인라인으로 둔다 — .os-memo-unread 클래스는 상세 모달 본문에 함께
  //    끼워 넣는 <style> 안에만 있어서 목록에는 적용되지 않는다.
  return `<span title="아직 열어보지 않은 메모 ${n}건" style="display:inline-block;min-width:18px;`
    + `padding:1px 6px;border-radius:999px;background:var(--pink,#E91E63);color:#fff;`
    + `font-size:10.5px;font-weight:800;text-align:center">${n > 99 ? '99+' : n}</span>`;
}
...
const OS_DETAIL_STYLE = `<style>
  ...
  /* ── 내부 메모 ── */
  .os-memo{border-top:1px solid #f2f2f2;margin-top:10px;padding-top:8px}
...
function osDetailHtml(s, catMap, readonly) {
  ...
  // 「삭제된 모집 건의 메모」 — 맨 아래, 해당 메모가 있을 때만.
  //   새창 출력(readonly)에는 카드 메모와 마찬가지로 넣지 않는다.
  if (!readonly) bodyHtml += osOrphanMemoHtml();
...
function osCardDetail(c, idx, catMap, readonly) {
  ...
  // 내부 메모 — 새창 출력(readonly)에는 넣지 않는다(인쇄·화면 공유 시 내부 대화 노출 방지).
  //   카드에 고유 번호가 없으면(옛 시트가 아직 채워지기 전) 붙일 자리가 없어 생략한다.
  const memo = (!readonly && c && c.uid) ? osMemoSectionHtml(c.uid, idx) : '';
  return `<div class="os-card">${head}<div class="os-fields">${inner}</div>${memo}</div>`;
}
...
async function osRefreshMemos() {
  ...
  document.querySelectorAll('#osDetailBody .os-memo').forEach(el => {
    ...
    html = osMemoSectionHtml(cards[idx].uid, idx);
    el.outerHTML = html;
