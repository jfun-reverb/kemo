...
function osRowMemoCell(s) {
  ...
  const attrs = clickable
    ...
    : ' style="';
  // ⚠️ 스타일을 인라인으로 둔다 — 모달 안 메모 스타일은 상세 본문에 함께 끼워 넣는
  //    <style> 안에만 있어서 목록에는 적용되지 않는다.
...
const OS_DETAIL_STYLE = `<style>
  ...
  .os-memo-dot{display:inline-block;width:7px;height:7px;border-radius:50%;
    background:var(--pink,#E91E63);flex-shrink:0}
...
// ══════════════════════════════════════════════════════════════
// 내부 메모 (모집 건 카드별 — 관리자 전용, 마이그레이션 297·298)
...
// 「새 메모 있음」 점은 **그 묶음을 누른 순간** 그 묶음에서만 사라진다.
//   서버 쪽 읽음 처리는 상세를 여는 순간 이미 끝나 있다(osOpenDetail). 이 점은
//   화면에서 「어디에 새 게 있었는지」를 알려 주는 표시일 뿐이다.
//   ⚠️ 아무 데나 한 번 누를 때 전부 지우면, 모집 건이 여럿일 때 어느 것을 확인했는지가
//      남지 않는다 — 하나를 누른 순간 나머지 표시까지 같이 사라지기 때문이다.
//      그래서 지우는 범위를 **누른 묶음 하나**로 좁힌다.
let _osMemoDotHandler = null;

function osArmMemoDotClear() {
  const modal = document.getElementById('orientDetailModal');
  if (!modal) return;
  // 이전에 걸어 둔 것이 있으면 먼저 뗀다 — 열 때마다 쌓이는 것 방지
  if (_osMemoDotHandler) modal.removeEventListener('click', _osMemoDotHandler, true);
  _osMemoDotHandler = function (ev) {
    const wrap = (ev.target && ev.target.closest) ? ev.target.closest('.os-memo') : null;
    if (!wrap) return;   // 메모 묶음 바깥(시트 내용·닫기 등)을 누른 것 — 아무것도 안 지운다
    wrap.querySelectorAll('.os-memo-dot').forEach(d => d.remove());
  };
  // 캡처 단계로 듣는다 — 접기·고침 같은 안쪽 버튼이 눌려도 함께 잡힌다.
  modal.addEventListener('click', _osMemoDotHandler, true);
}
...
async function osRefreshMemos() {
  ...
  document.querySelectorAll('#osDetailBody .os-memo').forEach(el => {
    ...
    // ⚠️ 점은 다시 그릴 때 되살아나지 않는다 — 상세를 여는 순간 서버 읽음 처리가
    //    끝나 안 읽은 수가 0이 되기 때문이다. 그래서 「지금 화면에 점이 있었는지」를
    //    직접 들고 넘긴다. 안 그러면 **메모 하나 남겼을 뿐인데 아직 확인 안 한
    //    다른 모집 건의 점까지 같이 사라진다.**
    const hadDot = !!el.querySelector('.os-memo-dot');
    if (slot === 'orphan') {
      const box = el.closest('.os-memo-orphan');
      if (!box) return;
      box.outerHTML = osOrphanMemoHtml();
      osRestoreMemoState('#osDetailBody .os-memo[data-memo-slot="orphan"]', wasOpen, hadDot);
      return;
    }
...
// 다시 그린 메모 묶음에 「펼침」·「새 메모 점」을 되돌려 놓는다.
//   펼침 = 사용자가 직접 연 것도 지켜야 하고, 점 = 아직 확인 안 한 자리를 지켜야 한다.
function osRestoreMemoState(selector, wasOpen, hadDot) {
  const next = document.querySelector(selector);
  if (!next) return;
  if (wasOpen) next.classList.add('open');
  if (hadDot && !next.querySelector('.os-memo-dot')) {
    const head = next.querySelector('.os-memo-head');
    if (head) {
      const dot = document.createElement('span');
      dot.className = 'os-memo-dot';
      dot.title = '아직 열어보지 않은 메모가 있습니다';
      dot.setAttribute('role', 'img');
      dot.setAttribute('aria-label', '아직 열어보지 않은 메모가 있습니다');
...
// 모집 구간 이름표.
// 🔴 **이 숫자는 「견적 기준값」과 따로 관리된다** — 이 화면은 기준값을 부르지 않아 값을 못 읽는다.
//    기준값(`tier_slots_*`)만 고치고 여기를 빠뜨리면 **관리자 상세만 옛 숫자로 거짓말한다.**
//    둘을 함께 고칠 것(2026-09-21 — 스탠다드를 100 → 150 으로 옮기며 실제로 겪은 자리).
// ⚠️ `tmin` 에는 건수를 안 붙인다 — 「최소 인원 ~ 49」라 **고정 건수가 없다**.
const OS_TIER_LABEL = { tmin: '소량', t50: '라이트 (50건)', t100: '스탠다드 (150건)', t300: '프리미엄 (300건)', t500plus: '500건 (+실검작업)' };
