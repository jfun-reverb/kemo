...
// 저장 뒤 공통 안내(켜기·끄기·아이디 저장 셋 모두) — 사양서 ⑬
const AD_TRACKING_APPLY_NOTICE = '새로 들어오는 방문부터 적용됩니다. 이미 열려 있는 화면은 새로고침 전까지 이전 설정으로 동작할 수 있습니다.';
...
function renderAdTrackingPane(pane, d) {
  ...
  pane.innerHTML = `
    ...
    <div class="admin-card">
      ...
      <div class="admin-table-wrap">
        <table class="data-table">
          <thead><tr><th style="width:160px">시각</th><th style="width:160px">누가</th><th>무엇을</th></tr></thead>
...
// 공용 저장 — 성공이면 안내를 남기고 페인을 다시 그린다
async function _adTrackingSave(pixelId, enabled, successToast) {
  ...
  try {
    ...
    _adTrackingNotice = AD_TRACKING_APPLY_NOTICE;
...
async function saveAdTrackingPixelId() {
  ...
  await _adTrackingSave(value, !!d.enabled, value ? '픽셀 아이디를 저장했습니다.' : '픽셀 아이디를 지웠습니다.');
}
