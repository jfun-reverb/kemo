...
//   🔴 **값과 저장 경로는 그대로**다 — 입력칸(늘 보임)에서 Enter·칸 이동 → quoteInputChange → saveQuoteSetting(update_quote_setting).
...
//   나오면 고칠 수 있는 자리인지 안 보인다). 고친 뒤 Enter 또는 다른 칸으로 옮기면(change) 저장된다.
//   권한이 없으면 글자만.
  ...
  if (opts && opts.canEdit) {
    ...
    return `<td class="q-cell q-amount" data-qkey="${esc(r.key)}" title="${esc(tip)}"><div class="q-in-wrap${ph ? ' is-ph' : ''}">
      <input type="number" class="q-input" data-qkey="${esc(r.key)}" data-unit="${esc(r.unit)}" data-orig="${esc(v)}" value="${esc(v)}"
        ...
        onchange="quoteInputChange(this)" onkeydown="quoteInputKey(event, this)">
      <span class="q-unit">${esc(QUOTE_INPUT_UNIT[r.unit] || '')}</span>${tag}</div></td>`;
  }
...
// Enter = 확정(칸을 떠나 change 를 일으킨다), Escape = 원래 값으로 되돌리고 떠난다
function quoteInputKey(e, input) {
  if (e.key === 'Enter') { e.preventDefault(); input.blur(); }
  else if (e.key === 'Escape') { input.value = input.dataset.orig; input.blur(); }
}
// 값이 바뀌었을 때만 저장한다. 실패하면 칸을 원래 값으로 되돌린다(틀린 숫자가 저장된 것처럼 남지 않게).
//   저장되면 표를 다시 그리는데(범위 글이 따라 바뀐다), 그사이 옮겨 간 칸에 초점을 돌려준다.
async function quoteInputChange(input) {
  ...
  if (!ok) { input.value = input.dataset.orig; return; }
}
...
async function renderQuoteSettingsTable() {
  ...
  Object.keys(pending).forEach(k => {
    ...
    if (el) el.value = pending[k];   // data-orig 는 새 서버 값 — 칸을 떠나면 그 차이로 저장된다
  });
  if (focusKey) {
