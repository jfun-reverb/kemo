    ...
    return `<div class="camp-card" onclick="${isClickable?'openCampaign(\''+c.id+'\')':''}" style="${!isClickable?'opacity:.85;cursor:default':''}">
      <div class="camp-img" style="background:${c.image_url?'#f0f0f0':bgGrad};position:relative">
        ...
        <div class="camp-badges" style="z-index:5;position:absolute;top:8px;left:8px;display:flex;gap:4px">
          ${isActive?`<span style="background:rgba(14,126,74,.12);color:#0E7E4A;font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px">${t('campaign.badgeRecruiting')}</span>`:''}
        </div>
        ${isNew&&!isFull?`<div style="z-index:5;position:absolute;top:8px;right:8px"><span style="background:var(--pink);color:#fff;font-size:9px;font-weight:700;padding:2px 8px;border-radius:20px">${t('campaign.badgeNew')}</span></div>`:''}
