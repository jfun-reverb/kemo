...
function _reportFmtTime(v) {
  if (!v) return '';
  try { return new Date(v).toLocaleString('ko-KR', {hour12:false}); } catch (e) { return String(v); }
}
...
function _reportRowHtml(r) {
  const cells = REPORT_COLS.map(function(c) {
    if (c.key === 'receipt_url') {
      return r.receipt_url ? `<td><a href="${esc(r.receipt_url)}" target="_blank" rel="noopener">열기</a></td>` : '<td></td>';
    }
    ...
    if (c.key === 'receipt_uploaded_at' || c.key === 'ch_qoo10_at' || c.key === 'ch_cosme_at') {
      return `<td style="font-size:11px">${esc(_reportFmtTime(r[c.key]))}</td>`;
    }
      ...
      <div class="admin-card-header" style="align-items:flex-start">
        <div>
          ...
          <div style="margin-top:8px;font-size:12px;color:var(--muted);line-height:1.9">
            <div>만든 사람 ${esc(rep.created_by_name || '-')} · 만든 날 ${esc(_reportFmtTime(rep.created_at))}</div>
            <!-- 🔴 두 시각은 반드시 서로 다른 줄이다. 합치면 브랜드가 외부 데이터도 방금 것으로 읽는다. -->
            <div><strong>REVERB</strong> — 방금 조회 (${esc(_reportFmtTime(reverbQueriedAt))})</div>
            <div><strong>외부</strong> — ${extAttachedAt ? esc(_reportFmtTime(extAttachedAt)) + ' 기준' : '붙인 파일 없음'}</div>
          </div>
        </div>
