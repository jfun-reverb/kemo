...
async function openCampaign(id) {
  ...
  //   아래 두 화면에서 온 호출은 출처를 덮어쓰지 않는다 — 상세로 「돌아오는」 것이라
  //   원래 상세에 들어왔던 화면을 잃으면 안 된다:
  //     page-detail  = 응모 완료 후 재렌더
  //     page-activity = 활동관리(상세의 응모 버튼으로 들어감)의 뒤로가기
  const _cur = document.querySelector('#appShell .page.active');
  const _keep = _cur && (_cur.id === 'page-detail' || _cur.id === 'page-activity');
  if (!_keep) {
    const _from = {'page-campaigns': 'campaigns', 'page-mypage': 'mypage'}[_cur && _cur.id];
    _detailFrom = _from || 'home';   // 알림·딥링크 등 출처를 모르면 홈으로
  }
  ...
  $('detailContent').innerHTML = `
    <div class="detail-main">
      ...
      <!-- detail-head: 브랜드·형식·제목·리워드 배지·페이백 안내 한 덩어리.
           iOS 테마가 가로 넓은 화면에서 이 덩어리를 사진 오른쪽 열로 옮긴다(ios-theme.css).
           ⚠️ 이름이 없으면 붙일 자리가 없어 순서(:nth-child)로 잡아야 하는데 이 저장소는 그것을 금지한다. -->
      <div class="detail-head" style="background:#fff">
      ...
      </div><!-- /detail-head — 사진 오른쪽으로 옮기는 범위는 여기까지다(브랜드·형식·제목·리워드 배지·페이백 안내).
             ⚠️ 아래 정보표까지 이 안에 두면 가로 2단에서 표도 오른쪽 열로 딸려가 왼쪽이 텅 빈다. -->
      <div class="detail-body" style="background:#fff;border-bottom:1px solid var(--line);margin-bottom:10px">
        ${(()=>{
          ...
          rows.push(`<div class="dinfo-row" style="${ROW}"><div class="dinfo-key" style="${KEY}">${t(periodLabelKey)}</div><div class="dinfo-val" style="${VAL}">${periodValue}</div></div>`);
          ...
          if (!isEvent && !isMonitor) {
            ...
            if (fwLines.length) {
              rows.push(`<div class="dinfo-row" style="${ROW}"><div class="dinfo-key" style="${KEY}">${t('detail.minFollowers')}</div><div class="dinfo-val" style="${VAL}">${fwLines.join('<br>')}</div></div>`);
            }
          }
...
function navigateBackFromActivity() {
  ...
  const _fallback = function () {
    if (_activityFrom === 'mypage' || !_activityCampId) {
      ...
      //    (iOS: goBackFrom 이 history.back 을 못 쓸 때의 폴백이라 pushHistory=false 유지)
  ...
  if (typeof goBackFrom === 'function') goBackFrom('activity', _fallback);
  else _fallback();
}
