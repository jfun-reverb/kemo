...
async function renderMyApplyList() {
  ...
  container.innerHTML = filtered.map(a => {
    ...
    // 미읽음 메시지 수 — ⋮ 버튼의 빨간 점과 아래 메시지 버튼이 함께 쓴다
...
function openMypageSub(sub, pushHistory) {
  ...
  if (typeof setGnbTitle === 'function') {
    const _k = {applications:'mypage.menu.applications','profile-basic':'mypage.menu.basic','profile-sns':'mypage.menu.sns','profile-address':'mypage.menu.address',paypal:'mypage.menu.paypal',password:'mypage.menu.password','email-settings':'mypage.menu.emailSettings',withdraw:'mypage.withdrawView.title'}[sub];
    setGnbTitle(_k ? t(_k) : '');
  }
...
function openApplyActionModal(appId) {
  ...
  // 메시지 항목 — iOS 앱 전용(카드에서 메시지 버튼을 뺐다). 취소된 응모는 메시지 진입 차단이라 숨긴다.
  const _msgBtn = $('applyActionMsgBtn');
  if (_msgBtn) {
    const _isNativeApp = !!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform());
    const _show = _isNativeApp && app.status !== 'cancelled';
