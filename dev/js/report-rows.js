...
//   🔴 **계정 열이 있는 채널을 더하는 날은 「네 곳」을 함께 고친다** — 「두 곳」이 아니다(2026-09-18 정정).
//      ①이 대장 ②공유용 서버 함수 `get_report_share_data`(449)의 코드 ↔ 회원 표 칸 짝
//      ③`_excelSnsUrl` 의 채널별 주소 줄 ④`fetchInfluencersForReport`(storage.js)의 조회 칸.
//      ⚠️ ①②만 고치면 **새 채널 계정이 화면까지 안 오는데 오류도 안 난다** — 조회에 칸이 없어 값이 비고,
//         주소를 만들 줄이 없어 링크도 안 걸린다. 조용히 빈 칸으로 보인다.
...
//      · 저장된 값이 **주소**면 그대로 쓴다 — `http` 로 시작하면 그 주소 그대로, 스킴이 없으면
//        `https://` 만 붙인다. 예전처럼 「@ 떼기」만 하면 `instagram.com/https://…` 가 된다.
