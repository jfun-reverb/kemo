...
빈도를 제한할지** / 그대로 갈지. 배경은 마이그레이션 `373`·`374` 파일 **머리말**에 있습니다
(2026-08-25 추가 — 그전에는 374 본문에만 있어서 파일을 열어도 눈에 안 띄었습니다).

⚠️ **이 작업이 사양서나 작업표로 열린다면, 위 결정 사항을 그 문서의 「선결 조건」에 그대로 옮길 것.**
문서로 착수하는 사람은 함수 정의를 먼저 열지 않고, 「선결 조건」은 착수 전에 반드시 읽히는 자리입니다.
지금 이 경고는 **네 곳**에 있습니다 — 이 README · `373` 머리말 · `374` 머리말 · **`374` 함수 본문**
(`register_push_token` 의 UPSERT 위. 머리말로 **옮긴 게 아니라 복사**한 것이라 원본이 그대로 남아 있습니다).
