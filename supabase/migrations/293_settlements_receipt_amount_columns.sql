-- ============================================================
-- 293_settlements_receipt_amount_columns.sql
...
--   로 바꾼다. 실제 계산 로직 교체는 다음 마이그레이션(294)에서 헬퍼·함수 3개를 동시에
...
--     캠페인 상시가로 상한을 씌운 값. 다음 마이그레이션(294)부터 이 값을 실제로 만들어낸다.
...
--   함수 쪽(294)에서 항상 세 값을 함께 계산·저장하므로 어긋날 경로가 없다).
--
...
COMMENT ON COLUMN public.settlements.receipt_amount_jpy IS
  '[293] 정산 생성 시점 영수증 원금액(엔, 소수는 버림) 스냅샷 — amount_source=''receipt_amount''(리뷰어형)인 '
  '행에서만 값이 있고, 그 외(reward/product_price/product_plus_reward)는 NULL. 관리자 화면이 '
  '"영수증 N엔 → 상한 적용 M엔 지급"을 설명하는 데 쓰는 감사용 칸(마이그레이션 294부터 실제 값 채움).';
COMMENT ON COLUMN public.settlements.amount_cap_jpy IS
  '[293] 정산 생성 시점 적용된 상한(캠페인 product_price) 스냅샷 — amount_source=''receipt_amount''(리뷰어형)인 '
  '행에서만 값이 있고, 그 외는 NULL. amount_jpy = LEAST(receipt_amount_jpy, amount_cap_jpy) 를 버림한 값과 '
  '같아야 하나 DB 단 CHECK 정합 제약은 두지 않음(계산은 항상 마이그레이션 294 헬퍼 한 곳에서만 발생).';
...
-- 롤백
-- ============================================================
-- ⚠️ **294 를 먼저 되돌린 뒤에 이 파일을 되돌린다.** 294 의 함수들이 여기서 만든
--    칸 2개(receipt_amount_jpy·amount_cap_jpy)에 INSERT 하므로, 이 파일만 먼저
--    되돌리면 그 함수들이 그 자리에서 실패한다. 순서: 294 롤백 → 293 롤백.
