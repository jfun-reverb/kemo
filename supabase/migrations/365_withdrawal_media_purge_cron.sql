...
--
-- [V4] ★ 「지우고 → 표시」 순서가 지켜지는지 (일부러 실패시켜 본다)
--   개발 Edge Function 의 Secrets 에서 SUPABASE_SERVICE_ROLE_KEY 를 잠깐
--   틀린 값으로 바꾸거나, 저장소 통 이름 BUCKET 을 존재하지 않는 값으로
--   바꿔 배포한 뒤 [V3]-2·3·4 를 다시 한다(다른 결과물로).
--   기대: deleted_files=0 이고 **marked=0**, 그리고
--        SELECT media_purged_at ... → **NULL 그대로**.
--   → 파기 표시가 안 붙어야 정상이다. 붙으면 순서가 뒤집힌 것이므로
--     **즉시 되돌릴 것**(그 상태로 운영에 나가면 파일이 조용히 남는다).
--   확인 뒤 원래 값으로 되돌려 재배포.
--
