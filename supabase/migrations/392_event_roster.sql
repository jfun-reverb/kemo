...
--
-- 🔴 적용 순서 주의 — 이 파일은 387~391 보다 **먼저** 운영에 들어갔다.
--    8/28 팝업 행사가 하루 앞이라 사용자 지시로 먼저 적용했고, 그 시점 운영은
--    386 다음이 곧바로 392 였다. 나중에 「386 다음이 392」인 것을 보고
--    387~391 이 이미 적용됐다고 오해하지 말 것 (2026-08-27, 개발 세션 확인).
--    서로 건드리는 대상이 달라 충돌은 없다 — 이쪽은 조회 전용 신규 함수,
--    387~391 은 팔로워 판정·일괄 발송이다.
--
-- ⚠️ 이 함수를 나중에 고칠 때 — 시그니처를 바꾸지 않으면 `create or replace` 를 쓸 것.
--    `drop function` 은 아래 `revoke ... from public` 까지 함께 지운다. 이 저장소가
--    2026-08-27 에 같은 함정을 밟아(387) 비로그인 차단이 풀린 적이 있다.
--    부득이 drop 해야 하면 아래 revoke·grant 두 줄을 반드시 다시 실행할 것.
--
