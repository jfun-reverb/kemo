...
--   ★ 운영 데이터베이스: **적용됨** (2026-08-25 — 373 → 374 순서로 적용).
--      적용 후 실측: 표 설명 [373] / 정책 device_push_tokens_select_own(SELECT) / 행 단위 보안 켜짐 true /
--      저장된 토큰 0건 / 두 함수 실행 권한 {postgres, authenticated, service_role}
--      — 맨 앞 =X/(PUBLIC) 없음, anon 없음. 대조군으로 get_subscribed_admin_emails 를 같이 조회해
--      「닫힌 모습」이 실제로 다르게 나오는지 먼저 확인했다.
--      ⚠️ 발송 백엔드는 여전히 없다 — 이번 적용이 연 것은 「토큰 저장」뿐이다.
