-- 457_report_share_channel_match.sql
--   브랜드 공유 리포트에 캠페인의 「또는/그리고」 값을 함께 준다.
--
-- 베이스: **449**(report_share_sns_accounts) — 본문은 그대로이고 **한 줄**만 늘렸다.
--   449 → 이 파일. 414 → 449 → 457.
--
-- 왜
--   공유 화면(report.html)의 인증 상태 판정은 관리자 엑셀과 **같은 원본**(dev/js/report-rows.js)을
--   쓰는데, 2026-09-21 에 그 판정이 「또는」(channel_match='or') 캠페인을 가리게 바뀌었다.
--   그 갈래를 알려면 캠페인의 channel_match 가 필요한데 이 함수가 주지 않아,
--   **공유 화면만 옛 규칙(전부 요구)으로 판정**해 관리자 화면·서버와 다른 상태를
--   브랜드에게 보여주게 된다.
--
-- 🔴 **반드시 CREATE OR REPLACE 로만** — DROP 하면 413 이 비로그인 역할(anon)에 준 실행 권한이
--    풀려 **공유 링크가 전부 죽는다**. 이 파일에는 DROP 이 없다.
--
-- ⚠️ **늘린 자리는 하나뿐이다** — 결과물에 임베드된 campaigns 객체.
--    화면은 그 값을 g.campaign 으로 읽어 판정한다(report-rows.js 의 buildReportRows).
--    같은 함수의 campaign_rows 에는 **일부러 안 넣었다** — 표가 그 칸을 판정에 쓰지 않아,
--    쓰지도 않을 값을 브랜드에게 더 줄 이유가 없다.
--
-- ⚠️ channel_match 는 그 캠페인의 모집 조건이라 광고주가 이미 아는 값이다(개인정보 아님).
--
-- 검증
--   [V1] 적용 뒤 실행 권한이 그대로인지 (anon 이 살아 있어야 한다)
--        SELECT p.proname,
--               has_function_privilege('anon', p.oid, 'EXECUTE')          AS anon_ok,
--               has_function_privilege('authenticated', p.oid, 'EXECUTE') AS auth_ok
--          FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
--         WHERE n.nspname = 'public' AND p.proname = 'get_report_share_data';
--        → anon_ok 가 false 면 즉시 멈추고 413 의 GRANT 를 다시 건다.
--
--   [V2] 공유 링크를 실제로 열어 **인증 상태 칸을 눈으로** 본다.
--        🔴 이 변경이 바꾸는 유일한 외부 노출이다. 「또는」 캠페인이 든 리포트를 고를 것.
