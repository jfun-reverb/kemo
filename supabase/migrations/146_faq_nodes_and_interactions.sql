...
-- [155 정정: body_ko 줄바꿈 통일 — 실질 변경 없음(단일 문장)]
INSERT INTO public.faq_nodes
...
-- [155 정정2: "대표로 설정한 SNS" → "캠페인이 기준으로 정한 SNS" + 리뷰어형 면제 추가 + 줄바꿈 통일]
INSERT INTO public.faq_nodes
  (id, parent_id, kind, label_ko, label_ja, body_ko, body_ja,
   action_type, action_target, action_label_ko, action_label_ja,
   is_human_handoff, relevant_stages, sort_order, active)
...
VALUES
  ('00000002-0000-0000-0001-000000000014'::uuid,
   ...
   '이미 신청했다고 떠요',
   'すでに応募済みと表示されます',
   '같은 캠페인에 중복 응모는 안 됩니다. 이미 응모하셨을 수 있습니다.
...
VALUES
  ('00000002-0000-0000-0002-000000000001'::uuid,
   ...
   '심사는 언제 끝나나요',
   '審査はいつ終わりますか',
   '현재 심사 상태는 이 화면 맨 위, 또는 응모이력에서 확인할 수 있습니다.
...
-- Q3-1: 영수증을 어떻게 올리나요
-- [155 정정6: 리뷰어형(구매형) 전용 명시 추가 + 줄바꿈 통일]
INSERT INTO public.faq_nodes
  (id, parent_id, kind, label_ko, label_ja, body_ko, body_ja,
   action_type, action_target, action_label_ko, action_label_ja,
   is_human_handoff, relevant_stages, sort_order, active)
...
주소는 게시물이 보이는 공개 페이지 것을 붙여넣으세요.',
   '※このご案内は、投稿を提出するキャンペーン（ギフティング・訪問型）向けです。モニター型はレシートを提出します。
...
-- [155 정정3+6: 캠페인 가이드라인 지정 태그로 변경 + 기프팅·방문형 대상 명시 + 줄바꿈 통일]
INSERT INTO public.faq_nodes
  (id, parent_id, kind, label_ko, label_ja, body_ko, body_ja,
   action_type, action_target, action_label_ko, action_label_ja,
   is_human_handoff, relevant_stages, sort_order, active)
...
자세한 조건은 각 캠페인 가이드라인을 확인하세요.',
   '※このご案内は、投稿を行うキャンペーン（ギフティング・訪問型）向けです。
...
VALUES
  ('00000002-0000-0000-0003-000000000005'::uuid,
   ...
   '결과물이 반려됐어요, 재제출은?',
   '成果物が差し戻されました。再提出は？',
   '결과물이 반려(다시 하기)됐을 때는 다음 순서로 합니다.
1. 아래 「成果物提出」 버튼으로 제출 화면을 엽니다.
...
-- Q4-1: 제품은 언제 오나요
-- [155 정정5: 기프팅/리뷰어(구매형)/방문형 3유형 구분 안내 + 줄바꿈 통일]
INSERT INTO public.faq_nodes
  (id, parent_id, kind, label_ko, label_ja, body_ko, body_ja,
   action_type, action_target, action_label_ko, action_label_ja,
   is_human_handoff, relevant_stages, sort_order, active)
...
VALUES
  ('00000002-0000-0000-0004-000000000002'::uuid,
   ...
   '상품이 잘못/누락/파손되어 왔어요',
   '商品が違う / 届いていません',
   '상품이 다르거나·파손·미착이면 한 분씩 확인이 필요합니다.
...
VALUES
  ('00000002-0000-0000-0006-000000000001'::uuid,
   ...
   'SNS 계정을 등록/수정하려면',
   'SNSアカウントを登録・修正したいです',
   'SNS 계정과 팔로워 수는 다음 순서로 등록·수정합니다.
1. 아래 버튼으로 마이페이지 SNS 계정을 엽니다.
2. 계정과 팔로워 수를 입력합니다.
3. 저장합니다.
...
VALUES
  ('00000002-0000-0000-0007-000000000001'::uuid,
   ...
   '그 외 문의',
   'その他のお問い合わせ',
   '위 질문으로 해결되지 않으면 아래 입력란에서 직접 문의 바랍니다.
담당자가 확인 후 답변합니다.',
