...
**관련 베타 계획:** 오리엔시트는 베타 서비스 계획 중 우선 착수 항목.
...
| ⑧ | 일본어 게이트 | **필수 차단 + 권한자 긴급 발행 우회** — 일본어 필수 칸 미입력 시 발행 버튼 차단. 단 super_admin(또는 지정 권한자)이 사유 입력 후 긴급 발행 가능 + 그 사실 기록 |
...
| 첨부 이미지 예시 | `img1~8` 또는 콘텐츠 첨부 | Storage 업로드 경로 필요(작성 폼에서 이미지 업로드 받을지 §10 확인) |
...
- **일본어 발행 게이트(결정⑧)**: `addCampaign` 또는 발행 직전 검증 — 일본어 필수 칸(`brand_ja`/`product_ko`/일본어 콘텐츠 가이드 등) 미입력 시 발행 차단 + 안내. super_admin(또는 지정 권한자)이 **사유 입력 후 긴급 발행** 가능 → 우회 사실·사유 기록(컬럼 또는 audit). 일반 campaign_manager/campaign_admin은 우회 불가.
...
## 10. 사용자 확인 필요 (개발 착수 전 잔여)

대부분 추천값으로 확정됐고, 아래만 개발 PR 1·2 착수 시 한 번 더 확인:

1. **이미지 업로드** — 첨부 이미지 예시를 브랜드가 작성 폼에서 직접 올리게 할지(Storage 업로드), 아니면 URL/텍스트로만 받을지. (sales 기존 폼은 파일 업로드 없음 — 텍스트만이 현 패턴)
2. **모집인원·기간을 오리엔시트에서 받을지** — 신청 미연결 건은 견적·수량 데이터가 없어 이 값을 오리엔시트로 받아야 함. 신청 연결 건은 신청에서 승계 가능 → 두 경로 처리 방식.
3. **긴급 발행 우회 권한 범위** — super_admin 단독인지, campaign_admin 까지 허용인지(결정⑧은 super_admin 권고).
