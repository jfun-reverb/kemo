...
- **신청 목록 「셀프 오리엔시트」 열**: `renderSelfOrientCell`(신청 단위 요약 — 최근 상태 + 추가 건수, 만료 클라 판정). 목록 로드 시 `fetchOrientSheets` 1회 조회 후 `_orientByApp` 그룹(N+1 회피). 기존 「오리엔트 전달」(외부 URL) 열과 라벨 구분.
