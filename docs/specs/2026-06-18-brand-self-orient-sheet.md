...
```jsonc
{
  "recruit": {                    // 모집·업로드 정보 — 브랜드는 희망 기간만 입력
    "slots": "",                  // 모집인원 → campaigns.slots (숫자 파싱)
    "recruit_start": "",          // 희망 모집 시작(YYYY-MM-DD) → recruit_start
    "recruit_end": "",            // 희망 모집 마감 → deadline
    "upload_start": "",           // 희망 업로드 시작(YYYY-MM-DD, 2026-07-01 추가) → 발행 시 monitor(가구매·리뷰어)만 purchase_start
    "upload_end": ""              // 희망 업로드 마감 → 발행 시 3형식 공통 submission_end + monitor 는 purchase_end
    // 게시·결과 발표 등 나머지 일정은 관리자가 캠페인 등록 시 직접 채움.
    //   PR4 prefill: recruit_start/end→모집, upload_start/end→(monitor)구매기간+(공통)결과물 제출 마감일.
  },
  "brand": {
...
{ "brand": {"name":"","intro":"","official_accounts":""},
  "cards": [{ "form_type":"",                         // proxy_purchase|reviewer|seeding
    ...
    "recruit":{"recruit_start":"","recruit_end":"","upload_start":"","upload_end":""},  // 희망 모집·업로드 기간
...
### PR §15-D — 자동 채움 발행 + 가구매 결과물 검수 (2026-06-23)
**구현일:** 2026-06-23 / **브랜치:** `feature/orient-form-cards`

**무엇:** 제출된 오리엔시트(카드 N개)를 카드별로 캠페인으로 발행. 1 시트 = 카드 N개 → 캠페인 N개(§15-11). 가구매 결과물 검수 분기 포함.

**마이그레이션:**
- **196** `mark_orient_card_consumed(p_orient_id, p_card_idx, p_campaign_id)` — `is_admin()` 가드(매니저 포함), `FOR UPDATE` 잠금, status='submitted'만 허용, 카드별 멱등(이미 `campaign_id` 있으면 `already_published`), `jsonb_set`으로 `data.cards[idx].campaign_id`·`published_at` 기록, version+1, **모든 카드 발행 시** status='consumed'·consumed_at·campaign_id(레거시 single). 반환 `{success, status, all_published, published_count, total_count, version}`.
- **197** `campaigns` 컬럼 — `proxy_purchase boolean DEFAULT false`(가구매=영수증만) + `emergency_publish_reason`/`emergency_published_by`/`emergency_published_at`(일본어 긴급 발행 기록).

**사용자 확정:** 발행 단위=카드 1개씩 / 추적=`data.cards[i].campaign_id` 인라인+전 카드 발행 시 consumed / 가구매 결과물 검수까지 한 PR / 일본어 게이트=오리엔 경로 한정·제품명·브랜드명·가이드·긴급 우회 기록 / 발행 권한=매니저 포함 전체 관리자(긴급 우회만 campaign_admin↑).

**발행 흐름:** 상세 모달 카드 「이 카드로 발행」(`osPublishCard`, 제출됨·미발행·형식 선택 카드만) → `applyOrientCardPrefill`(cards→`newCamp*`: 한국어→`newCampProductKo`·가이드 초안 `newCampGuide`, 일본어 표시칸 `newCampProduct`/`newCampTitle` 비움→관리자 보완, recruit_type proxy/reviewer→monitor·seeding→gifting, 판매처→채널, 가격/행사→reward_note) → `switchAdminPane('add-campaign')`(진입 시 `_orientPublishCtx` 초기화 후 재설정 — 수동 신규 캠페인 오인 방지) → `addCampaign`이 `_opc` 컨텍스트로 일본어 게이트(가이드 비면 매니저 차단/campaign_admin window.prompt 사유 긴급 발행) + camp에 `proxy_purchase`·`emergency_*` 추가 + `insertCampaign` id 캡처(storage.js `.select('id')` 반환) → `markOrientCardConsumed`.

**가구매 결과물 검수(⑦-b):** `proxy_purchase` 캠페인=영수증만 인증성공. `computeCertStatus`·`renderDelivAppRow`(completed)·`renderDelivResultCellMonitor`(리뷰 불필요 표시)(admin-deliverables.js), `_excelCertStatusKo`·`_excelCertStatusMonitorKo`+4 호출부(admin-excel.js), 인플 활동관리 STEP 2(리뷰 캡쳐) 숨김(application.js). `camp.proxy_purchase` 일관 참조(`fetchCampaigns` select '*'·campMap 포함). 비가구매 monitor 회귀 없음(기본 false).

**검토:** reverb-supabase-expert 196·197 작성. reverb-reviewer 🟡(Warning: addCampaign 165줄→다음 PR 헬퍼 분리 / `window.prompt`·`confirm` 신규→관리자 모달 패턴 비일관 / insertCampaign 예외 시 ctx 초기화 누락→**수정 완료**). qa light.

**미착수:** `window.prompt`→커스텀 모달(후속), PR⑤신청 파이프라인 화면.

---

## 구현 결과 — PR⑥ 발급 메일 자동 발송 (§14)

**구현일:** 2026-06-24
**관련 커밋:** (dev 머지 시 기록)
**worktree:** `feature/orient-form-cards`

### 사용자 확정 4종 (2026-06-24)
1. **발송 방식 = 발급 화면에서 바로 발송**(클라이언트 직접 호출, Webhook 아님 — "메일 없이 링크만" 케이스 보존·성공/실패 즉시 표시)
2. **단계 자동 전이 = 자동 이동 + 안전장치 2종**(①역행 방지=현재 단계가 `orient_sheet_sent` 이전 5단계[new·reviewing·quoted·paid·kakao_room_created]일 때만 전진 ②신청 연결 건만)
3. **수신자 이메일 없을 때 = 발급은 성공·발송/단계전이만 건너뛰고 안내**(링크 수동 복사 유도)
4. **범위 = 자동발송만**(재발송 버튼·발송 이력은 후속 조각으로 분리)

### 산출물
- **마이그레이션 198** `advance_to_orient_sheet_sent(p_application_id uuid)` — SECURITY DEFINER + search_path 고정, FOR UPDATE 행 잠금, 역행 방지 조건부 전이(멱등 `already_advanced`), REVOKE PUBLIC(service_role 전용). 단계 변경 audit는 `trg_brand_app_history`가 자동 기록(changed_by=NULL "시스템 자동").
- **Edge Function `notify-orient-sheet`** — 클라 invoke(`{orient_sheet_id}`). service_role로 시트·수신자·브랜드명 조회 → Brevo 1통 발송 → 발송 성공+연결 신청 있으면 advance RPC 호출. 링크 도메인은 환경변수 `PUBLIC_SALES_URL`(클라 조작 방지). 수신자 결정: 서베이=`applicant_email`→`email` / 비-서베이=`brands.contacts` is_primary→`primary_email`. 템플릿은 `templates.ts` 인라인 방식(신규 함수 배포 시 `_templates/` 디렉토리 번들 누락 회귀 회피).
- **메일 양식** `docs/email-templates/orient-sheet-invite.html`(브랜드 한국어, 작성 버튼·기한·문의처. 인플 4줄 푸터 의무 없음) + `sync-email-templates.sh`에 `notify-orient-sheet` 등록(templates.ts 생성 분기 포함).
- **storage.js** `sendOrientInviteMail(orientSheetId)` — `db.functions.invoke`(프로젝트 첫 클라 invoke 케이스). FunctionsHttpError 본문(reason) 추출. throw 안 함(발급 무효화 방지).
- **admin-orient.js** `osSubmitCreate` 발급 후 `osSendInviteAndShow(res.id)` 비동기 호출 + 발급 결과 모달에 `osCreateMailStatus` 인라인 상태 영역(발송됨/수신자 없음/실패 + 단계 이동 안내).

### 기술 결정
- **단계 전이를 Edge Function이 RPC 경유로 호출**(직접 UPDATE 아님) — 역행 방지·연결 건만 조건을 함수 198에 캡슐화, version 낙관적 락은 단계 전이 목적과 무관해 생략(트리거가 version 자동 증가).
- **brandName 우선순위** = `brands.name`(마스터 join) → `data.brand.name` → 수신자명 → "브랜드".
- reverb-reviewer Critical(`schedule_sent` 누락 주장)은 **오판** — 단계 순서 source of truth `BRAND_STATUS_ORDER_FOR_FUNNEL`(admin-brand.js)·CLAUDE.md 기준 `schedule_sent`(6)는 `orient_sheet_sent`(5)보다 **뒤** 단계라 추가 시 오히려 역행 버그. CHECK 제약의 `IN(...)` 나열 순서를 워크플로 순서로 오해한 것. 198 그대로 유지.
- sync 부수효과로 변경된 무관 파일 `notify-admin-daily-digest/templates.ts`는 원복(그 함수 docs 원본 stale은 별도 백로그).

### 배포 잔여 (운영 보류 — 전체 오리엔시트 완성 후 일괄)
- 개발/운영 양 환경 Edge Function 배포 + secrets(`PUBLIC_SALES_URL`·`BREVO_API_KEY`) 설정.
- 메일 발송 테스트는 **운영에서만**(supabase.md 정책 — 개발은 환경 동기화만).
- 마이그레이션 198 개발/운영 DB 적용.

---

## 구현 결과 — 작성 폼 가격·판매처 축소 (2026-06-25)

**사용자 확정:** ①가격은 **상시가만**(세일가·대형할인 행사/특가 제거) ②판매처 **Qoo10 고정**(마켓 드롭다운 제거) ③판매 URL 유지 + ttp://·스킴 누락 자동 보정 + Qoo10(qoo10.jp) 아니면 **경고만**(제출 허용). → §15-4·§15-10 「가격 3칸·마켓별 대형할인 행사」 본 절로 갱신(상시가 단일·Qoo10 단일).

**변경(DB 0 — data jsonb 자유 구조):**
- `dev/sales/orient.html`: 판매처 select→readonly "Qoo10" / 세일가·대형할인 칸 제거 / MARKETS·MARKET_EVENTS·marketOptionsHtml·eventOptionsHtml·onMarketChange 제거 / 판매 URL onSaleUrlInput·onSaleUrlBlur(normalizeUrl 보정)·toggleSaleUrlWarn(qoo10 미포함 경고) 추가 / collectCard sale={market:'Qoo10',url,price_regular} / clearCardTypeFields·미리보기 정리. 상시가 필수 검증 유지.
- `dev/js/admin-orient.js`: osCardDetail 세일가·대형할인 행 제거(판매처 Qoo10 폴백) / osBuildRewardNote price_sale·event·price_event 분기 제거.
- 빌드 산출물 `sales/orient.html`·`admin/index.html` 갱신.
- reverb-planner(경우의 수·하위호환)+reverb-reviewer GO. qa skip. 운영 보류(오리엔시트 전체).

## 구현 결과 — 발급 목록 삭제 (2026-06-25)

**관련 커밋:** (dev 머지 시 기록) / **worktree:** `feature/orient-form-cards`

### 사용자 확정 규칙
- 전체 관리자 삭제 가능 + 삭제 확인은 **브랜드명 재입력**(캠페인 삭제 패턴)
- **미발행(작성중·제출됨·만료)** = 오리엔시트 행만 삭제
- **발행됨(consumed)** = 연결 캠페인 중 **신청이 1건이라도 있으면 삭제 차단**, 전부 신청 0건이면 캠페인+시트 함께 삭제(`delete_brand` "연결 0건만 삭제" 정신)
- 발행 캠페인 삭제 시 인플 신청·결과물까지 소멸하는 파괴성 → "신청 있으면 차단"으로 보호

### 산출물
- **마이그레이션 199** `delete_orient_sheet(p_orient_id)` — 분기 기준은 status가 아니라 **발행 캠페인 연결 여부**(일부만 발행된 submitted 시트도 그 캠페인 보호 — 경우의 수 8 반영). 발행 캠페인 없음=`is_admin`·시트만 / 있음=`is_campaign_admin`·캠페인 FOR UPDATE 잠금→신청 체크→0건이면 일괄 삭제. SECURITY DEFINER+search_path 고정, REVOKE PUBLIC+GRANT authenticated. 반환 `{success, deleted_campaign_ids}` / `{success:false, reason:'blocked_has_applications'|'permission_denied'|'not_found', campaign_ids}`.
- **storage.js** `deleteOrientSheet(orientId)` — retryWithRefresh + rpc 래퍼.
- **admin-orient.js** — osRowHtml 「삭제」 버튼(빨강) + `osPublishedCampaignCount` + `osOpenDelete`/`osCheckDeleteConfirm`/`osExecuteDelete` + `orientDeleteModal`(발행 캠페인 N개 연결 시 경고 표시·브랜드명 재입력 시 버튼 활성).

### 기술 결정 / 검토
- DELETE 행 단위 보안 정책(super_admin 한정, 186)은 그대로 두고 SECURITY DEFINER RPC로 우회(RLS 완화 안 함).
- 경쟁 상태: applications↔campaigns 외래키 없어 "신청 체크~삭제" 완전 직렬화는 안 되나(발행 캠페인 즉시 삭제가 드물고 확인 모달 있어 허용 수준, supabase-expert 확인).
- reverb-supabase-expert GO(수정 없음) · reverb-reviewer GO · qa light.

### 배포 잔여 (운영 보류)
- 마이그레이션 199 개발/운영 DB 적용(개발 적용 후 스모크 3종: 미발행 삭제·신청 있는 발행 차단·신청 0건 발행 삭제).

## 구현 결과 — 예시 이미지·파일 업로드 (2026-06-26)

**관련 PR:** 인프라(저장소·정책) PR #593, 작성 폼 업로드 UI는 후속 PR.
**마이그레이션:** 200 (`orient_upload_bucket_and_policy`).

### 추가된 것
- **저장소(버킷) `orient-images`** — 공개 읽기, 파일당 10MB, 허용 형식 8종(jpg·jpeg·png·webp·pdf·docx·xlsx·pptx, **svg·html 제외**). 멱등 생성.
- **토큰 검증 함수 `orient_token_can_upload(p_token text) → boolean`** — SECURITY DEFINER(search_path='', REVOKE PUBLIC + GRANT anon/authenticated). orient_sheets에 익명 SELECT 정책이 0개라 storage 정책에서 직접 조회가 막히는 문제를 우회. text로 받아 내부에서 ::uuid 캐스팅을 예외 처리(비-uuid 폴더명이 정책 평가를 깨지 않게). 조건: status IN (draft,submitted) + 미만료.
- **storage.objects 정책 2개** — 익명 INSERT(`bucket_id='orient-images' AND orient_token_can_upload((storage.foldername(name))[1])` → 살아있는 토큰 폴더만), 익명+인증 SELECT(공개 읽기). UPDATE/DELETE 미부여.
- **작성 폼(orient.html)** — 이미지 행에 「파일 선택」 버튼 추가(URL 직접 입력 유지). 카드당 최대 5개. 이미지는 긴변 2048 JPEG 압축(HEIC→JPEG heic2any), 문서는 원본 업로드. 경로 `{token}/{난수}.{ext}` → public URL을 `data.cards[].images[].value`(type:'upload')에 저장. 업로드 중 제출 버튼 비활성, 실패 시 토스트 + URL 폴백.

### 기술 결정
- **공개 읽기 버킷** 채택(비공개+서명 URL 대비): 관리자 상세 모달(admin-orient.js)이 http/https 링크를 그대로 렌더 → 관리자/발행 측 무변경. 경로가 추측 불가 UUID 토큰 폴더라 노출 위험 낮음(기존 campaign-images 패턴과 동일).
- data.images 구조는 기존 `{type, value}` 재사용(하드코딩 'url' → 실제 종류 반영).

### 후속/백로그
- 만료·삭제된 오리엔시트 폴더의 고아 파일 정기 정리(현재 익명 DELETE 미부여 — 대시보드 수동 또는 후속 마이그레이션).
- 운영 적용: 운영 Supabase에 마이그레이션 200 실행(버킷·함수·정책) 후 운영 배포.

## 구현 결과 — 3필드 리치 텍스트 (2026-06-26)
- 작성 폼 카드별 **리뷰 가이드·금지 표현(NG)·추가 안내** 를 평문 → 간단 리치 텍스트(굵게/기울임/밑줄/취소선/링크). 관리자 기준데이터 미니 에디터와 동일 방식(contenteditable+execCommand)을 sales 정적 페이지에 인라인 이식 + DOMPurify@3.1.6 CDN.
- 익명 작성→관리자 열람 = 저장형 XSS 동선이라 **3중 sanitize**: 저장(richValue)·미리보기(pvRowHtml)·관리자 표시(osFieldHtml+sanitizeCautionHtml). 허용 태그 b/i/u/s/a/br/p, 이미지·script·svg·이벤트핸들러 차단, 링크 https/mailto만.
- 기존 평문 draft는 로드 시 줄바꿈 보존(richInitial). 형식 변경 시 contenteditable 초기화(clearCardTypeFields isContentEditable 분기). 관리자 가이드 초안 복사는 osStripHtml로 평문화.
- 버튼 B/I/U/S+링크(이미지 제외 — 예시 이미지 업로드 별도). 적용 3필드만. DB 변경 없음(jsonb 슬롯 동일). DOMPurify 미로드 시 태그 제거 폴백.
