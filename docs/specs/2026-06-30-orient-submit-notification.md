...
**상태:** ✅ **운영에 있음** (2026-09-02 기획 세션 확인 — Edge Function `notify-orient-submitted`(`origin/main`)). 배포 시점·마이그레이션 번호는 아래 「구현 결과」가 단일 소스다. ⚠️ 확인한 것은 **`origin/main` 에 함수 파일이 있다**는 것까지다 — 메일 함수는 git 머지와 실제 배포가 별개라, 배포 시각은 `supabase functions list --project-ref <운영>` 로만 확인된다.
