...
### 갭2 — campaign_manager의 캠페인 CUD·신청 승인 권한 (P2, 정책 모호)
- **현상**: `campaigns` CUD·`applications` 상태변경이 서버·클라 양쪽에서 `is_admin()`(전체) → **campaign_manager도 캠페인을 만들고/수정/삭제하고 신청을 승인·반려할 수 있음.**
- **쟁점**: 이름이 "campaign_**manager**"인데 실제로는 campaign_admin과 거의 동일한 쓰기 권한. 의도된 설계인지(매니저=실무 운영자라 캠페인·신청은 다룸), 아니면 manager는 읽기 위주여야 하는지 **정책 확정 필요**. 현재는 클·서 일치라 "버그"는 아니고 "정책 미정의".
...
## 정책 확인 필요 (사용자 결정 대상)
1. **campaign_manager의 범위**: 캠페인 CUD·신청 승인을 manager가 해도 되나(현행 유지) vs 읽기 위주로 좁히나(갭2).
2. **갭1(인플 민감정보) 개선 착수 여부**: 지금 별도 사양으로 잡을지, 백로그로 둘지.
3. **갭3(search_path) 정정**: 작은 작업이라 다음 DB 변경 때 끼워넣을지.
