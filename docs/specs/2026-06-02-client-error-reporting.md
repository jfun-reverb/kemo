...
**상태:** ✅ **운영에 있음** (2026-09-02 기획 세션 확인 — `report_client_error`). 배포 시점·마이그레이션 번호는 아래 「구현 결과」가 단일 소스다. ⚠️ 「구현 결과」가 `(개발 세션이 채울 것)` 인 채로 비어 있다 — 개발 세션이 채울 몫이다.
...
## 구현 결과
**구현일:** 2026-06-02 · **마이그레이션:** `165_client_error_logs.sql` · **★운영 배포 완료**

**운영에 있다는 근거**(2026-09-02 `origin/main` 산출물): `report_client_error` 인플루언서 2건·관리자 1건.

### 초안 대비 변경 사항
- **추가된 것 — 마스킹 강화**(`279_report_client_error_mask_auth_token.sql`). 오류 발생 화면에 `#reset-pw?token_hash=…` 가 그대로 기록돼 있던 것을 발견해, 자릿수 치환만으로는 부족하다고 보고 **마스킹 함수 자체**에 토큰 규칙을 넣었다.
- **추가된 것 — 「정상 거부」 구분**(`308_client_error_logs_is_expected.sql`). 서버가 마땅히 거부한 것(비밀번호 오입력·메일 미확인 등)까지 미해결 배지에 세어져, **늘 빨간 화면**이 되면 진짜 결함이 묻힌다. 기록은 하되 `is_expected=true` 로 갈라 배지에서 뺀다.
- **빠진 것 — 관리자 앱 오류 수집.** `collectClientError` 가 관리자 앱에서 무음이라 **관리자 화면 오류는 한 건도 안 쌓인다.** 화면의 「앱」 필터에 관리자 선택지가 있는데 결과가 늘 0이다 — 수집을 열지, 선택지를 뺄지 **미결**.

### 구현 중 기술 결정 사항
- **기록이 앱을 막지 않는다** — `logAppError` 는 절대 throw 하지 않고, 실패해도 조용히 지나간다.
- ⚠️ **정상 거부 목록(`APP_ERROR_EXPECTED_PATTERNS`)은 자리마다가 아니라 한 곳에 둔다** — 부르는 자리의 순서와 무관하게 걸리게 하기 위함이다(재전송 연타 안내를 넣고도 기록이 계속 쌓인 실측이 근거).
- 🔴 **다만 코드값이 여러 기능에서 겹치면 전역 패턴에 넣지 않는다** — 2026-09-02 실측: `invalid_status` 는 오리엔시트 발행·행사 뽑기·결과물 상태도 던지는 값이라, 문구로 뭉뚱그리면 그쪽 결함까지 묻힌다. 그런 것은 **부르는 자리에 목록을 넘긴다**(`CANCEL_APPLICATION_EXPECTED`).

⚠️ **이 절은 2026-09-02 에 뒤늦게 채웠다.**
