...
### 1-3. 웹 사양서와의 관계 (전제)
- 웹 사양서가 dev 에 들어가면 `base.css` 에 `--shell-max`(기본 480, `≥600` 에서 720)·`--reading-max`(560)가 생기고, `≥600` 블록 안에 카드 3열 · 시트 4자리 가운데 카드 560 · 읽기 폭 **20자리** · `#campSlider` **`min(--reading-max, 35vh)`** 정사각(⚠️ **웹은 지금도 이 값이고 정사각이다** — 앱만 달라졌다, §3-1) · `#appShell` 좌우 안전영역이 들어간다. 앱은 그 CSS 를 그대로 실으므로 **이 사양은 그 위에서 「덮어쓰기」로 동작한다.**
...
- 🔴 **`#campSlider` 는 앱에서 웹과 갈라졌다**(2026-09-14, 사용자 지적) — **정사각 고정을 없애고**(`application.js` 의 인라인 `aspect-ratio:1/1` 삭제) 칸이 **사진 자기 비율**을 따르게 했다. 폭 상한은 **`max-width:min(var(--reading-max), 50vh)`**.
  ...
  - ⚠️ **웹(dev)은 안 바뀌었다** — 여전히 정사각 + `35vh` 다. 이 변경은 **iOS 브랜치에만** 있다(§1-2 공용 파일 원칙). **웹 사양서를 고치지 말 것** — 그쪽은 웹을 정확히 적고 있다.
