...
- 응모이력 필터를 상단바로 올리고 내리는 `moveApplyFilterToGnb(on)` 은 `mypage.js` 에 있고 **부르는 자리가 셋**이다 — `navigate()`(`pageName!=='mypage'` 면 내림) · `openMypageSub`(`sub==='applications'` 면 올림) · `openMypageList`(무조건 내림. 「같은 페이지 안의 전환이라 `navigate` 의 복귀 로직이 안 걸린다」는 주석이 붙어 있다).
...
### 1-3. 웹 사양서와의 관계 (전제)
- 웹 사양서가 dev 에 들어가면 `base.css` 에 `--shell-max`(기본 480, `≥600` 에서 720)·`--reading-max`(560)가 생기고, `≥600` 블록 안에 카드 3열 · 시트 4자리 가운데 카드 560 · 읽기 폭 17자리 · `#campSlider` 560 정사각(가로일 때 `70vh`) · `#appShell` 좌우 안전영역이 들어간다. 앱은 그 CSS 를 그대로 실으므로 **이 사양은 그 위에서 「덮어쓰기」로 동작한다.**
...
- `:root{--reading-max:672px}` — 읽기 폭을 Apple 기준으로(웹의 17자리가 이 변수를 쓴다). **시트 4자리는 예외** — `.modal`·`#legalModal .legal-sheet`·`#policyNoticeModal .legal-sheet`·`.login-prompt-sheet{max-width:560px}` 를 다시 박는다(대화창은 672 가 너무 넓다). ⚠️ 웹 규칙과 **같은 선택자·같은 특이도**로 적는다(§1-2).
...
- **응모이력 필터의 자리 — 이 문서에서 여기 한 곳만 정한다.** 기준은 목적지도 경로도 아니라 **결과 상태 하나**: 🔴 **응모이력 목록이 지금 화면에 있으면 상단바, 없으면 목록 안.** 왼쪽이든 오른쪽이든 보이기만 하면 상단바다(응모이력 탭은 오른쪽, 응모이력→상세·메시지 짝은 왼쪽 — 셋 다 상단바 / 마이페이지 탭·캠페인 짝·짝 없는 화면은 목록 안).
  ...
  - ⚠️ **기존 호출 셋은 손대지 않는다** — `navigate()` 의 `pageName!=='mypage'`(`app.js`) · `openMypageSub` 의 `moveApplyFilterToGnb(sub==='applications')` · **`openMypageList` 의 `moveApplyFilterToGnb(false)`**(`mypage.js` — 「응모이력 → 마이페이지는 같은 페이지 안의 전환이라 `navigate` 의 복귀 로직이 안 걸린다」는 주석이 붙어 있다) 셋 다 그대로. **새 가지가 늘 그 뒤**라 기존 호출과의 순서는 따지지 않아도 된다(웹·`fill` 에서는 분할 가지가 안 돌아 기존 동작 그대로).
...
### 3-6. 검증 (완료 기준)
1. **웹 무변화(양성 대조)** — 기준은 **iOS 브랜치의 착수 직전 커밋**(dev 판이 아니다 — iOS 브랜치는 dev 에 없는 마크업·분기를 이미 갖고 있어 dev 와는 원래 다르다). 착수 전에 그 커밋의 웹 빌드(`index.html`)를 로컬 미리보기로 띄워 크롬 **390·1180** 폭에서 홈·목록·상세·마이페이지(응모이력·프로필)·메시지 6화면을 찍어 두고, 구현 뒤 같은 조건으로 다시 찍어 **픽셀 단위로 같다**(웹에는 `data-layout`·`data-split` 이 안 붙는다). 🔴 0이 아니면 게이트가 샌 것.
