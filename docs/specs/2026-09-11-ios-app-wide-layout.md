...
### 1-3. 웹 사양서와의 관계 (전제)
- 웹 사양서가 dev 에 들어가면 `base.css` 에 `--shell-max`(기본 480, `≥600` 에서 720)·`--reading-max`(560)가 생기고, `≥600` 블록 안에 카드 3열 · 시트 4자리 가운데 카드 560 · 읽기 폭 **20자리** · `#campSlider` **`min(--reading-max, 35vh)`** 정사각(2026-09-11 정정 — 방향별 규칙 없음) · `#appShell` 좌우 안전영역이 들어간다. 앱은 그 CSS 를 그대로 실으므로 **이 사양은 그 위에서 「덮어쓰기」로 동작한다.**
...
- `#campSlider` 는 웹 규칙 그대로 — **`max-width:min(var(--reading-max), 35vh)`**(2026-09-11 정정. 옛 「560 정사각·가로 `70vh`」는 **사용자 지적으로 바뀌었다** — 정사각이라 폭을 키우면 높이가 따라 커져 PC 에서 화면의 70%를 먹었다).
  - 🔴 **`--reading-max` 를 672 로 덮어도 이 사진은 안 커진다 — 높이(`35vh`)가 잡아 주기 때문이다.** 그 안전판이 **높이 상한 하나에 걸려 있다.**
...
**2026-09-14 — 정사각 고정 제거**(iOS 세션). 위 2026-09-11 정정이 높이는 잡았지만 **회색 여백**을 남겼다 — 칸은 정사각인데 사진 대부분이 가로로 긴 배너라 위아래가 비었다(사용자 지적). 이제 `aspect-ratio:1/1` 을 **없애** 칸 높이 = 폭 × 사진 비율이고, 폭 상한은 **`min(var(--reading-max), 50vh)`**(높이가 폭에 딸려 오므로 기준을 키웠다).

🔴 **위 §3-1 의 「`35vh`」 서술과 그 아래 경고는 이 변경으로 낡았다** — 안전판이 **폭 상한 하나**로 바뀌었을 뿐 「하나에 걸려 있다」는 경고 자체는 그대로 유효하다(iOS 테마가 `#campSlider` 를 따로 적을 때 폭 상한을 빠뜨리면 **앱에서만** 터진다). 본문 정정은 **기획 세션에 넘겼다.**

⚠️ 함께 확인한 것 — 세로 화면은 여백만 사라지고 배치가 그대로다. 가로 화면의 2단 배치(사진 옆 머리 정보)·안내 띠 2열·정보표 2열도 아이패드 실기에서 정상.
