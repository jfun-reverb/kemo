...
### 1-3. 웹 사양서와의 관계 (전제)
- 웹 사양서가 dev 에 들어가면 `base.css` 에 `--shell-max`(기본 480, `≥600` 에서 720)·`--reading-max`(560)가 생기고, `≥600` 블록 안에 카드 3열 · 시트 4자리 가운데 카드 560 · 읽기 폭 **20자리** · `#campSlider` 폭 상한(⚠️ **값과 정사각 여부는 웹 사양서가 정본이다** — 여기에 숫자를 적지 않는다. 앱이 무엇을 덮는지는 §3-1) · `#appShell` 좌우 안전영역이 들어간다. 앱은 그 CSS 를 그대로 실으므로 **이 사양은 그 위에서 「덮어쓰기」로 동작한다.**
...
- 🔴 **`#campSlider` 는 앱에서 웹과 갈라졌다**(2026-09-14, 사용자 지적) — **정사각 고정을 없애고**(`application.js` 의 인라인 `aspect-ratio:1/1` 삭제) 칸이 **사진 자기 비율**을 따르게 했다. 폭 상한은 **`max-width:min(var(--reading-max), 50vh)`**.
  - **왜 35 → 50 인가** — 정사각일 때는 폭 상한이 곧 높이 상한이었지만, 이제 **높이가 폭에 딸려 온다.** 같은 몫을 유지하려면 기준을 키워야 한다(가로로 긴 배너면 50vh 폭이 **약 28vh 높이**).
  - **왜 없앴나** — 높이는 35vh 로 잡혔는데 **칸이 정사각이고 사진 대부분이 가로로 긴 배너**라 위아래 회색 여백이 크게 남았다.
  - 🔴 **`--reading-max` 를 672 로 덮어도 이 사진은 안 커진다 — `50vh` 가 잡아 주기 때문이다. 그 안전판이 그 값 하나에 걸려 있다.** ⚠️ **축이 높이에서 폭으로 바뀌었다** — `50vh` 는 이제 **폭**을 화면 높이로 재는 값이다. 지우면 `672` 가 그대로 먹는다.
  - ⚠️ **이 변경은 웹에도 이식하기로 정해졌다**(2026-09-14 사용자 결정 — 처음엔 앱 전용이었다). **웹 쪽 값·서술은 웹 사양서가 정본**이고, 갈라지는 것은 **가로 2열 배치와 iOS 훅 클래스**뿐이다(§3-5 예외 항목).
  - 🔴 **여기에 「웹은 지금 이렇다」를 적지 않는다** — 처음엔 「앱만 달라졌다」고 적었는데 **하루 만에 뒤집혔다.** 웹 값을 이 문서가 따라 적으면 웹이 바뀔 때마다 **여기가 조용히 거짓이 된다.** 웹 값이 필요하면 웹 사양서를 본다.
  - 🔴 **그래서 iOS 테마가 `#campSlider` 를 따로 적을 때(여백 뚫기 `width:calc(100% + 32px)`) 높이 상한을 반드시 함께 적는다.** 폭만 적으면 **앱에서 기둥까지 풀려 있어**(§3-1) 사진이 웹보다 더 커진다. ⚠️ 이 사양서가 기둥을 푸는 유일한 문서라, **이 함정은 앱에서만 터진다.**
...
🔴 **「화면 안쪽 모양은 안 바꾼다」에 예외가 생겼다**(2026-09-14, **사용자가 화면을 보며 직접 지시**) — 가로에서 상세 화면을 **①머리 정보를 사진 오른쪽 열로 ②인스타그램·LINE 안내 띠를 2열로 ③캠페인 정보표를 2열로** 바꿨다(+ 위 대표 사진 정사각 해제). **이 사양이 스스로 금지한 자리라 예외로 명시해 둔다** — 다음 사람이 「규칙 위반」으로 되돌리지 않게.
  - ⚠️ **2·3단계에 영향 둘**: ⓐ그 가로 배치는 `data-layout`(fill·split·triple)과 **무관하게** `@media (min-width:600px) and (orientation:landscape)` 로만 걸려 있어, **모드를 넣을 때 조건이 겹친다.** ⓑ사진 폭이 `50vh` 라 **화면 높이 기준**이어서, `split`·`triple` 에서 상세가 **좁은 칸**에 들어가면 폭이 그 칸이 아니라 화면 높이를 따라 **어긋날 수 있다.** 🔴 **둘 다 `split` 착수 때 먼저 푼다**(㉠ 보다 앞).
  - ⚠️ 이 변경은 **「웹 동작 변화 0」(§3-6 ①)의 예외**이기도 하다 — `application.js` 를 고쳤으므로 iOS 브랜치의 웹 빌드에서도 사진 모양이 바뀐다. **사용자 지시로 의도한 것**이라 회귀가 아니다.
- 로딩 방식 A/B/C · 앱스토어.
...
## 웹 쪽 변경으로 이 사양서가 고쳐진 기록

**2026-09-11 — 상세 대표 사진 높이 상한**(웹 병합 요청 #1496). 웹 사양서 §3-2 가 `#campSlider` 를 읽기 폭 560 에만 묶었는데, **정사각이라 폭을 키우면 높이가 따라 커져** PC 1440×780 에서 화면의 **70%**를 먹었다(사용자 지적). 웹이 `max-width:min(var(--reading-max), 35vh)` 로 고치고 **가로 전용 `70vh` 규칙을 삭제**했다.

이 사양서에서 고친 것은 **낡은 서술 두 줄**(§1-2 웹 전제 목록 · §3-1 덮어쓰기 목록)과 **새 경고 하나**다 — 🔴 **`--reading-max` 를 672 로 덮어도 사진이 안 커지는 것은 높이 상한이 잡아 주기 때문이고, 그 안전판이 하나뿐**이라 iOS 테마가 `#campSlider` 를 따로 적을 때 높이 상한을 빠뜨리면 **앱에서만** 터진다(기둥까지 풀려 있어 웹보다 크게).

⚠️ **점검 회차를 다시 돌리지 않았다** — 바뀐 것이 「사실 정정 두 줄 + 경고 하나」라 설계 결정이 안 바뀌었다. 표·단계와 맞물리는지만 확인했다(규칙 D 「본문에 넣는다가 회차를 다시 돌린다는 아니다」).
