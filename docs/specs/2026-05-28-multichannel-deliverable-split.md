...
- 다중 캠페인 엑셀: **채널 조합별 그룹 시트 분리** (→ §6-2 참조. 초안 「합집합 분리」 → 사용자 검수에서 단일/다중 형식 일치를 위해 변경)
...
**다중 캠페인 결과물 엑셀** (`exportSelectedCampaignsDeliverables`) — **채널 조합별 그룹 시트 분리 (확정)**:
선택한 캠페인들의 채널 구성이 다를 수 있어 초안에서는 「합집합 분리 vs 단순 라벨」 양안 권고였으나, 사용자 검수에서 **단일 캠페인 엑셀과 형식을 일치**시키는 방향으로 확정 (같은 정보 단위가 단일/다중에서 동일하게 한 행으로 정렬되도록).
- 선택 캠페인을 **정렬된 채널 키 기준으로 그룹화**한 뒤 그룹마다 별도 시트 생성
- 시트명: `리뷰 · {채널 라벨1+채널 라벨2+…}` (예: `리뷰 · Qoo10`, `리뷰 · Qoo10+LIPS`, `리뷰 · @cosme`)
- 시트명 구분자는 `+` 사용 (ExcelJS 시트명 금지 문자 `/`·`\`·`?`·`*`·`:`·`[`·`]` 회피)
- 각 그룹 시트 컬럼: 캠페인 2 + 인플 7 + 영수증 9 + 채널 N×6 (그 그룹의 채널 조합 그대로 펼침)
- 비monitor 캠페인 + 채널 미지정 monitor 캠페인은 마지막 「통합 시트」(기존 24컬럼) 1장에 모음 — 해당 캠페인이 1건이라도 있을 때만 시트 생성(빈 시트 회귀 방지)
- 「합집합 분리」 안을 채택하지 않은 이유: 채널 구성이 다른 캠페인들을 한 시트에 펼치면 공란 셀이 대거 발생하고 신청자별 한 행 패턴이 깨짐. 「단순 라벨」 안을 채택하지 않은 이유: 단일 캠페인 엑셀과 컬럼 구조가 달라져 같은 데이터를 두 가지 형식으로 읽어야 함

→ 단일 캠페인 = 채널 컬럼 분리, 다중 캠페인 = **채널 조합 그룹 시트 분리**, 두 형식이 한 캠페인 단위 보기에서 동일하게 보임
...
- **PR 3 — 엑셀 채널별 컬럼 분리**:
  ...
  - 다중 캠페인 엑셀은 §6-2 확정대로 **채널 조합별 그룹 시트 분리**
...
- ~~다중 캠페인 엑셀 정책 (§6-2 합집합 분리 vs 단순 라벨)~~ → **확정 완료**: 채널 조합별 그룹 시트 분리 (§6-2 참조)
- 검수 모달 「다른 채널 상태 안내 박스」 위치·디자인
- 알림 라벨 일본어 문안 (예: 「「@cosme」レビュー」 표기)
...
- **PR 4 (사양 2 PR 3)** — 엑셀 채널별 컬럼 분리. 단일 캠페인은 6컬럼×채널 수 펼침, 다중 캠페인은 **채널 조합별 그룹 시트 분리** (PR #340·#341·#342 구현 완료, §6-2 참조)
...
사양 §6-2 권고(단순 라벨 vs 합집합 분리) → 실제 구현은 **채널 조합별 그룹 시트 분리** (제3 옵션). 사용자가 단일·다중 형식 일치 의도("같은 정보를 한건만 vs 여러건")를 명시. **사양서 §6-2 본문 갱신 완료(2026-05-28)** — 권고 양안 자리에 확정 결과·근거·시트명 규칙이 들어가 있음. §11 PR 3·§14 추후 확인 디테일도 동기화 완료.

> ⚠️ **2026-06-08 번복:** 이 「채널 조합별 그룹 시트 분리」 결정은 사용자 재요청으로 **단일 「리뷰어 결과물」 탭 통합**으로 변경됨. 아래 「구현 결과 — 다중 엑셀 단일 「리뷰어 결과물」 탭 통합 (2026-06-08)」 참조.

## 구현 결과 — 다중 엑셀 단일 「리뷰어 결과물」 탭 통합 + URL 출력 (2026-06-08, PR #453)

**구현일:** 2026-06-08
**관련 커밋:** 289f46a (dev 머지 `c30aa27`, PR #453) / 운영 배포 dev→main PR

### 변경 사항 (§6-2 「채널 조합별 그룹 시트 분리」 번복)
다중 캠페인 결과물 엑셀의 **채널 조합별 그룹 시트 분리**(§6-2, 2026-05-28 확정)를 **단일 「리뷰어 결과물」 시트 통합 + 채널 전체 합집합 컬럼**으로 변경.

- **이전:** 큐텐 단독 → `리뷰 · Qoo10`, 큐텐+엣코스메 → `리뷰 · Qoo10+@cosme`, @cosme 단독 → `리뷰 · @cosme` … 채널 구성마다 별도 탭
- **이후:** 모집 타입이 같은(monitor) 캠페인은 채널 구성과 무관하게 **「리뷰어 결과물」 한 탭**. 채널은 선택 캠페인 전체의 **합집합 컬럼**으로 가로 펼침. 1신청 = 1행, 본인이 낸 채널 칸만 채우고 안 낸 채널은 빈칸. 인플루언서·영수증 정보는 행당 1회만.
- 비monitor(기프팅·방문형) + 채널 없는 monitor 캠페인은 기존대로 마지막 「결과물」 통합 시트로 폴백.

### §6-2 우려를 수용한 이유 (의도 번복)
§6-2는 「합집합 분리」를 채택하지 않은 이유로 ①채널 구성이 다른 캠페인을 한 시트에 펼치면 공란 셀 대거 발생 ②신청자별 한 행 패턴이 깨짐 을 들었다. 이번에 사용자는 **모집 타입이 같은 리뷰어인데 채널 조합 차이로 탭이 갈리면 같은 인플루언서·영수증 정보가 탭마다 반복돼 취합이 어렵다**는 운영 불편을 우선해 단일 탭을 명시 요청. 합집합 공란 셀은 감수(미제출 채널 칸은 빈칸), 신청자별 한 행 패턴은 오히려 유지됨(1신청 1행).

### 추가 변경 — URL 그대로 출력
영수증·리뷰 이미지 URL 셀을 `「영수증 보기」`·`「리뷰 이미지 보기」` 라벨 하이퍼링크에서 **URL 문자열 그대로 표시**(클릭은 유지)로 변경. 게시물(post) URL 셀과 형식 일치. 적용 범위: 다중 통합 시트·단일 캠페인 시트·`_buildMonitorGroupSheet` 전부.

### 구현 디테일
- `dev/js/admin-excel.js` 다중 선택 함수: 채널 조합 그룹화(`monitorGroups`)·`pickGroupSheetName`·`usedSheetNames`·`groupKeys` 루프 제거 → `monitorCovered`(리뷰어 캠페인 id 집합) + `monitorChannelSet`(채널 합집합)으로 단순화. `_buildMonitorGroupSheet`를 1회만 호출(시트명 `리뷰어 결과물`).
- `_buildMonitorGroupSheet(... , titleOverride)` 파라미터 추가 — row1 제목을 `리뷰어 결과물 통합 (N개 캠페인)`으로. 기존 호출처 없어 하위 호환 무영향.
- URL 셀: `{text: <url>, hyperlink: <url>}`. `renderReceipt9`의 `base[5].text = '영수증 보기'` 덮어쓰기 줄 제거.
- 단일 캠페인 엑셀(`exportCampaignDeliverables`)은 원래 단일 시트라 탭 구조 무변경 (URL 출력만 동반 변경).
- DB·RLS·약관 영향 없음. reverb-reviewer GO 2회(통합 변경 + URL 변경), qa-tester skip(순수 클라이언트 ExcelJS 로직).
