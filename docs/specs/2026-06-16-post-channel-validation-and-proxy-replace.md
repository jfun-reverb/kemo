...
총 13건(모두 instagram 캠페인): 반려 8(lips 4·other 3·x 1) + 검수중 3(x·tiktok·other) + **승인 2(x→instagram, 삭제 보호 — 잘못 승인 의심, 운영 수동 판단 필요)**. 반려·검수중 11건은 super_admin이 검수 화면 「채널 불일치 게시물 삭제」로 정리(가려진 행은 빌더 보완으로 노출됨). 게시물 2건+ 신청 20개(2건 13·3건 6·4건 1).

### 현재 기준 분류 전환 (사용자 추가 요청 2026-06-16) — 구현 완료
사용자 불만: 인증성공인데 과거 채널 미지정 레거시 행 때문에 「채널 미분류」 목록에 계속 잡혀 혼란. 사용자 결정: ①미분류 숨김=채널 인증샷 모두 승인 ②불일치 숨김=같은 신청에 승인된 정상 게시물 있을 때만 ③목록·필터·배지는 숨기되 검수 모달 정리 박스는 유지.
- **PR1 채널 미분류 숨김**(`_finalizeMonitorReprs`): `repr==='approved'`(채널별 리뷰 모두 승인)면 stateSet에 `legacy_no_channel` 미추가 → 목록·필터·건수 자동 반영. 검수 모달 unassignedBox 유지. dev PR#507
- **PR2 채널 불일치 배지 숨김**: `buildDeliverableGroups`에 `g.posts` 보존 → `renderDelivAppRow`가 `hasValidApprovedPost`(같은 신청에 캠페인 채널 일치 승인 post 존재) 계산 → `renderDelivStatusCell(opts)` 배지 조건에 `&& !hasValidApprovedPost`. 검수 모달 mismatchedBox 유지. 빌더 공유 함수(countCertSuccess·운영현황·진행현황) g.posts 미참조 → 회귀 없음. dev PR#508
- DB 무변경. reviewer GO 2회. 검수 모달 정리 박스(unassignedBox·mismatchedBox)는 allDelivs 별도 계산이라 목록 판정과 분리 — 과거 문제 행 정리 경로 보존.
