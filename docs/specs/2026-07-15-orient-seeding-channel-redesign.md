# 오리엔시트 시딩 — 채널 목록·선택 방식 개편 + 레버브 요구사항 필드

**작성일:** 2026-07-15
**작성 세션:** 기획
**관련 화면:** 브랜드 셀프 오리엔시트 작성 폼(시딩 카드) + 관리자 오리엔시트 상세·발행

> 브랜드가 작성하는 시딩 오리엔시트의 「채널별 게시 가이드」를 ① 채널 목록을 인스타그램-피드/릴스로 세분화하고 ② 드롭다운 단일 선택 → 체크박스 다중 선택으로 바꾸고 ③ 소구 키워드를 통합 1칸으로 단순화하며 ④ 폼 마지막에 「레버브 측 요구사항(선택)」 필드를 추가한다. 상위 사양서 `docs/specs/2026-06-18-brand-self-orient-sheet.md` §15 형식 재설계의 후속.
