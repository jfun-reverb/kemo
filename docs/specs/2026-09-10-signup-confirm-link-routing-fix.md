...
- ⚠️ **초대 링크 복귀(`consumeInviteReturn`)는 가입 확인 착지 경로에서 원래도 안 돌았고 지금도 안 돈다** — `SIGNED_IN` 핸들러의 `if (!currentUser)` 안에만 있는데, 이 착지에서는 세션 복원이 먼저 `currentUser` 를 세운다(수정 전에는 재설정 플래그가 먼저 빠져나갔다). 이번 범위 밖 — 별개 결함으로 기획 판단.
...
- 개발서버 시나리오 **1·2(실제 확인 링크)·5(실제 재설정)·6** — (아래 갱신)
