...
**작성일:** 2026-07-14
**작성 세션:** 기획/설계
**상태:** 🔴 **종결 (2026-07-16)** — 시스템 연동 취소, LINE 공식계정 매니저 **운영**으로 대체. 근거 = 딥리서치 2회(§구현 결과).

> ⚠️ **결론 먼저 (다음 세션 필독):** 이 사양서의 "LINE 대화를 우리 화면에서 주고받기" 방향은 **LINE 구조상 실현 불가**로 판명됐고, 사용자의 진짜 니즈(대화방이 많아 어느 방이 명단 누구인지 식별 안 됨)는 **LINE 매니저 자체 기능(채팅 표시이름 별명 편집 — 무료)** 으로 개발 없이 해결한다. 상세·근거는 맨 아래 「구현 결과」. 아래 초안 본문은 **검토 이력**으로만 보존.

> (초안 의도) 아웃바운드 인플루언서 명단(`outbound_influencers`, 구글시트 917명 이관 예정)의 나노~메가 인플루언서와 LINE 공식계정 `@reverb.jp`로 주고받는 대화를, 관리자(영업) 화면에서 직접 주고받을 수 있게 통합한다. LINE Messaging API(공식계정) 기반.
...
## 구현 결과 — 종결: 시스템 연동 취소, 운영으로 대체

**종결일:** 2026-07-16 · **판단 세션:** 기획/설계 · **근거:** 딥리서치 2회(작업 ID `w9h51bw8j`, `wcscbfe4n` — LINE Developers·LY Corp 1차 문서)

### 왜 방향을 바꿨나 (니즈 재정의)
초안은 "LINE 대화를 우리 관리자 화면에서 주고받기"를 전제했으나, 사용자 실제 문제는 **대화 통합·미읽음 추적이 아니라 "LINE 대화방이 많은데 어느 방이 명단(`outbound_influencers`)의 누구인지 식별이 안 됨"** 하나였다. 이 관점으로 재조사한 결과 **시스템 연동으로 풀 문제가 아님**이 확정됐다.

### 딥리서치 판정 (전부 LINE 공식 1차 문서, 핵심은 신뢰도 high)
1차 조사(경량 연동 실현성):
- **"LINE 열기" 딥링크 = 불가** — userId로 특정 대화방을 여는 공식 URL 스킴 없음(여는 방향은 유저→공식계정뿐).
- **미읽음/미응답 추적 = 불가** — 영업이 LINE 매니저에서 보낸 답장은 **어떤 웹훅으로도 우리에게 안 옴**. 공식 통계 API도 매니저 발신은 집계 제외.
- **완전 무저장 = 불가 / 본문 무저장+메타 자체저장 = 부분 가능** — 대화 목록 조회 REST API 없음(웹훅 이벤트를 자체 저장해 재구성해야 함).
- **★결정타(신뢰도 medium)** — 오퍼레이터가 chat.line.biz에서 쓰는 **Chat 모드는 웹훅을 자동 비활성화** → "영업은 LINE 앱에서 채팅 + 우리 화면은 보기만"이 양립 불가.

2차 조사(식별 해법 비교):
- **해법1 = LINE 매니저 자체 기능(개발 0) = 정답** — 채팅 우측 **연필 아이콘으로 상대 표시이름을 별명 편집**(최대 20자, 관리자 전용·유저 미표시/미통지, **무료**). 한 번 매칭해 별명을 명단 이름으로 바꾸면 이후 계속 식별. 태그는 무료 총 5개·대화당 1개(2026-03 축소)라 다수엔 부족 → 실질 운영은 **유료 챗Pro 옵션(월 3,000엔+세, 태그 300개·대화당 30개+태그 필터+5년 이력, 일본 제공)**. 노트는 무료 대화당 1개.
- **해법2 = 개인화 링크 자동 매핑 = 영업 매니저 화면 식별엔 도움 안 됨** — 기술은 성립(친구추가 웹훅 userId + Account Link linkToken/nonce)하나 **매핑은 우리 백엔드에만 존재, LINE 매니저 채팅 UI엔 반영 불가**(API로 매니저 표시이름·태그 자동설정 없음 — Users 엔드포인트 읽기전용). 표준 follow 이벤트는 경로 추적 불가라 LIFF 중간페이지 우회 별도 필요. 결국 우리 별도 조회화면에서만 유효.

### 결정 (2026-07-16 사용자 확인)
- **LINE 연동 시스템 개발 안 함.** 우리가 만든 매핑은 LINE 매니저 화면에 안 떠서 영업 실무 식별에 기여 못 함(역효과). → 사양서 종결.
- 식별 문제는 **LINE 공식계정 매니저 운영으로 해결**:
  1. 영업이 대화 상대를 명단과 한 번 매칭한 뒤, 채팅 우측 **연필로 표시이름을 「명단 이름(등급 등)」으로 편집** (무료·즉시).
  2. 대화방 수가 많아 별명만으로 부족하면 **챗Pro 옵션(월 3,000엔) 도입 후 태그로 분류**(등급·상태 필터). 도입 여부는 실제 대화방 규모 보고 판단 — 현재 미도입.
  3. 우리 명단 화면(`/admin#outbound`)은 "별명에 무엇을 적을지" 참조 정보(이름·등급·단가) 제공 역할.

### 후속 (필요 시)
- 위 1·2 절차를 실무자 가이드(`docs/OPERATOR_GUIDE.md`)에 반영할지는 별도 판단(현재 미반영).
- 실계정 검증 남음: Chat 모드에서 유저 인바운드 웹훅이 실제로 죽는지 / 무료 태그 현행 한도(5·1) — 다만 채택 방향(별명 편집)은 이 검증과 무관하게 무료로 동작.
- 근거 메모리: `project_line_integration_feasibility`.
