...
## 추가 검토·설계 항목 (2026-07-14 완성도 보강)

> 초안 작성 후 완성도 관점 재검토에서 발견. 놓치기 쉬운 순.

### 🔴 선결 조건 (정하지 않으면 나머지 설계가 헛돎)
1. **LINE 매니저 앱과 "이중 응대" 충돌** — 영업이 현재 LINE 공식계정 매니저(앱/웹)에서 직접 채팅 중. 우리 플랫폼이 같은 채널을 붙이면 **발신 창구 2곳**이 됨. 영업이 LINE 앱에서 보낸 답장은 우리 DB에 안 남고 기록이 갈라짐. → **응대를 플랫폼으로 일원화할지, 병행할지** 정책 먼저 결정.
2. **자동응답(응답 모드) 설정 충돌** — LINE 공식계정 「응답 모드」(챗봇/수동 채팅/둘 다)와 기존 인사말·자동응답이 웹훅 흐름과 부딪힘. 채널 설정 전제 조건 정리.
3. **선행 의존성 — 아웃바운드 명단 운영 미배포** — `outbound_influencers`(917명)는 현재 dev만·운영 미배포. LINE 연동은 그 위에 얹으므로 **명단 운영 배포가 선행**. 순서 강제.

### 🟠 기술 설계 필수
4. **일↔한 번역** — 명단은 일본 인플, 영업은 한국어. `translate-message`(마이그레이션 235, Google 번역) 패턴을 그대로 응용 → 인플 일본어 원문 + 한국어 번역 병기. **우리가 이미 가진 자산 재사용.**
5. **미디어 저장** — LINE 이미지는 LINE 서버에서 일정 시간 후 만료. 인플이 보낸 사진을 즉시 우리 Storage로 다운로드·저장하는 파이프라인 필요(content API). 스티커·동영상 표시 방식.
6. **친구 라이프사이클 상태** — follow/unfollow/block 이벤트 → 명단에 "연결됨/차단됨/미연결" 상태 배지. 차단되면 발신 실패 처리.

### 🟡 운영·UX 보강
7. **담당자 배정** — 917명을 여러 영업이 나눠 관리 시 담당자별 인박스·미응답 필터.
8. **온보딩 흐름** — 첫 컨택(LINE 밖) → 공식계정 추가 유도 → 매핑까지 실제 영업 동선. 추가 유도 QR/링크에 명단 식별값 심기 검토(경우의수 2와 연결).
9. **새 메시지 알림** — 도착 시 영업에게(탭 배지·알림·폴링).
10. **대화 보존·감사** — 영업 대화는 계약 근거 → 삭제·보존 정책, 권한별 열람.
11. **비용 재검증** — API push가 「1:1 수동 채팅 무료」에 해당하는지 개발/운영 실측 후 LINE 플랜 확정(리스크로 이미 명시, 플랜 확정 전 필수).
