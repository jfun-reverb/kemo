...
## 사용자 확인 필요
1. **번역 범위** — 양방향 전부 vs 인플 일본어→관리자 한국어만?
2. **표시 방식** — 원문+번역 병기 / 번역만(원문 접기) / 토글?
3. **번역 플랫폼** — DeepL Free / Google / LibreTranslate(셀프호스팅)?
4. (파이프라인 아키텍처 A/B/C는 §의심 1 권고=B로 진행, 개발 세션 최종 확정)
