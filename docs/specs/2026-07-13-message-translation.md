...
**작성일:** 2026-07-13
**상태:** ✅ **운영에 있음** (2026-09-02 기획 세션 확인 — `body_translated`(관리자 7곳·인플루언서 3곳)). 배포 시점·마이그레이션 번호는 아래 「구현 결과」가 단일 소스다.
...
- **추가된 것**: 관리자 받은편지함 미리보기·대화 내 검색이 한국어 번역본도 활용(`fetchMessagePreviews` select 확장 + `searchAdminMsg` 필터 확장). **운영팀 본인 발신 메시지에도 일본어 번역 서브 표시**(2026-07-13 사용자 후속 요청 — 인플에게 실제 나간 일본어를 관리자가 검수 가능, 캡션 「인플루언서 화면에는 위 일본어가 본문으로 표시됩니다」).
