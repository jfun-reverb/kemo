...
3. **번역 플랫폼** — ✅ **DeepL API 무료**(월 50만 자). ⚠️PR3(약관) 착수 전 확인: **DeepL API Free plan의 텍스트 보존·학습 정책**(웹 무료 번역기와 API는 조건이 다름 — API는 미저장 명시 여부를 딥리서치로 검증). 메시지 본문이 해외(DeepL) 전송되므로 개인정보처리방침 국외이전 기재 필수.
