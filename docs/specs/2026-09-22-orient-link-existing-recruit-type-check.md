...
### 배포·검증 기록
- 개발: 464 개발 DB 적용 → #1653. 운영: 적용 전 운영 함수가 237 판(형식 검사 없음)인 것을 조회로 확인 → 464 운영 DB 적용(검사 포함·비로그인 실행 불가 확인) → #1655 로 이 변경만 골라 운영 관리자 화면에 반영(2026-09-22, 오리엔시트 묶음 제외)
- 개발서버 브라우저 확인: V1(리뷰어 카드 → 리뷰어 캠페인만) · V7(정상 연결 → 「연결(발행)되었습니다」, 확인 뒤 연결 해제로 원복) · V5(시딩 카드 → 「이 브랜드에 시딩 캠페인이 없습니다…」) · V4(리뷰어·방문형 7개 브랜드의 시딩 카드 → 0건) · V6(서버 직접 호출 → `recruit_type_mismatch`, 카드 무변경)
- ⚠️ 미확인: V2(가구매 카드 — 개발서버에 대상 시트 없음, 코드 검토로만) · V8(옛 구조 다중 카드는 V1·V5 에서 2카드 시트로 함께 봄) · 운영 화면에서 직접 눌러 보지는 않음
