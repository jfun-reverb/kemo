...
**작성일:** 2026-06-04
**작성 세션:** 기획/설계
**상태:** ★ 운영 배포 완료 (2026-06-05, PR #436 · main 9391c80). 아래 「구현 결과」 참조.
...
> **★ 운영 배포 완료: 2026-06-05 (PR #436, main `9391c80`)** — PR 1·2·3 + 후속 손질 일괄 배포.
> 커밋: PR 1 `500defa` / PR 2·3 `da61b45` / 회사 관리 정리 `30c5689` / 브랜드 상태 한글 `23f21b2` / 브랜드번호 이동 `b25dc7d`.
> 운영 DB 마이그레이션 170: **백필 대상 0건**(운영 brands에 자유텍스트 미연결 회사명 없음) — 실행해도 무효, 일관성만 확보. qa full 6/0 PASS.
>
> **사양서 범위 밖 후속 손질(같은 배포에 동봉):**
> - 회사 관리: 목록 담당자 컬럼 + 모달 담당자 입력칸 3종 제거(담당자는 브랜드 정보로 일원화). **DB `contact_*` 컬럼·기존 값은 보존**(화면에서만 제거, `fetchCompanies` 조회 쿼리에서도 제외). 상태 표시 한글화(활성/보관).
> - 브랜드 관리: 상태 배지 한글화(활성/비활성). 브랜드번호(`brand_no`) 열 삭제 → 브랜드명 셀 위 작은 글씨로 이동(데이터·검색 유지).
> - 브랜드 상세 모달 담당자 행 리사이즈 넘침 수정(`.brand-contact-row > input { min-width:0 }`).
> - 회사↔브랜드 상태는 **독립**(회사 보관해도 하위 브랜드 status 불변 — `archiveCompany`는 `companies.status`만 UPDATE).
> - 백로그: 회사 `contact_*` DB 컬럼 완전 삭제를 원하면 별도 마이그레이션 필요.

### PR 1 — 회사 정보 백필 (마이그레이션 170)

**구현일:** 2026-06-05
**관련 커밋·PR:** `500defa` — `supabase/migrations/170_brands_company_backfill.sql` (PR #436)
**상태:** ★ 운영 배포 완료. 운영 백필 대상 0건, 개발 신규 회사 1건 검증.
...
**구현일:** 2026-06-05
**관련 커밋·PR:** `da61b45` (PR #436)
**상태:** ★ 운영 배포 완료. reverb-reviewer GO + qa full 6/0 PASS.
