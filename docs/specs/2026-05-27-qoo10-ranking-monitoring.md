...
## 1. 데이터 모델 (마이그레이션 — 보류 해제 시 재채번)

> ⚠️ 본 보류 사양은 작성 시점 「153부터」를 가정했으나 그 후 **153 ~ 159가 모두 사용됨**(다중채널 결과물·정책 통지 등). 보류 해제·재설계 시점에는 `ls supabase/migrations/ | tail -3` 으로 최신 번호 확인 후 그 다음 번호 사용. 본 보류 사양 본문의 모든 「153」 / 「154」 / 「155」 표기는 「보류 해제 시점 재채번 번호 N · N+1 · N+2」로 읽을 것. 멀티 세션 규칙상 신규 번호는 한 세션에서만.
