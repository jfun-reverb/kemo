...
| 8 | ✅ **「앱 안 공지」 수단 — 결정됨(2026-09-15 사용자): 기존 `POLICY_NOTICE` 팝업·홈 배너만. 회원 메일은 보내지 않는다** | 사양서 「단계」 2 에 수단이 없었다(S5) | — |
...
| 11b | 방침 개정 공고 + 앱 안 공지 | `docs/PRIVACY_{kr,ja}.md` · `dev/lib/shared.js` `POLICY_NOTICE`(팝업·배너, 메일 없음) | §8.1 신설 · §5 한 줄 · 부칙 | 10 · 11a + 선결 7·8 | ✗ | 개발 + 사용자 |
...
- **담당 파일:** `docs/PRIVACY_kr.md` · `docs/PRIVACY_ja.md` · `dev/lib/shared.js` `POLICY_NOTICE`(2026-09-15 결정 — 팝업·홈 배너만, 회원 메일 없음)
...
- **주의:** 회원 메일(`notify-policy-change`)은 **쓰지 않기로 결정**했다 — 전 회원 발송이라 되돌릴 수 없어서. 바꾸려면 사용자 확인부터
