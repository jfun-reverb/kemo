...
**상태:** ✅ **관리자 화면은 운영에 있음** · 🔴 **인플루언서 노출은 마이그레이션 343 으로 제거됨** (2026-09-02 기획 세션 확인 — `admin-settlements`·`markSettlementPaid`). 배포 시점·마이그레이션 번호는 아래 「구현 결과」가 단일 소스다. 🔴 인플루언서 쪽은 「잠금」이 아니라 **코드에서 삭제**됐다. 되살리는 절차는 마이그레이션 343 의 「되돌리는 방법」.
