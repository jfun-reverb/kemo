...
**작성일:** 2026-05-14
**작성자:** 기획 세션
**구현 담당:** 개발 세션 (완료 — 2026-05-15)
**상태:** ✅ **운영 배포 완료 (마이그레이션 128, 2026-05-15)** — `order_number` 필드 추가 + `purchase_date`·`purchase_amount` 필수화 + 관리자 검수 모달 마스킹 해제 + `update_receipt_admin()` 원격 호출 함수 + `receipt_edit_history` 이력 테이블. 코드(app.js·admin.js)는 dev 잠재, main merge 보류 중
**실제 마이그레이션:** 128 (122~127이 다른 기능에 먼저 사용됨)
**관련 사양서:** 없음 (신규)
