...
| ID      | 기능            | 상세                                                |
| ------- | --------------- | --------------------------------------------------- |
| INF-060 | 계정 카드(이전) | 아바타(이름 첫 글자) + 이름 + SNS핸들 + 이메일 — 햄버거 메뉴 최상단으로 이전 |
...
| ID      | 기능           | 상세                                                                        |
| ------- | -------------- | --------------------------------------------------------------------------- |
| INF-080 | 햄버거 메뉴    | 계정 카드(아바타·이름·핸들·이메일) / 홈 / キャンペーン / 「マイページ」 접기·펼치기 아코디언(기본 펼침, 서브 7종 + 「未登録」 배지) / メッセージ(배지) / 通知(배지 9+) / ログアウト / 退会する (우측 슬라이드 패널, 인증 페이지 숨김). 마이페이지 랜딩 흡수(2026-05-21). flex 레이아웃에서 구분선·항목이 찌부러지지 않게 `.nav-menu>*{flex-shrink:0}` 적용 |
