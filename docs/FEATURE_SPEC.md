...
> 최종 업데이트: 2026-05-04 (v10)
>
> **v10 개정 (2026-04-25~30 운영 배포분)**
> - §5.5 `brand_applications.status` 10단계 확장 (`kakao_room_created` 추가) + 신청번호 포맷 v2(`JFUN-{R|S}-YYMMDD-NNN`, migration 078)
> - §5.11 신규 — `campaign_caution_history` audit 테이블 (migration 077, super_admin SELECT 한정)
> - §22.6 — 브랜드 서베이 상태 전이 9 → 10단계 (`kakao_room_created` 입금 ↔ 단톡방 개설 가시화)
> - §31 신규 — 캠페인 상태 도움말 모달 (관리 표 헤더 ⓘ)
> - §32 신규 — 캠페인·신청·결과물 표 brand_ko / product_ko 컬럼 분리 + 검색 매칭
> - §33 신규 — 민감 항목 변경 모달 + audit (`campaign_caution_history`)
> - §34 신규 — 참여방법/주의사항 편집 모달 분리 + bundle summary 한·일 양언어 + 미리보기 다국어
> - §35 신규 — 결과물 영수증 구매정보 관리자 화면 마스킹 (`purchase_date`/`purchase_amount`)
> - §36 신규 — 결정 이벤트 트리거 `draft↔pending` 양방향 허용
...
|-----------------------|-------------|------|
| id                    | uuid (PK)   | 자동 생성 |
| application_no        | text UNIQUE | `JFUN-R-YYMMDD-NNN`(reviewer) / `JFUN-S-YYMMDD-NNN`(seeding) 자동 채번. JST 기준, SECURITY DEFINER 트리거. **2026-05-04 migration 078**: v1 `JFUN-{Q|N}-YYYYMMDD-NNN`(Q→R, N→S, 연도 2자리)에서 변경, 기존 데이터 일괄 변환 |
...
### 5.11 campaign_caution_history (migration 077, 2026-04-30)
...
RLS: **SELECT는 `is_super_admin()` 한정** (일반 admin·인플루언서 모두 비공개). INSERT/UPDATE/DELETE 정책 미정의 — `record_caution_history()` SECURITY DEFINER RPC 경유 INSERT 만 허용 (campaign_admin 이상 가드, `SET search_path = ''`). 인덱스: `(campaign_id, changed_at DESC)`.

조회 진입점:
- 캠페인 더보기 메뉴 「변경 이력」 (super_admin 한정) → 타임라인 모달
- 인플루언서 응모이력의 「現在の文言と比較」 토글 → v2 스냅샷 ↔ 현재 비교

영향: PROJECT_CONTEXT §2-1에 audit 테이블 1줄 보강. 인플루언서 개인정보를 보유하지 않으므로 PIPA/APPI 영향 무관 (운영 감사용).
...
- 트리거:
  - `generate_brand_application_no` (BEFORE INSERT) — JST 기준 `JFUN-{R|S}-YYMMDD-NNN` 채번 (2026-05-04 migration 078: v1 `JFUN-{Q|N}-YYYYMMDD-NNN`에서 변경, 기존 데이터 일괄 변환)
...
## 33. 민감 항목 변경 모달 + audit — 2026-04-29 (migration 077, d79ad39)
...
### 33.2 경고 모달 (`#sensitiveChangeModal`)
...
### 33.3 audit 테이블 — `campaign_caution_history` (migration 077)
- 컬럼: `id uuid PK, campaign_id uuid FK ON DELETE CASCADE, changed_by uuid (auth.users), changed_at timestamptz default now(), field text (caution_items|participation_steps|...), before jsonb, after jsonb`
- INSERT는 `record_caution_history()` 트리거만 (수동 INSERT 차단)
- SELECT는 `is_super_admin()` RLS — 일반 admin·인플루언서 모두 비공개

### 33.4 동시 편집 잠금
- 함께 도입된 DB 잠금 마이그레이션이 캠페인 편집 시 advisory lock 획득 — 마지막 쓰기가 이전 변경을 모르고 덮어쓰는 사고 방지

### 33.5 영향 파일
- `supabase/migrations/077_*.sql` — `campaign_caution_history` 테이블 + audit 트리거 + DB 잠금
- `dev/admin/index.html` — `#sensitiveChangeModal` 마크업
- `dev/js/admin.js` — 변경 감지 + 모달 호출
