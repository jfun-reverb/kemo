...
### 22.6 상태 전이 — 9단계 파이프라인 (2026-04-22 확장, 2026-04-27 표시 순서 재정렬)

**표시 순서 (2026-04-27, PR #127 — 실제 영업 워크플로 반영)**:
```
new → reviewing → quoted → paid → orient_sheet_sent → schedule_sent → campaign_registered → done(최종완료)

rejected: any stage → rejected 로 분기 가능 (reviewing 이후 어느 단계에서든)
되돌리기: any stage → new 로 복귀 가능 (실수 되돌림)
```

> **2026-04-27 변경 이유**: 입금이 워크플로 거의 마지막에 위치한 이전 표시 순서가 영업팀의 실제 흐름(견적 수락 → 입금 → 오리엔테이션 → 일정 합의 → 캠페인 등록)과 어긋났음. **DB 데이터·status 값은 그대로**, 표시 순서·`BRAND_APP_STATUS_ORDER`·`BRAND_STATUS_ORDER_FOR_FUNNEL` 만 재정렬.

**이전 순서 (2026-04-22 ~ 2026-04-26)**:
```
new → reviewing → schedule_sent → quoted → orient_sheet_sent → campaign_registered → paid → done
```

**확장 이력:**
- migration 064 (2026-04-22): `schedule_sent`, `campaign_registered` 추가 (8단계). "완료" 라벨 → "최종완료"
- migration 065 (2026-04-22): `orient_sheet_sent` 추가 (9단계, quoted 뒤 단계)
- CHECK constraint로 확장만 허용, 기존 데이터(new/reviewing/quoted/paid/done/rejected) 무변경
