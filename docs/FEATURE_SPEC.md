...
| ID      | 기능           | 상세                                                                        |
| ------- | -------------- | --------------------------------------------------------------------------- |
| INF-080 | 햄버거 메뉴    | 계정 카드(이름·핸들·이메일 + 우측 알림 벨·배지, 아바타 없음) / 홈 / キャンペーン / 「マイページ」 접기·펼치기 아코디언(기본 펼침, 서브 7종 각 min-height:48px + 「未登録」 배지) / メッセージ(배지) / ログアウト / 退会する (우측 슬라이드 패널, 인증 페이지 숨김). 通知는 계정 카드 우측 벨로 통합(별도 항목 아님). 폼 화면 백버튼 없음. 마이페이지 랜딩 흡수(2026-05-22). flex 레이아웃에서 구분선·항목이 찌부러지지 않게 `.nav-menu>*{flex-shrink:0}`, 미읽음 수 `_lastUnread` 캐시로 재렌더 시 배지 즉시 복원 |
