...
### 2.7 마이페이지 (입력 폼 컨테이너 — 진입은 햄버거 메뉴, 2026-05-21 랜딩 흡수)

> 랜딩(목차) 화면 `#mypage-list` 는 제거되고 계정 정보·메뉴 목차는 GNB 햄버거 메뉴로 흡수됨(INF-080 참조). 마이페이지는 입력 폼 화면 7종의 컨테이너로만 남음.

| ID      | 기능            | 상세                                                |
| ------- | --------------- | --------------------------------------------------- |
| INF-060 | 계정 카드(이전) | 이름 + SNS핸들 + 이메일 + 우측 알림 벨(미읽음 배지) — 햄버거 메뉴 최상단으로 이전. 아바타 없음 |
| INF-061 | 메뉴 목차(이전) | 応募履歴, 基本情報, SNSアカウント, 配送先, PayPal, パスワード変更, メール受信設定 — 햄버거 「マイページ」 아코디언으로 이전. 미입력 항목 「未登録」 배지도 햄버거에 표시 |
| INF-062 | 상세 서브페이지 | 햄버거 서브항목 클릭 시 ← 뒤로가기 + 타이틀 헤더의 폼 화면으로 직행. 뒤로가기·기본 진입은 応募履歴 복귀(`closeMypageSub`) |
...
| INF-073 | 메일 수신 설정  | 캠페인 홍보 메일 ON/OFF 토글(ON=`resubscribe_marketing()` 동의시각 갱신, OFF=`marketing_opt_in=false`) + 업무 알림 메일 상시 발송 안내 (2026-05-20) |
| INF-074 | 메일 수신거부 라우트 | `#unsubscribe?token=` 1-click 수신거부. 비로그인 토큰만으로 `unsubscribe_by_token()` 호출, 성공/무효 화면 (2026-05-20) |
...
| ID      | 기능           | 상세                                                                        |
| ------- | -------------- | --------------------------------------------------------------------------- |
| INF-080 | 햄버거 메뉴    | 계정 카드(이름·핸들·이메일 + 우측 알림 벨·배지, 아바타 없음) / 홈 / キャンペーン / 「マイページ」 접기·펼치기 아코디언(기본 펼침, 서브 7종 각 min-height:48px + 「未登録」 배지) / ログアウト / 退会する (우측 슬라이드 패널, 인증 페이지 숨김). メッセージ 메뉴 항목 제거(응모이력과 목적지 중복, 응모건 카드 메시지 버튼으로 진입). 通知는 계정 카드 우측 벨로 통합(별도 항목 아님). 폼 화면 백버튼 없음. 마이페이지 랜딩 흡수(2026-05-22). flex 레이아웃에서 구분선·항목이 찌부러지지 않게 `.nav-menu>*{flex-shrink:0}`, 미읽음 수 `_lastUnread` 캐시로 재렌더 시 배지 즉시 복원 |
| INF-081 | 바텀탭 (deprecated) | 2026-04 햄버거 메뉴로 대체. 키보드 간섭 제거 목적                          |

### 2.10 응모건 메시지 (인플루언서 ↔ 관리자, PR 1·2 — 개발 검증 중)
- **인플루언서(PR 1):** 응모이력 카드의 「メッセージ」 버튼(미읽음 배지) → 게시판형 **페이지** (`#page-messages`, 해시 `#messages-{id}`, `dev/js/messaging.js` — 2026-05-22 모달→페이지 전환: 모바일 키보드가 모달을 가리던 문제를 `#appShell` 키보드 패턴 상속으로 해결). 헤더 뒤로가기→응모이력, 취소(cancelled) 응모는 진입 차단(toast). 발송: 텍스트 + 이미지 첨부(클라 자동 압축 2048px·HEIC→JPEG, 최대 5장). 본인 25분 내 회수(첨부 즉시 삭제). 숨김·회수 메시지 본문/첨부 가림(placeholder)
- **인플루언서 GNB·알림(PR 2):** ~~햄버거 메뉴 「メッセージ」 항목~~ → **2026-05-22 제거**(응모이력과 목적지 중복). 응모건 메시지는 응모이력 카드의 메시지 버튼(미읽음 배지 `_myMsgUnreadByApp`)으로 진입. 관리자 답장 시 앱 내 알림 `message_received` → 알림 모달 클릭 시 메시지 모달 직접 오픈(kind 로 분기 — application_cancelled 와 ref_table 공유). 알림 진입점은 햄버거 계정 카드 우측 벨
...
| ADM-055 | 설문 메일받기 토글 | **DEPRECATED (migration 103, 2026-05-11)** — `admins.receive_brand_notify` 단일 컬럼 방식은 §40 「관리자 메일 수신 설정」 신규 테이블 `admin_email_subscriptions` 로 교체됨. Edge Function 도 `get_subscribed_admin_emails('brand_notify')` 원격 호출 함수(RPC) 경유로 변경. 본 행은 마이그레이션 전 이력 기록용 |
...
| receive_brand_notify | boolean | **DEPRECATED (migration 103)** — `admin_email_subscriptions` 테이블로 교체. §40 / §5.12 참조. 컬럼은 호환 위해 유지(다음 사이클 안정성 확인 후 별도 마이그레이션에서 DROP 예정) |
| created_at | timestamptz   | 생성일                                             |
...
| ~~admin_memo~~        | ~~text~~    | **migration 122 (2026-05-14) DROP** — multi-entry `brand_application_memos` 로 최종 이전 (123). §48-2 참조 |
...
| quote_sent_url        | text NULL   | 견적서 URL (migration 112, 2026-05-12). http/https CHECK. URL 입력 시 스킴 없는 입력은 `https://` 자동 prefix (2026-05-15) |
| ~~orient_sheet_sent_at~~ | ~~timestamptz~~ | **migration 126 (2026-05-15) DROP** — 운영자가 입력해온 「오리엔시트 전달 일자」가 실제로는 입금일 의미였음 → 신규 `paid_at` 컬럼으로 이전 후 컬럼 제거 |
| orient_sheet_sent_url | text NULL   | OT 시트 URL (migration 112). http/https CHECK. 날짜 분리(126) 후 URL 만 보존. 인라인 편집은 URL textbox 만 |
| paid_at               | timestamptz NULL | 입금 확인 일자 (migration 126, 2026-05-15). 운영자 수동 입력. 신청 목록 「입금 정보」 컬럼 뒤 「입금 날짜」 셀에서 인라인 편집 (체크박스 + date picker) |
| payment_flags         | jsonb NOT NULL DEFAULT '{}' | 입금여부 4종 체크 (migration 114~116, 2026-05-12). 키: `recruit`/`product`/`transfer`/`free` boolean. §48 참조 |
...
### 5.12 admin_email_subscriptions (migration 103, 2026-05-11)

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | uuid PK | 자동 생성 |
| admin_id | uuid FK | `admins.id` ON DELETE CASCADE |
| mail_kind | text | `lookup_values(kind='admin_email_kind')` code 참조 |
| created_at | timestamptz | 기본 now() |
| UNIQUE | (admin_id, mail_kind) | 중복 구독 방지 |

목적: 관리자별 메일 수신 설정. 종전 `admins.receive_brand_notify` 단일 컬럼 방식을 복수 메일 종류를 지원하는 범용 테이블로 교체.

행 단위 보안 정책(RLS): SELECT 모든 관리자, INSERT/UPDATE/DELETE 본인 또는 super_admin.

메일 종류 카탈로그(`lookup_values(kind='admin_email_kind')` 활성 항목 — 마이그레이션 164 이후):
- `brand_notify` — 광고주 신청 접수 알림
- `daily_digest` — 관리자 일일 통합 메일 (구 `application_cancel`·`application_received` 흡수)
- `campaign_promo` — 캠페인 홍보 메일

헬퍼 함수: `get_subscribed_admin_emails(p_mail_kind text)` — 해당 종류 구독 관리자 이메일 배열 반환. Edge Function 수신자 조회에 사용.

### 5.13 [dev 검증 중] ng_sets (migration 107, 2026-05-12, 운영 미배포)

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | uuid PK | 자동 생성 |
| name_ko | text UNIQUE | 번들 이름 (한국어, 고유) |
| name_ja | text | 번들 이름 (일본어) |
| recruit_types | text[] | 적용 모집 타입 배열 (`monitor`/`gifting`/`visit`) |
| items | jsonb NOT NULL DEFAULT '[]' | NG 항목 배열. 항목 구조: `{html_ko, html_ja}` |
| sort_order | integer | 표시 순서 |
| active | boolean DEFAULT true | 활성 여부 |
| created_at / created_by | timestamptz / uuid | 생성 일시·관리자 |
| updated_at / updated_by | timestamptz / uuid | 수정 일시·관리자 |

목적: NG 사항(금지 행동) 번들 관리. `caution_sets` 패턴 완전 미러링. 기존 `campaigns.ng` (Quill rich text) 컬럼 대체.

행 단위 보안 정책(RLS): SELECT `is_admin()` 관리자 전용, INSERT/UPDATE/DELETE `is_campaign_admin()` 이상.

캠페인과의 관계:
- `campaigns.ng_set_id uuid FK ON DELETE SET NULL` — 원본 번들 참조
- `campaigns.ng_items jsonb NOT NULL DEFAULT '[]'` — 캠페인 저장 시 스냅샷 복사
- 인플루언서는 `campaigns.ng_items` 스냅샷만 경유 (번들 수정 후에도 기존 캠페인 무영향)

items 구조 상세:
- `html_ko` — 한국어 NG 항목 (HTML, DOMPurify sanitize, inline 서식만 허용)
- `html_ja` — 일본어 NG 항목 (동일)
- 이미지(`<img>`) 포함 가능 — `uploadContentImage()` 경유, Supabase Storage `campaign-images/content/` 저장

### 5.14 [dev 검증 중] campaign_caution_history NG 확장 (migration 109, 2026-05-12, 운영 미배포)

migration 077(§5.11)에 컬럼 4종 추가:

| 컬럼 | 타입 | 설명 |
|------|------|------|
| ng_set_id_prev | uuid | NG 번들 변경 전 ID (NULL 허용) |
| ng_set_id_next | uuid | NG 번들 변경 후 ID (NULL 허용) |
| ng_items_prev | jsonb | NG 항목 변경 전 스냅샷 (NULL 허용) |
| ng_items_next | jsonb | NG 항목 변경 후 스냅샷 (NULL 허용) |

`record_caution_history()` 원격 호출 함수(RPC) 시그니처 확장(11→15개 파라미터, 신규 4개 모두 DEFAULT NULL — 기존 호출처 하위 호환).

closed 캠페인의 NG 변경 차단 트리거: migration 108이 075 확장 (caution/participation/ng 모두 차단).
...
- **수신자** (migration 103, 2026-05-11 갱신): `get_subscribed_admin_emails('brand_notify')` 원격 호출 함수(RPC) 가 반환한 이메일 + env `NOTIFY_ADMIN_EMAILS`(콤마 구분) 외부 이메일 합산(중복 제거). 이전 `admins.receive_brand_notify=true` 단일 컬럼 방식은 deprecated — §40 참조
- **폴백**: RPC 조회 실패 시 env만 사용
...
- **상세 모달**: 제품 테이블(name/url/price/qty), `final_quote_krw` 수동 입력, `quote_sent_at` 체크, 상태 드롭다운, "되돌리기"(new 복귀). **사업자등록증 다운로드 섹션은 2026-04-21 제거** (migration 057)
- **제품별 multi-entry 내부 메모 (`brand_application_memos.product_idx`, migration 122/123/124/125, 2026-05-14)**: 신청 단위 단일 `admin_memo` 컬럼 → multi-entry 메모 테이블로 단계 이전. 신청 목록 표의 「내부 메모」 셀이 **모든 제품 행마다** 표시. 셀에 최신 메모 1줄 + 분홍 미확인 카운트 배지 + ✎ 모달 진입 버튼. ✎ 클릭 시 모달 헤더에 "신청번호 · 제품명 — 내부 메모" + 메모 N개 누적(작성자·시각·수정/삭제). 모달 진입 시 그 페어 메모 일괄 `mark_brand_app_memos_read` 처리로 본인 셀 배지 0. 변경 이력 모달에 `memo_added/edited/deleted` + 제품 표시. legacy backfill 데이터는 `(자동 이전)` 표기
- **입금 날짜 (`paid_at`, migration 126, 2026-05-15)**: 기존 「오리엔시트 전달」 컬럼이 실제로는 입금일 의미였음. 「입금 정보」 열 뒤 신규 「입금 날짜」 컬럼에서 인라인 편집(체크박스 + date picker). 오리엔시트 전달 셀은 URL 만 표시·편집
- **URL 자동 prefix (2026-05-15)**: 견적서/오리엔시트 URL 입력 시 스킴 없는 입력은 `https://` 자동 prefix (`example.com` → `https://example.com`). `safeBrandUrl` 화이트리스트(http/https) 검증 유지로 위험 스킴 차단
...
> **⚠️ 2026-05-15 이후 정책 변경**: 본 §35의 마스킹은 §49(영수증 필수 필드 강화)에서 해제됨. 관리자 검수 시 마켓 주문 대조 위해 `purchase_date`·`purchase_amount` + 신규 `order_number`를 다시 노출. §49 참조.
...
## 40. 관리자 메일 수신 설정 — 2026-05-11 (migration 103)

### 40.1 개요
관리자별로 수신할 메일 종류를 개별 on/off 설정. 종전 `admins.receive_brand_notify` 단일 boolean에서 `admin_email_subscriptions` 테이블(§5.12) 기반 복수 메일 종류 구조로 전환.

### 40.2 UI — 관리자 계정 페인 (`/admin#admin-accounts`)
- 각 계정 행의 「메일받기」 셀에 현재 수신 중인 메일 종류 회색 칩 표시
- 「설정」 버튼 → 메일 수신 설정 모달 오픈
- 모달: 메일 종류별 체크박스 (`lookup_values(kind='admin_email_kind')` 카탈로그 기반)
- 권한 규칙: super_admin은 다른 관리자 설정도 편집 가능, 그 외는 본인 설정만
...
### 40.4 수신자 조회 흐름
- Edge Function → `get_subscribed_admin_emails(p_mail_kind)` 원격 호출 함수(RPC) 호출
- 반환값 + 환경 변수 `NOTIFY_ADMIN_EMAILS` (중복 제거) → 합산 발송
- DB 조회 실패 시 `NOTIFY_ADMIN_EMAILS` 만으로 폴백

### 40.5 신규 메일 종류 추가 방법
`lookup_values` 테이블에 `kind='admin_email_kind'` 행 1건 INSERT만으로 가능 — 마이그레이션 불필요.

### 40.6 영향 파일
- `supabase/migrations/103_admin_email_subscriptions.sql`
- `dev/js/admin.js` — 메일 수신 설정 모달 + 칩 표시
- `dev/lib/storage.js` — `fetchAdminEmailSubscriptions` / `upsertAdminEmailSubscription`

### 40.7 약관·개인정보처리방침 영향
없음 (관리자 내부 운영 설정, 개인정보 수집 변경 없음).

---

## 41. 인플루언서 응모 취소 — 2026-05-11 (migration 104)

### 41.1 개요
인플루언서가 본인 신청을 직접 취소. 결과물 승인 완료 건은 취소 불가, 구매·방문 기간 이후 취소는 사유 입력 강제 + 동의 체크박스 추가. 관리자에게 즉시 알림(admin_notices) + 일일 요약 메일(§46).

### 41.2 취소 가능 조건
| 상태 | 취소 가능 여부 |
|------|------|
| pending (심사 대기) | ✅ 가능 |
| approved (승인) — 결과물 승인 없음 | ✅ 가능 |
| approved — 결과물 1건 이상 approved | ❌ 불가 (「이미 검수 승인된 게시물이 있어 취소할 수 없습니다」 알림) |
| rejected (비승인) | ❌ 불가 |
| cancelled | ❌ 이미 취소됨 |

### 41.3 취소 사유 (lookup_values kind='cancel_reason', 시드 6종)
| 코드 | 한국어 | 일본어 |
|------|------|------|
| `schedule_unavailable` | 일정 문제 | スケジュールの都合 |
| `personal_reason` | 개인 사정 | 個人的な理由 |
| `product_mismatch` | 상품 불일치 | 商品の相違 |
| `delivery_issue` | 배송 문제 | 配送の問題 |
| `account_change` | SNS 계정 변경 | SNSアカウント変更 |
| `other` | 기타 | その他 |

구매/방문 기간 이후 취소 시 `cancel_after_purchase_start` 코드 추가 노출 (violation_reason 에 추가, migration 104).

### 41.4 취소 단계 (cancel_phase)
- `recruit` — 모집 기간 중 취소 (배송 전)
- `purchase` — 구매 기간 중 취소
- `visit` — 방문 기간 중 취소
- `post` — 게시물 제출 기간 중 취소
- `other` — 기타 단계
...
### 41.5 인플루언서 화면
- 응모이력 페이지에서 취소 가능한 신청 항목에 「취소」 버튼 노출
- 취소 모달: 사유 선택(드롭다운) + 기타 메모 입력
- 구매·방문 기간 이후: 추가 동의 체크박스 「상품 수령 후 취소는 위약금이 발생할 수 있습니다」 표시

### 41.6 취소 RPC — cancel_application(uuid, reason_code, reason_note, acknowledged)
- 본인 소유 신청 검증 (본인이 아니면 403)
- 결과물 승인 완료 건 차단
- 구매 기간 이후 취소 시 `acknowledged=true` 강제 (미전달 시 에러)
- `applications` 업데이트: `status='cancelled'`, `cancelled_at`, `cancel_reason`, `cancel_reason_code`, `cancel_phase`, `previous_status`
- `(user_id, campaign_id)` 부분 유니크 인덱스 변경: `cancelled` 행 제외 → 같은 캠페인 재응모 가능

### 41.7 관리자 알림
- 취소 직후 `admin_notices` 테이블에 자동 INSERT (모집 단계 `cancel_phase='recruit'` 제외 — 모집 중 취소는 일상적이라 과다 알림 방지)
- 전일 취소 건 요약 메일은 §46 참조

### 41.8 영향 파일
- `supabase/migrations/104_application_cancel.sql`
- `dev/js/application.js` — 취소 버튼 + 모달
- `dev/js/admin.js` — 취소 상태 배지·필터
- `dev/lib/storage.js` — `cancelApplication()` 함수

### 41.9 약관·개인정보처리방침 영향
- TERMS — 취소 조항 보강 (cancel_phase별 취소 규정, 구매 후 취소 위약금 안내)
- PRIVACY — `applications` 취소 컬럼 5종 수집 항목 추가 (cancel_reason 등 — 운영 분쟁 대응용)

---

## 42. NG 사항 번들화 — 2026-05-12 (migration 107-109, dev 검증 중, 운영 미배포)

### 42.1 개요
기존 `campaigns.ng` (Quill rich text 단일 컬럼)를 `ng_sets` 번들 테이블(§5.13) + 캠페인 스냅샷 방식으로 교체. caution_sets(§32) 패턴 완전 미러링.

### 42.2 관리자 화면 변경
**기준 데이터 페인 NG 탭** (PR-C, 진행 예정):
- 기존 lookup 방식(`lookup_values kind='ng_item'`) → 번들 CRUD로 전환
- 기존 시드 6건은 `active=false` 비활성 처리 후 「기본 NG 묶음」 번들 1건으로 흡수

**캠페인 등록·편집 폼** (NG-PR-B, dev):
- 기존 Quill 에디터 제거 → NG 번들 카드 + 편집 모달
- 번들 드롭다운(recruit_type 필터) + 「번들 다시 불러오기」
- 인라인 items 편집: 한국어/일본어 각 항목에 미니 에디터(이미지 포함 가능)
- 변경 경고 모달(sensitiveChangeModal) NG 트리거 추가
- closed/expired 캠페인 NG 수정 차단 (migration 108, 075 확장)

### 42.3 인플루언서 화면
- 캠페인 상세 NG 섹션: `campaigns.ng_items` jsonb 우선 렌더, 없으면 legacy `ng` 폴백
- 이미지 포함 NG 항목 표시 지원

### 42.4 이미지 첨부 (§43과 동일 패턴)
- 항목 편집 미니 에디터에서 이미지 버튼 → 파일 선택 → `uploadContentImage()` → Supabase Storage `campaign-images/content/`
- 이미지 클릭 시 크기 조정 팝오버 (Small 25%·Medium 50%·Large 75%·Original)
- 교차 사이트 스크립팅 방어: `_isAllowedContentImageSrc`로 https + `*.supabase.co` 이외 차단

### 42.5 캠페인 백필 정책
- 모든 기존 캠페인 `ng_items='[]'`로 시작 (백필 없음)
- 인플루언서 화면은 legacy `ng` 컬럼 폴백으로 기존 데이터 그대로 표시
- 기존 `campaigns.ng` 컬럼은 DEPRECATED — 1주 관찰 후 NG-PR-F에서 DROP 검토

### 42.6 영향 파일
- `supabase/migrations/107_ng_sets.sql` — ng_sets 테이블 + campaigns ng 컬럼
- `supabase/migrations/108_ng_lock_trigger.sql` — closed 캠페인 ng 변경 차단
- `supabase/migrations/109_caution_history_ng_columns.sql` — audit 테이블 NG 컬럼 추가
- `dev/js/admin.js` — NG 번들 카드·모달·변경 경고
- `dev/js/campaign.js` — 인플루언서 NG 렌더 (jsonb 우선 + legacy 폴백)
- `dev/lib/storage.js` — `fetchNgSets` / `upsertNgSet` / `deleteNgSet`

### 42.7 약관·개인정보처리방침 영향
없음 (NG 사항은 캠페인 가이드라인 — 개인정보 수집 없음).

---

## 43. 미니 에디터 이미지 업로드 + 크기 조정 — 2026-05-12

### 43.1 개요
캠페인 참여방법(`participation_steps`)·주의사항(`caution_items`)·NG(`ng_items`) 항목 편집용 미니 에디터에 이미지 첨부 기능과 삽입 후 크기 조정 팝오버 추가.

### 43.2 이미지 업로드 흐름
1. 미니 에디터 툴바 「이미지」 버튼 클릭
2. 파일 선택 다이얼로그 (제한: 5MB / jpg·png·webp)
3. `uploadContentImage(file)` 호출 → Supabase Storage `campaign-images/content/{uuid}.{ext}` 저장
4. 반환된 URL이 `<img class="rich-img" src="...">` 형태로 커서 위치 삽입
5. 평문 textarea 입력이었던 참여방법 desc 필드도 미니 에디터로 전환 (굵게·기울이기·밑줄·취소선·링크·이미지 사용 가능)

### 43.3 이미지 크기 조정 팝오버
- 삽입된 이미지 클릭 시 팝오버 노출
- 버튼 4개: Small(25%) / Medium(50%) / Large(75%) / Original(width 제한 해제)
- `style="width:X%"` 인라인 스타일로 적용
- DOMPurify 화이트리스트에 `style` 속성 포함 (width 한정)
- 팝오버는 이미지 우하단에 위치, 다른 곳 클릭 시 닫힘

### 43.4 보안
- 저장 시 `sanitizeCautionHtml` → `_isAllowedContentImageSrc` 화이트리스트 검증
  - 허용: https + `*.supabase.co` 도메인
  - 차단: 외부 도메인, data:, http:
- DOMPurify 저장+렌더 이중 sanitize

### 43.5 렌더링 하위호환
- legacy 평문 데이터: `miniRichHtml` 헬퍼가 `esc()` + 줄바꿈 폴백 처리
- 인플루언서 화면, 관리자 미리보기, 변경 이력 모두 `miniRichHtml` 헬퍼로 통일

### 43.6 줄바꿈 개선 (2026-05-12 ee2a500)
- 엔터키 입력 시 `<br>` 정확한 처리 (기존 `<div>` 중첩 문제 해결)

### 43.7 영향 파일
- `dev/js/admin.js` — 미니 에디터 이미지 버튼 + 크기 조정 팝오버
- `dev/lib/storage.js` — `uploadContentImage()` 함수
- `dev/lib/shared.js` — `sanitizeCautionHtml` / `_isAllowedContentImageSrc` / `miniRichHtml`

---

## 44. 브랜드 서베이 예상 견적 공식 업데이트 — 2026-05-12 (migration 111)

### 44.1 개요
광고주 신청(brand_applications) 제품 데이터에서 자동 계산되는 예상 견적(`estimated_krw`)의 리뷰어(reviewer) 타입 공식을 개선. 기존 고정값 1인당 2,500원 → 제품별 모집비(`recruit_fee_krw`) + 이체수수료(`transfer_fee_krw`) 합산 방식으로 전환.

### 44.2 products JSON 구조 (신규 필드)
```json
{
  "name": "상품명",
  "qty": 10,
  "price_jpy": 5000,
  "recruit_fee_krw": 3000,
  "transfer_fee_krw": 500
}
```
- `recruit_fee_krw`: 인원 1명당 모집 수수료 (원화)
- `transfer_fee_krw`: 인원 1명당 이체수수료 (원화)

### 44.3 예상 견적 계산 공식 (reviewer 타입)
```
estimated_krw = (total_jpy × 10) + Σ(recruit_fee_krw × qty) + Σ(transfer_fee_krw × qty)
```
seeding 타입은 변경 없음: `total_jpy × 10` 기본 공식 유지.

### 44.4 트리거 확장
기존 `BEFORE INSERT` → `BEFORE INSERT OR UPDATE OF products, form_type` 으로 확장.
기존에는 products 업데이트 시 estimated_krw가 재계산되지 않는 버그가 있었으며 이를 수정.

### 44.5 영향 파일
- `supabase/migrations/111_recalc_with_recruit_fee.sql`
- `dev/js/admin-brand.js` — 제품 폼 `recruit_fee_krw` / `transfer_fee_krw` 입력 필드 추가 (관리자가 신청 편집 시 입력)

### 44.6 약관·개인정보처리방침 영향
없음 (내부 견적 계산 로직 변경, 인플루언서/광고주 노출 없음).

---

## 45. 브랜드 서베이 URL 컬럼 추가 — 2026-05-12 (migration 112)

### 45.1 개요
광고주 신청 상세 모달에 견적서 URL과 OT 시트 발송 정보를 입력·관리할 수 있는 컬럼 3종 추가.

### 45.2 추가 컬럼 (brand_applications)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| `quote_sent_url` | text NULL | 견적서 URL 링크 (http/https CHECK) |
| ~~`orient_sheet_sent_at`~~ | ~~timestamptz NULL~~ | **migration 126 (2026-05-15) DROP** — 운영자가 입력해온 「오리엔시트 전달 일자」가 실제로는 입금일 의미였음 → `paid_at` 으로 이전 |
| `orient_sheet_sent_url` | text NULL | OT 시트 URL 링크 (http/https CHECK). 날짜 분리(126) 후 URL 만 보존 |
| `paid_at` | timestamptz NULL | 입금 확인 일자 (migration 126, 2026-05-15) |

### 45.3 관리자 UI
- 상세 모달에 「견적서 URL」 입력 필드 (저장 시 클릭 가능 링크로 표시)
- 「OT 시트 발송 일시」 날짜·시각 입력 + 「OT 시트 URL」 입력 필드
- URL은 `http://` 또는 `https://` 스킴만 허용 (CHECK 제약 + 클라이언트 검증)
- 담당자가 구글 드라이브 등 외부 문서 링크를 직접 관리 가능

### 45.4 영향 파일
- `supabase/migrations/112_brand_app_url_columns.sql`
- `dev/js/admin-brand.js` — 상세 모달 URL 입력 필드 + 링크 렌더
- `dev/lib/storage.js` — `updateBrandApplication()` 파라미터 확장

### 45.5 약관·개인정보처리방침 영향
없음 (내부 운영 URL 관리, 인플루언서 개인정보 관련 없음).

---

## 46. 응모 취소 일일 요약 메일 — 2026-05-12 (migration 113)

> **[DEPRECATED 2026-06-02, migration 164]** 본 독립 메일은 `notify-admin-daily-digest`(관리자 일일 통합 다이제스트) §2 응모 취소 섹션으로 흡수됨. Edge Function `notify-application-cancelled-daily` 및 메일 템플릿은 제거 완료(cron 은 2026-05-18 해제). 아래 내용은 이력 참고용.

### 46.1 개요
전일(한국 시각 0~24시) 동안 발생한 인플루언서 응모 취소를 하루 1회 관리자에게 요약 메일로 발송. 건별 즉시 알림(§41.7 `admin_notices`) 외 일별 집계 리포트 추가.

### 46.2 발송 스케줄
- Edge Function: `notify-application-cancelled-daily`
- pg_cron: 매일 UTC 00:00 (= 한국시간 09:00) 자동 호출
- 수신자: `get_subscribed_admin_emails('application_cancel')` ∪ 환경 변수 `NOTIFY_ADMIN_EMAILS`

### 46.3 발송 조건
- 전일 취소 건수 = 0: **미발송** + `application_cancel_digest_runs.status='skipped_no_data'` 로그
- 같은 날짜 중복 호출: UNIQUE 제약으로 자동 차단 (멱등성 보장)
- 대상 취소: `cancel_phase != 'recruit'` 인 취소만 포함 (모집 단계 취소는 일상적이라 요약 제외)

### 46.4 메일 내용
- 제목: 「[REVERB JP] 전일 응모 취소 요약 — N건」
- 본문: 캠페인명 / 인플루언서 이름 / 취소 단계 / 취소 사유 / 취소 일시 테이블

### 46.5 로그 테이블 — application_cancel_digest_runs
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | uuid PK | 자동 생성 |
| run_date | date UNIQUE | 발송 기준 일자 (중복 방지) |
| cancel_count | integer | 집계된 취소 건수 |
| status | text | `sent` / `skipped_no_data` / `failed` |
| sent_at | timestamptz | 발송 시각 |
| recipient_count | integer | 수신자 수 |

### 46.6 영향 파일
- `supabase/migrations/113_application_cancel_digest_infra.sql` — 로그 테이블 + pg_cron 등록
- `supabase/functions/notify-application-cancelled-daily/index.ts` — Edge Function 본체
- `supabase/functions/notify-application-cancelled-daily/README.md`
- `docs/email-templates/application-cancelled-daily.html` — 메일 템플릿 (source of truth)
- `scripts/sync-email-templates.sh` — 템플릿 동기화 스크립트 업데이트

### 46.7 약관·개인정보처리방침 영향
없음 (내부 관리자 메일 — 외부 발송 아님, 새 개인정보 수집 없음).

---

## 47. [dev 검증 중] 캠페인 상태 재설계 — 2026-05-07 (migration 097, 운영 미배포)

### 47.1 개요
기존 5단계(`draft/scheduled/active/paused/closed`)에서 `paused` 제거 + `expired` 신규 추가. closed/expired 분리로 게시 종료 단계를 명확화.

### 47.2 상태 변경
| 기존 | 변경 | 설명 |
|------|------|------|
| paused | **제거** | 일시정지 기능 폐지 |
| closed | 유지 (의미 변경) | 모집 종료, post_deadline까지 인플루언서 화면 노출 유지 |
| (신규) expired | **추가** | post_deadline 경과로 인플루언서 화면에서 완전 비노출 |

최종 5단계: `draft / scheduled / active / closed / expired`

### 47.3 자동 전이 규칙
- `recruit_start` 도래: `scheduled → active` (기존 유지)
- `deadline` 경과: `active → closed` (기존 유지)
- `post_deadline` 경과: `closed → expired` (신규)

### 47.4 시각 분류
- **노출 그룹** (컬러 배지): `scheduled`(파랑) / `active`(초록) / `closed`(핑크)
- **비노출 그룹** (회색·점선): `draft` / `expired`

### 47.5 영향 파일
- `supabase/migrations/097_campaign_status_redesign.sql`
- `dev/js/admin.js` — 상태 드롭다운·배지·도움말 모달 업데이트
- `dev/js/campaign.js` — 인플루언서 화면 expired 캠페인 비노출 처리
- `dev/lib/storage.js` — `autoCloseCampaigns()` + `autoExpireCampaigns()` 추가

---

## 48. 브랜드 서베이 「입금여부」 컬럼 — 2026-05-12 (migration 114·115·116)

### 48.1 개요
브랜드 서베이 신청 목록 페인(`/admin#brand-applications`)에 「오리엔트 전달」 오른쪽으로 「입금여부」 컬럼 신설. 4종 체크 상태(모집비용·상품비용·이체수수료·무료모집)를 관리자가 칩 클릭으로 직접 토글하거나 새로고침 아이콘으로 자동 채울 수 있다. products 변경 시 DB 트리거가 자동 재계산.

### 48.2 데이터 모델
- `brand_applications.payment_flags jsonb NOT NULL DEFAULT '{}'` (migration 114)
- 키 4종 (boolean):
  - `recruit` — 모집비용 (Σ qty × recruit_fee_krw > 0)
  - `product` — 상품비용 (Σ qty × price > 0)
  - `transfer` — 이체수수료 (Σ qty × transfer_fee_krw > 0)
  - `free` — 무료모집 (관리자 명시 토글 전용, 자동 계산 대상 아님)

### 48.3 헬퍼·원격 호출 함수 (RPC)
- `calc_brand_app_payment_flags(p_products jsonb) RETURNS jsonb` (migration 114)
  - IMMUTABLE + SET search_path = ''
  - products 합계 기반 recruit/product/transfer 산출, free=false 고정
  - EXCEPTION WHEN OTHERS → 이상 데이터 시 모두 false 안전 폴백
- `recalc_brand_app_payment_flags(p_application_id uuid) RETURNS jsonb`
  - SECURITY DEFINER + is_admin() 가드 + SET search_path = ''
  - migration 115 (2026-05-12): free 보존 → false 리셋. 4종 모두 완전 초기화
  - 클라이언트 새로고침 아이콘 핸들러

### 48.4 트리거 (migration 116)
- `auto_recalc_brand_app_payment_flags()` 트리거 함수
- `trg_brand_app_auto_recalc_payment_flags BEFORE INSERT OR UPDATE OF products ON brand_applications`
- INSERT 시: calc 헬퍼 결과 그대로 (free=false)
- UPDATE 시: products IS DISTINCT FROM OLD.products 일 때만 발화. recruit/product/transfer 자동 재계산, free 키는 OLD 값 보존(관리자 명시 토글 보호)
- payment_flags 만 따로 변경하는 경로(칩 토글 UPDATE·새로고침 RPC)에는 발화 안 함

### 48.5 백필
- migration 114 끝에 `UPDATE brand_applications SET payment_flags = calc_brand_app_payment_flags(products) WHERE payment_flags = '{}'::jsonb`
- 운영 47건 일괄 적용. version 1씩 증가(touch 트리거) — 운영 적용 시점 사전 공지 권장

### 48.6 행 단위 보안 정책 (RLS)
- 기존 `brand_applications` 정책 그대로 — 관리자(`is_admin()`)만 SELECT/UPDATE/DELETE
- 컬럼 단위 별도 정책 없음

### 48.7 UI 사양
- 셀 구조: 세로 배치 4행 + 우측 새로고침 아이콘
- 칩 컬러:
  - 미체크: `#F0F0F0` 배경 + `#999` 글자
  - 체크 (recruit/product/transfer): `#FCE4EC` 배경 + `#B91D5F` 글자 (REVERB 핑크)
  - 체크 (free): `#E8F5E9` 배경 + `#16A34A` 글자 (초록 차별화 — 비용 청구 없음 의미)
- 무료모집 ON 시: 다른 3종 시각 숨김 (DB 값은 보존)
- CSS 스코프: `.brand-app-pay-cell` 안만 적용

### 48.8 영향 파일
- `supabase/migrations/114_brand_app_payment_flags.sql`
- `supabase/migrations/115_recalc_payment_flags_reset_free.sql`
- `supabase/migrations/116_brand_app_payment_flags_auto_trigger.sql`
- `dev/lib/storage.js` — `fetchBrandApplications` SELECT 에 payment_flags 추가, `updateBrandApplication` 반환 select 갱신, `recalcBrandAppPaymentFlags(id)` 신규
- `dev/js/admin-brand.js` — `renderBrandAppPaymentFlagsCell` + `toggleBrandAppPaymentFlag` + `refreshBrandAppPaymentFlags` + `_rerenderBrandAppPaymentCell` 신규, `_syncBrandAppCur` 에 payment_flags 동기화 추가
- `dev/admin/index.html` — thead 「입금여부」 헤더 + colspan 28→29
- `dev/css/admin.css` — `.brand-app-pay-cell` 스코프 + 칩 4종 + 새로고침 버튼 스타일

### 48.9 동작 매트릭스

| 액션 | recruit/product/transfer | free |
|---|---|---|
| 상세 모달 products 수정 후 저장 | 자동 재계산 (트리거 116) | OLD 값 보존 |
| 칩 직접 클릭 토글 | 단일 키만 변경 | 단일 키만 변경 |
| 새로고침 아이콘 (RPC 115) | 자동 재계산 | **false 리셋** (완전 초기화) |
| 신규 신청 INSERT (트리거 116) | products 합계 기반 자동 | false 고정 |

### 48.10 운영 적용 순서
운영 SQL Editor 에서 순차 실행:
1. `114_brand_app_payment_flags.sql` (컬럼 + 헬퍼 + RPC + 백필)
2. `115_recalc_payment_flags_reset_free.sql` (RPC 동작 변경)
3. `116_brand_app_payment_flags_auto_trigger.sql` (자동 트리거)

검증 SQL:
```sql
SELECT
  (SELECT count(*) FILTER (WHERE payment_flags='{}'::jsonb OR payment_flags IS NULL) FROM public.brand_applications) AS empty_count,
  (SELECT pg_get_functiondef('public.recalc_brand_app_payment_flags'::regproc) LIKE '%free 키도 false%') AS rpc_v2,
  (SELECT count(*) FROM pg_trigger WHERE tgname='trg_brand_app_auto_recalc_payment_flags') AS trigger_count;
```

기대값: empty_count=0, rpc_v2=true, trigger_count=1.

### 48.11 미해결 / 후속
- 칩 색상 톤은 운영 사용 후 운영팀 피드백에 따라 조정 가능
- 향후 「수동 모집비 확인」 같은 추가 플래그 필요 시 동일 payment_flags jsonb 에 키 추가 (마이그레이션 없이)

---

## 49. 영수증 필수 필드 강화 + 관리자 수정 + 변경 이력 — 2026-05-15 (마이그레이션 128)

### 49.1 개요
리뷰어(monitor) 캠페인 영수증 제출 시 **주문번호·구매일·구매금액 3종 필수 입력**으로 강화. 관리자 결과물 검수 모달에서 §35(마스킹) 정책을 **해제**하고 3종 값을 수정할 수 있게 변경. 모든 관리자 수정은 변경 이력 테이블에 자동 기록. 결과물 엑셀 export의 영수증 그룹을 6→9 컬럼으로 확장(주문번호·구매일·구매금액 모두 포함).

관련 사양서: `docs/specs/2026-05-14-receipt-required-fields.md`.

### 49.2 데이터 모델 (마이그레이션 128)
- `deliverables.order_number text NULL` — 영수증 주문번호. kind=receipt 행에서만 사용. 기존 행 NULL 허용 (백필 없음)
- `receipt_edit_history` 테이블 신규
  - 컬럼: `id bigserial PK`, `deliverable_id uuid NOT NULL FK CASCADE`, `changed_by uuid NOT NULL`, `changed_by_name text NOT NULL`, `changed_at timestamptz DEFAULT now()`, prev/next 6종 (order_number·purchase_date·purchase_amount × 2), `source text CHECK('admin_edit')`
  - 인덱스: `(deliverable_id, changed_at DESC)`
  - 행 단위 보안 정책(RLS): SELECT는 `is_admin()` 한정(campaign_manager 포함). INSERT/UPDATE/DELETE 정책 미정의 → 원격 호출 함수(RPC)만 INSERT 가능
  - `delete_admin_completely` cascade 체인: admins → applications → deliverables → receipt_edit_history 자동 정리

### 49.3 원격 호출 함수 (RPC)
- `update_receipt_admin(p_deliverable_id uuid, p_order_number text, p_purchase_date date, p_purchase_amount numeric) RETURNS void` (마이그레이션 128)
  - SECURITY DEFINER + SET search_path = ''
  - 권한 가드: `is_campaign_admin()` 미만이면 RAISE (PostgreSQL 표준 코드 42501)
  - 입력 검증: 주문번호 빈값·공백 차단, 200자 상한 / 구매일 NOT NULL / 구매금액 NOT NULL + 0 이상 (양수만이 아닌 0 이상 — 무료 시연 케이스 허용)
  - 대상 검증: kind='receipt'가 아니면 차단
  - no-op: 3종 모두 기존값과 동일하면 이력 미기록 후 RETURN
  - `FOR UPDATE` 행 잠금으로 동시 수정 충돌 방지
  - 변경 시 deliverables UPDATE + receipt_edit_history INSERT (prev/next 스냅샷)
  - GRANT EXECUTE TO authenticated

### 49.4 인플루언서 UI (`dev/index.html` + `dev/js/application.js`)
- 활동관리 영수증 폼 `#receiptForm` 안에 monitor 전용 컨테이너 `#monitorReceiptFields` 신규
  - 「주문번호」 한 줄 텍스트 (`#receiptOrderNumber`, maxlength=200)
  - 「구매일」 date input (`#receiptDate`)
  - 「구매금액 (円)」 number input (`#receiptAmount`, min=0)
- monitor 캠페인일 때만 표시 (visit·gifting은 숨김)
- `addDraftImage()` 함수에서 monitor 분기로 3종 필수 검증 + `insertDraftDeliverable` payload에 `order_number`/`purchase_date`/`purchase_amount` 추가 전달
- i18n 키 5종 신규 (ja/ko 양언어): `orderNumberLabel`, `purchaseDateLabel`, `purchaseAmountLabel`, `needOrderNumber`, `orderNumberTooLong`, `needPurchaseDate`, `needPurchaseAmount`, `invalidPurchaseAmount`

### 49.5 관리자 UI (`dev/js/admin.js`)
- 결과물 검수 모달 2종(`openDelivDetail` + `openDelivCombined`/`renderDelivPanelContent`)의 receipt 분기에 공통 헬퍼 `renderReceiptInfoBlock(d)` 호출
- 표시: 영수증 정보 박스 (주문번호 · 구매일 · 구매금액) — 값 없으면 빨간 「미입력」 placeholder
- 「수정」 버튼: campaign_admin 이상만 노출 (`isCampaignAdminOrAbove()` 가드 + RPC 가드 이중 방어)
  - 클릭 시 인플레이스 폼 전환 (저장/취소 버튼)
  - 저장 → `updateReceiptAdmin` RPC → 모달 + 결과물 페인 모두 재렌더 (`refreshPane('deliverables')`)
- 「변경 이력 보기」 토글: 모든 관리자 사용 가능 (campaign_manager 포함)
  - 클릭 시 `fetchReceiptEditHistory(id)` 결과를 타임라인으로 펼침 (관리자명 · 시각 · 필드별 prev → next 비교)
- review_image kind는 영수증 정보 블록 대상이 아님 (영수증과 별개 단계)

### 49.6 결과물 엑셀 (`dev/js/admin.js`)
- 영수증 그룹 컬럼 수: 6 → 9 (마스킹 해제 정책에 맞춰 구매일·구매금액·주문번호 모두 노출)
- 영수증 9컬럼 순서: 타입 / 제출일 / 검수일 / 상태 / **주문번호** / **구매일** / **구매금액** / 이미지 / URL
- 단일 캠페인 결과물 export: A:S 19컬럼 → A:V 22컬럼 (인플루언서 7 + 영수증 9 + 결과물 6). 영수증 이미지 임베드 L → O, URL M → P. 결과물 이미지 R → U, URL S → V.
- 다중 캠페인 결과물 export: A:U 21컬럼 → A:X 24컬럼 (캠페인 2 + 인플루언서 7 + 영수증 9 + 결과물 6). 영수증 이미지 N → Q, URL O → R. 결과물 이미지 T → W, URL U → X.
- 영수증용 9컬럼 헬퍼 `renderReceiptCells9(d)` 신규 — 기존 6컬럼 헬퍼 `renderDeliverableCells(d)`를 호출 후 주문번호·구매일·구매금액 3개를 끼워 넣음

### 49.7 정책 변경 (§35 마스킹 해제)
- 2026-04-30 (커밋 0d2b599)에서 관리자 모달에 마스킹했던 `purchase_date`/`purchase_amount`를 다시 노출
- 영수증 이미지에 이미 보이는 값이라 실질 신규 노출은 없음. 「수정 가능 폼에 다시 표시」 수준
- 개인정보 처리방침(PRIVACY) 영향: 「수집하는 개인정보」에 **주문번호** 신규 추가 검토. 운영 배포 전 `/약관확인` 슬래시 명령 권장

### 49.8 영향 파일
- `supabase/migrations/128_receipt_required_fields.sql` — 컬럼·테이블·RPC + 헤더 rollback 블록
- `dev/index.html` — 영수증 폼 input 3종 (monitor 전용 컨테이너)
- `dev/js/application.js` — addDraftImage 검증 + 폼 초기화·disable 분기 확장
- `dev/js/admin.js` — renderReceiptInfoBlock 헬퍼 + 4종 핸들러(enterReceiptEditMode/cancelReceiptEdit/saveReceiptEdit/toggleReceiptHistory) + 두 모달의 receipt 분기 + select 컬럼 보강 + 엑셀 export 4종 함수
- `dev/lib/storage.js` — insertDraftDeliverable 시그니처 확장 + updateReceiptAdmin·fetchReceiptEditHistory 신규
- `dev/lib/i18n/{ja,ko}.js` — 키 8종 추가

### 49.9 운영 적용 순서
1. 운영 SQL Editor → `128_receipt_required_fields.sql` 전체 (BEGIN/COMMIT 트랜잭션 통째로)
2. 검증 SQL (마이그레이션 헤더 주석 끝 안내 SQL 3건): order_number 컬럼 존재 / receipt_edit_history RLS 활성 / update_receipt_admin RPC 존재
3. main 머지 → Vercel 운영 자동 배포
4. 운영서버에서 monitor 캠페인 1건으로 시나리오 검증 (신규 제출 + 관리자 수정 + 변경 이력 1회)

### 49.10 미해결 / 후속
- 기존 receipts 테이블·035 dual-write 트리거는 dead code 상태로 보존. 추후 정리 시 PR 별도
- 사양서 §3은 "receipts 테이블 컬럼 추가" 가정이었으나 실제 활동관리 코드가 deliverables 직접 INSERT 방식이라 deliverables 단일 사용으로 변경 — 사양서 §9 갱신
- PRIVACY 「수집하는 개인정보」에 주문번호 추가 검토 필요
