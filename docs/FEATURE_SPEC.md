...
| ~~admin_memo~~        | ~~text~~    | **migration 122 (2026-05-14) DROP** — `products[i].admin_memo` (jsonb 내부 필드) 로 이전. §48-2 참조 |
...
| quote_sent_url        | text NULL   | 견적서 URL (migration 112, 2026-05-12). http/https CHECK |
| orient_sheet_sent_at  | timestamptz NULL | OT 시트 발송 일시 (migration 112) |
| orient_sheet_sent_url | text NULL   | OT 시트 URL (migration 112). http/https CHECK |
...
- **제품별 내부 메모 (`products[i].admin_memo`, migration 122, 2026-05-14)**: 신청 단위 단일 `admin_memo` 컬럼이 제품별 jsonb 필드로 분리됨. 신청 목록 표의 「내부 메모」 셀이 **모든 제품 행마다** 표시되며 셀 클릭 시 인라인 편집 textarea 진입 → 저장 시 `updateBrandApplication(id, {products: nextProducts}, prevVersion)` (입금여부/가격체크와 동일 패턴). 신규 등록 모달에서 일괄 `admin_memo` 입력 textarea 제거 (등록 후 제품별 메모는 신청 목록에서 인라인 편집)
...
| `orient_sheet_sent_at` | timestamptz NULL | OT 시트 발송 일시 |
| `orient_sheet_sent_url` | text NULL | OT 시트 URL 링크 (http/https CHECK) |
