...
| ADM-006 | 광고주 신청 메뉴 | `storefront` 아이콘 + "광고주 신청" — `/admin#brand-applications` 라우팅. 모든 관리자 접근 가능. §22 참조 |
...
| business_license_path | text        | Storage 경로 `YYYY/MM/{uuid}/license.{ext}` (reviewer 전용, `brand-docs` 비공개 버킷) |
...
│   ├── 상세 모달 (제품 테이블, 사업자등록증 signed URL, final_quote_krw)
...
├── /reviewer (Qoo10 리뷰어 모집 폼 + 사업자등록증 업로드)
...
- **UI 언어**: 한국어 (광고주 대상)
- **choice landing** (`/`): `/reviewer`(JFUN-Q) 또는 `/seeding`(JFUN-N) 선택
- **reviewer 폼**: Qoo10 리뷰어 모집 — 상품 복수 입력(최대 50), 실시간 견적(엔×10 + 2500×인원 + VAT 10%), 사업자등록증 파일 업로드(PDF/JPEG/PNG 10MB) + 세금계산서 이메일
- **seeding 폼**: 나노 인플루언서 시딩 — 상품 정보만, 견적은 수동 협의(estimated_krw=0)

### 22.3 DB (migration 052/053/054)
- `brand_applications` 테이블 (§5.5), `brand_app_daily_counter` 채번 테이블 (§5.6)
- `admins.receive_brand_notify boolean DEFAULT false` (§5.4)
- Storage: `brand-docs` **비공개** 버킷 (10MB, PDF/JPEG/PNG), 경로 `YYYY/MM/{uuid}/license.{ext}`. SELECT는 관리자 signed URL만
- RLS: 신청 INSERT는 anon·authenticated 허용, SELECT/UPDATE/DELETE는 `is_admin()`만. `brand_app_daily_counter`는 SECURITY DEFINER 트리거 전용
- 트리거:
...
### 22.5 관리자 페인 (`/admin#brand-applications`)
- **권한**: `is_admin()` (모든 관리자)
- **리스트**: `admin-pane-list` 공통 구조. 필터(폼타입/상태/기간/브랜드명 검색, multi-filter 스타일 통일), pending(new) 배지, 오래된 순 정렬
- **상세 모달**: 제품 테이블(name/url/price/qty), 사업자등록증 signed URL 다운로드 버튼, `final_quote_krw` 수동 입력, `quote_sent_at` 체크, `admin_memo` 텍스트, 상태 드롭다운, "되돌리기"(new 복귀)
...
- 사업자등록증은 비공개 버킷 + 관리자 signed URL만 (접근 감사 대상)
- PRIVACY §2.2/§6.1에 광고주 수집 조항·보유기간 명시됨 (ffee4ca)
...
### 25.2 사이드바
- 로고 + 접기 토글 (56px 축소)
- 메뉴 섹션: 대시보드 → 캠페인(관리/신청/결과물/광고주 신청) → 회원관리(인플루언서) → 관리자 설정(기준 데이터[super_admin 한정]/관리자 계정)
