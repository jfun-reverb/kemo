...
| ADM-006 | 광고주 신청 메뉴 | `storefront` 아이콘 + **UI 라벨 "브랜드 서베이"**(2026-04-20 리네임) — `/admin#brand-applications` 라우팅. 내부 용어·DB·라우트·함수명은 `광고주 신청`/`brand_applications` 그대로 유지. 모든 관리자 접근 가능. §22 참조 |
...
- **reviewer 폼**: Qoo10 리뷰어 모집 — 상품 복수 입력(최대 50), 실시간 견적(엔×10 + 2500×인원 + VAT 10%), 사업자등록증 파일 업로드(PDF/JPEG/PNG 10MB) + 세금계산서 이메일. **2026-04-21 intro 리디자인**: Qoo10 리뷰 샘플 이미지 + 브랜드 로고 클릭 시 홈 이동
- **seeding 폼**: 나노 인플루언서 시딩 — 상품 정보만, 견적은 수동 협의(estimated_krw=0). **2026-04-21 intro 리디자인**: 시딩 샘플 이미지 2종 + 브랜드 로고 홈 링크
- **제출 경로**: 클라이언트는 `supabase.rpc('submit_brand_application', {payload, license_path})` 호출 (§22.3 migration 056 참조). 직접 `.insert().select()` 금지

### 22.3 DB (migration 052/053/054/056)
