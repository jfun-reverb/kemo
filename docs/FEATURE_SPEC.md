...
### 22.6 상태 전이 — 9단계 파이프라인 (2026-04-22 확장)
```
new → reviewing → schedule_sent → quoted → orient_sheet_sent
                                                   ↓
                                        campaign_registered → paid → done(최종완료)
...
## 27. 관리자 공지사항 시스템 — 2026-04-22 (migration 063)

### 27.1 개요
관리자들 사이의 시스템 변경·긴급 경고·일반 안내를 앱 내에서 공유하는 공지 보드. 이메일/슬랙 의존 없이 관리자 페이지 접속 시 즉시 확인 가능.

### 27.2 DB (§5.7 / §5.8)
- `admin_notices`: category/title/body_html/pin/created_by/*
...
- **미읽음 배지**: 본인 읽음 기록 차집합. 9+면 `9+` 표시
- 아이콘: Material Icon `campaign` (이모지 금지 — 2026-04-22 reviewer 지적으로 push_pin도 Material Icon으로 교체)

### 27.4 리스트 페인
- 필터: 카테고리(4종) + 핀 고정 여부 + 검색(제목·본문 plain text)
- 정렬: 핀 고정 우선 → 최신순
- 각 행: 카테고리 pill / 제목 / 작성자 / 작성일 / 읽음 체크 표시
...
- 대시보드 최근 3건 카드 (`/admin#dashboard`)
- **로그인 시 미읽음 팝업**: 관리자 세션 복원 직후 `admin_notice_reads` 대비 미읽음 건 존재 시 모달로 강제 노출
