...
**조치 계획:** `docs/specs/2026-09-02-audit-remediation-plan.md` (실측 번호 M1~M14는 그쪽 표와 같다)
...
**§1-1 방침 개정 통지 함수** — **확인하는 행위가 곧 전 회원 메일 발송**이다. 호출해 보지 말 것. 코드 근거(호출자 검증 0건·이스케이프 0건)와 배포 확인(`supabase functions list`)으로 이미 확정됐다.
...
- 두 칸의 합이 곧 **「보수를 지급하겠다」는 메일을 받았는데 지급이 없는 사람**의 규모다.
