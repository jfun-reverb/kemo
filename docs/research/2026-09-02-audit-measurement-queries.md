...
## ★ 실측 결과 (2026-09-02 운영에서 실행)

**M1·M13 은 빠져 있다** — 묶음 A 의 선행이라 **착수 직전에** 개발 세션이 돌린다(「읽는 순간의 상태」가 판정 근거라 미리 읽으면 낡는다).

| 실측 | 결과 | 판정 |
|---|---|---|
| **M2** | 확정 탈퇴 2명(2026-08-30·08-31) 중 **응모 이력이 있는 사람은 1명**, 그 사람의 **응모 2건**에 이메일·이름·SNS계정·주소 전부 잔존 | ✅ **조치 완료**(과거분 정리 + 마이그레이션 396, 운영 적용) |
| **M4** | 무보수 시딩·방문형 승인 응모 **650건** / 금액 없는 리뷰어형 **0건** | 🔴 **650건 발현** |
| **M14** | 하루 최대 검수 **1,305건** · 1,000 넘은 날 **1일** (응모는 최대 797) | 🔴 **하루 발현** |
| M3 | 0 | 잠복 (버튼 두 번이면 도달) |
| M5 | 0 | **대상 없음** |
| M6 | **151** (1,200 미만) | **사거리 밖** |
| M7 | 0 | 잠복 |
| M8 | 0 | 잠복 |
| M9 | 채널 2개 이상 active 캠페인 **0건** | **대상 없음** |
| M10 | 캠페인당 결과물 최대 **74**(×2 = 148 경로) · 감사용 결과물 **0** | 낮음 |
| M11 | `gifting` · `monitor` · `visit` **세 값뿐** | ✅ **CHECK 를 지금 걸 수 있다** |
| M12 | 0 | **대상 없음** |
| M15 | `sent` 1 · `failed` 0 | 잠복 |

### 실측이 바꾼 것

🔴 **M2 — 조사 문서가 틀렸다.** 「확정된 탈퇴가 아직 없다(`CLAUDE.md` 기준)」고 적었으나 **2건이 있고 개인정보가 그대로 남아 있다.** 문서 서술을 믿고 쓴 것이고 실측이 뒤집었다. **C-1 의 우선순위가 A 다음으로 올라간다** — 지금 방침 위반이 진행 중이다.

✅ **M9 가 0이라 D-1(「또는」의 뜻을 다시 정하는 기획 결정)이 급하지 않다.** 채널 2개 이상 캠페인이 없다. 그런 캠페인을 만드는 순간 발생하는 잠복이다.

✅ **M11 이 세 값뿐이라 B-6(CHECK 제약)을 지금 걸면 위반 행 0으로 통과한다.** 값이 늘면 못 건다.

✅ **절반이 「대상 없음」 또는 「잠복」이다**(M3·M5·M7·M8·M9·M12·M15). 직전 조사와 같은 비율이고, **조사 문서만 믿고 착수했으면 없는 문제를 고칠 뻔했다.**
...
**무엇을 볼 것**
- 🔴 **2026-09-02 실행 결과: 응모 2건이다.** ⚠️ **「2명」이 아니라 「1명의 응모 2건」** — 확정 탈퇴는 2명이지만 응모 이력이 있는 사람은 1명이다(`count(distinct user_id)` = 1). 처음 보고에서 이 구분을 안 했다.
- ✅ **조치 완료** — 과거분 손으로 정리 후 재확인 0건(양성 대조 전체 응모 4,579), 마이그레이션 **396**(파기 함수에 응모 표 절 추가) 개발·운영 양쪽 적용.
- ⚠️ **이 칸에 「확정된 탈퇴가 아직 없으니 0이 정상」이라 적혀 있었다.** `CLAUDE.md` 서술을 믿은 것이고 **틀렸다.** 문서가 「없다」고 해서 없는 것이 아니다.
- **조치 뒤 다시 돌려 0이 되는지 확인한다.** 그것이 C-1 의 완료 기준이다.
...
## ⚠️ 예약 실행·메일 검증의 함정 셋 (2026-09-03 실측에서 확인)

메일이 실제로 나갔는지 확인할 때 **셋 다 「안 나갔다」로 오독하게 만든다.**

1. 🔴 **다이제스트 표에 오늘 날짜 행이 없는 것이 정상이다** — 그 함수들은 `digest_date` 에 **어제 날짜**를 쓴다(`computeWindow()` 의 `yesterdayKstMs`). **어제 날짜 행이 오늘 실행분**이다. 코드를 안 열면 「안 돌았다」로 보고하게 된다.
2. 🔴 **`cron.job_run_details` 의 `succeeded` 는 증거가 약하다** — 그건 「HTTP 요청을 보냈다」까지고, **함수가 403 으로 막혀도 `succeeded`** 다. 기록표와 **함수 로그**를 봐야 갈린다.
3. ★ **결정적 증거는 함수 로그다** — 예: `[notify-deliverable-decision] caller check passed { isServiceRole: false }`. 이 한 줄이 ①차단이 실제로 돌고 ②서비스 키가 통과했음을 동시에 보여준다.

⚠️ **「예약 실행 시각」과 「조회 시각」을 함께 적어라** — 「안 나갔다」와 「아직 안 돌았다」가 같은 모양으로 보인다.

## ⚠️ 검증이 **성립하지 않는** 경우 — 게이트가 앞에서 걸린다 (2026-09-03)

**A-4(익명 창구의 원소 스키마 검사)를 기능으로 시험할 수 없다.** `submissions_open=false` 라 **0단계 게이트가 먼저** 걸려 원소 검사에 **도달하지 못한다.**

🔴 **「잘못된 값을 넣었더니 거부됐다」가 나올 수가 없고, 나온다면 그건 다른 이유로 거부된 것이다.** 개발·운영 **양쪽 다** 같은 상태다.

**그래서 대신 확인한 것**(개발 세션, 2026-09-03) — `anon` 실행 권한 **true**(창구 안 죽음) · 검사가 `prosrc` 에 들어감 · 저장된 20행 중 걸리는 것 **0건** · 검사식 9개 경우가 의도대로(정상·키없음·`null`·관리자 추가키 **통과** / 객체아님·금액문자열·수량문자열·배열·600자 **거부**).

⚠️ **이 유형은 마이그레이션 272(응모 마감 트리거)와 같다** — 그때도 SQL 편집기가 서비스 키로 돌아 트리거를 재현하지 못했다. **「앞에 다른 관문이 있는가」를 먼저 보고 검증 방법을 고른다.**

## ⚠️ `toLocaleString` — 「숫자니까 안전」이 아니다 (2026-09-03)

🔴 **`String.prototype.toLocaleString` 이 존재해서 문자열도 숫자 서식을 그냥 통과한다.** 익명 창구가 `jsonb` 로 받은 값에 문자열이 섞이면 **HTML 표에 그대로 들어간다** — A-4 가 그 경로였다(`notify-brand-application`, `Number()` 로 먼저 거르도록 고침 + 주석).

**전수(2026-09-03, Edge Function 5개 10건)** — `Number()`·`Math.round()` 로 거르지 않고 **직접 부르는 자리 3곳**:

| 자리 | 값의 출처 | 판정 |
|---|---|---|
| `notify-admin-daily-digest:242` | `influencers` 팔로워 칸 | **숫자 칸** — 위험 낮음 |
| `notify-influencer-daily-digest:984`·`:987` | `campaigns.product_price`·`reward` | **숫자 칸** — 위험 낮음 |

⚠️ **당장 위험은 아니지만 「숫자 칸이니까 안전」이라는 전제에 기대고 있다.** `notify-brand-application` 이 위험했던 이유는 그 값이 **익명 입력의 `jsonb`** 였기 때문이다 — **정규 칸과 자유 형식을 갈라서 보라.**
