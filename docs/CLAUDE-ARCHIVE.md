...
## 12. 회원 탈퇴 배포 이력 (2026-08-19~21)

> 현재 동작·지금 유효한 제약은 `CLAUDE.md` 「회원 탈퇴」 절. 여기는 **언제 무엇이 나갔나**만 보관한다.

- **2026-08-20** — 마이그레이션 345~361·363~366 운영 적용(**362 제외**). 전수 조회로 확인. Edge Function 2종(`notify-withdrawal-scheduled`·`purge-withdrawal-media`) 운영 배포
- **2026-08-20** — 6개월 파기(작업 12) 운영 적용(364·365 + `purge-withdrawal-media`). 개발서버에서 파일이 실제로 사라지는 것까지 확인(다섯 갈래 실측)
- **2026-08-20** — 계정 차단(작업 8) 358·359 운영 적용
- **2026-08-21** — 화면 운영 배포(병합 요청 #1172, 커밋 `56e4540d`). 회원 화면(작업 9 — 다섯 모습·2단 확인·접수·취소)과 관리자 화면. **함수를 먼저 넣은 것**은 정산 3단계의 「버튼은 열렸는데 함수가 없던 15시간」을 반복하지 않기 위해서다(그 교훈 자체는 `CLAUDE.md` 정산 절에 일반화된 형태로 있다)
- **2026-08-21** — 367(자주 묻는 질문 탈퇴 5문항)을 **화면 배포 뒤에** 적용. 먼저 넣으면 화면이 없는데 「마이페이지에서 탈퇴하세요」라고 안내하는 거짓 안내가 된다
- **약관을 기다리지 않은 이유** — 현행 이용약관 제6조 1항(「마이페이지에서 언제든지 탈퇴 신청」)과 개인정보처리방침 §6(「탈퇴일로부터 5일 경과 후 파기」)이 **이미** 이 흐름을 적고 있어, 화면이 없던 때가 오히려 문서가 거짓인 상태였다. 개정(작업 18)이 필요한 것은 **6개월 저장소 파기·6개월 재가입 차단** 같은 새 조항이다
