...
- 결과물 등록 시 **#PR, #広告, #プロモーション 중 하나 필수 표기**(캠페인 주의사항·자주 묻는 질문 자동응답이 안내. 「#アンバサダー」는 업계 기준상 광고 표시로 인정되지 않아 2026-09-10 에 예시에서 뺐다)
- 검수 단계에서는 **관리자가 결과물 검수(승인/반려)에서 눈으로 확인하고, 없으면 반려 사유로 돌려보낸다. 자동 검사는 없다** — 게시물은 주소(URL)만 받고 본문을 읽을 수단이 없다. ⚠️ 이 줄은 오래 「자동화 검사 + 수동 확인 이중」이라 적혀 있었으나 사실이 아니었다(2026-09-10 정정 — 근거·현황 `docs/research/2026-09-10-japan-stealth-marketing-regulation.md` §5-①)
- 미표기 시 반려 → 인플루언서가 수정 후 재제출하는 흐름(결과물 관리 페인)
- 구현 지점: 결과물 검수 페인(`dev/js/admin-deliverables.js`) 반려 사유 템플릿 · 캠페인 주의사항 번들(`caution_sets`)
- 규제 전체 조사·사용자 결정(2026-09-10): `docs/research/2026-09-10-japan-stealth-marketing-regulation.md`(§7 결정 · §8 후속 후보)

### 3-2. 정산 (실제 운영 기준 — 2026-08-05 정정)

> ⚠️ **아래는 실제로 가동 중인 규칙이다.** 이 절에 원래 적혀 있던 「원천징수 22%·조세조약 세율·원화 기준 칸(`base_amount_krw`·`tax_rate`·`exchange_rate_krw_jpy` 등)」은 **초기 구상이며 구현된 적이 없다** — 그 칸들은 데이터베이스에 존재하지 않고, 약관 제13조 2항(「수수료 및 관련 세금은 회사가 부담하며 회원은 리워드 전액을 수령」)과도 어긋난다. 2026-06-22 사양서가 이미 지적했던 것을 이번에 반영했다.

- **통화: 엔화(¥) 단일.** 원화 환산·환율 기록 없음
- **원천징수 없음 · 전액 지급**(약관 제13조 2항)
- **송금 수단: PayPal 수동 송금.** 자동 송금 기능 없음 — 관리자가 외부에서 보낸 뒤 시스템에 「송금완료」로 기록
- **금액 산정은 모집 형식별로 다르다**
  - 리뷰어형(monitor, 가구매 포함) = **영수증 실결제액**, 단 캠페인 제품 가격을 **상한**으로 자름(2026-08-05, 마이그레이션 299·300)
  - 시딩(gifting)·방문형(visit) = 캠페인 현금 리워드(`campaigns.reward`)
  - ⚠️ 리뷰어형은 `campaigns.reward` 를 **쓰지 않는다** — 화면·메일에 현금 리워드를 덧붙이면 지급되지 않는 금액을 약속하게 된다
- **금액은 생성 시점 스냅샷**으로 고정(캠페인을 나중에 수정해도 이미 만들어진 정산액은 안 바뀐다)
- **현재 인플루언서에게는 잠겨 있다**(`settlement_settings.influencer_visible=false`) — 관리자만 기록·조회
- 상세는 `CLAUDE.md` 「정산」 절과 사양서 `docs/specs/2026-08-05-settlement-receipt-amount-switch.md`

**미구현(구상 단계)**: 원천징수·조세조약 처리, 원화↔엔화 환산, PayPal API 자동 송금. 도입하려면 약관 제13조 2항 개정이 선행돼야 한다.
