...
⚠️ **이 표시는 그전까지 어디에도 문서화돼 있지 않았다**(`CLAUDE.md`·규칙 파일 전부 0건). 그래서 2026-09-02 에 **두 세션이 각자** 이 도장을 보고 「개발이 뒤처졌나?」부터 의심했다. 앞뒤를 알고 싶으면 도장이 아니라 `git log origin/main..origin/dev` 를 본다.
