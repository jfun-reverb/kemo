...
## 세션 현황 확인과 세션 이름 (2026-08-20 추가)

여러 세션을 동시에 돌리면 **어느 세션이 무슨 일감을 어디까지 했고, 지금 내 답을 기다리는지** 알 수가 없다. 터미널을 하나씩 열어보는 수밖에 없었다. 아래 두 가지로 해결한다.
...
살아 있는 세션마다 **상태(작업 중 / 대기 / 내 답 기다림) · 마지막 활동 시각 · 폴더 · 가지 · 커밋 안 된 파일 수**를 보여준다. 앞의 넷은 Claude Code 가 스스로 기록하는 값이라 **사람이 갱신을 잊어도 안 낡는다.**

- 6시간 넘게 조용한 세션은 「열려만 있는 세션」으로 따로 묶는다. 안 그러면 터미널만 떠 있는 세션까지 「지금 도는 중」에 섞여 목록 전체가 안 믿기게 된다.
- 같은 폴더를 **실제로 움직이는** 세션 둘 이상이 쓰고 있으면 경고한다. 방치된 세션은 이 경고에서 뺀다(과장된 경고는 곧 무시된다).

### 세션 이름 = 일감 + 진행률

세션 이름은 상태 표시줄과 `/현황` 양쪽에 그대로 뜬다. **여기에 일감과 진행률을 실으면 별도 장부가 필요 없다.**

```bash
node "$CLAUDE_PROJECT_DIR/.claude/scripts/session-name.js" "회원탈퇴 화면 2/5"
```

- 형식은 `일감이름 완료/전체`. **일감을 시작할 때와 단계가 넘어갈 때** 다시 부른다.
- 이름을 바꾼 시각이 함께 기록돼, `/현황` 이 「일감표시 갱신 N분 전」으로 보여준다 — **낡은 표시가 낡아 보이게** 하려는 것이다. 자동 칸과 달리 이 칸은 사람이 안 쓰면 낡기 때문.
- ⚠️ 이름을 안 붙이면 `reverb-jp-5b` 같은 자동 꼬리표가 남는데, **그 꼬리표는 폴더도 일감도 알려주지 않는다.** 여러 세션을 돌릴 때는 붙이는 것이 사실상 의무다.
- ⚠️ 이름 바꾸기는 **조상 프로세스를 거슬러 올라가 자기 세션을 찾는다.** 「같은 폴더에서 가장 최근에 움직인 세션」 같은 추측을 쓰면 **남의 세션 이름을 바꾼다** — 같은 폴더에 세션이 여럿인 상황이 실제로 있기 때문.

### ⚠️ 남은 위험 — 이름을 붙여도 폴더 공유는 안 풀린다

`/현황` 은 폴더 공유를 **알려줄 뿐 막지는 않는다.** 근본 해결은 `/새세션` 으로 작업 폴더를 나누는 것이다.

실제로 2026-08-20 에 메인 폴더 하나를 세션 2개가 동시에 쓰면서 이런 일이 있었다. 한쪽이 「커밋 안 된 파일 1개」로 알고 있는 사이 다른 쪽이 20개로 늘려 놨고, **아무 경고도 뜨지 않았다.**

★ 그리고 더 나쁜 형태가 같이 나왔다 — **남이 지금 만들고 있는 파일을 「과거 세션이 남긴 유물」로 오인하는 것.** 세션을 새로 시작하면 미커밋 변경이 보이는데, 그게 *이전에* 누가 두고 간 것인지 *지금* 옆에서 만들고 있는 것인지 **구분할 방법이 없다.** 그날 고문 세션은 미커밋 파일 하나를 과거 유물로 단정하고, 완성도까지 검증한 뒤 「지금 커밋해 잠글까요」라고 사용자에게 제안했다. 실제로는 다른 세션이 리뷰를 기다리던 작업이었고, 그대로 커밋했으면 **남의 진행 중 작업을 가로챘을 것이다.**

그래서 **공유 폴더에서 발견한 미커밋 변경은 내 것이 아니면 건드리지 않는다.** 커밋할 때는 `git add <내가 만든 파일>` 로 **골라서** 올리고, `git add -A`·`git commit -a` 는 쓰지 않는다. 출처가 헷갈리면 `/현황` 으로 그 폴더를 쓰는 다른 세션을 확인하고, 그 세션에게 물어본다(`SendMessage`).
...
- `.claude/commands/현황.md` — `/현황` 단축키 (세션 현황 한 장으로 보기)
