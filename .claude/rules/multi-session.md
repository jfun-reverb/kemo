...
`/현황` 은 폴더 공유를 **알려줄 뿐 막지는 않는다.** 근본 해결은 `/새세션` 으로 작업 폴더를 나누는 것이다. 실제로 2026-08-20 에 메인 폴더 하나를 세션 2개가 동시에 쓰면서, 한쪽이 「커밋 안 된 파일 1개」로 알고 있는 사이 다른 쪽이 15개로 늘려 놓은 상태가 확인됐다. 이때 아무 경고도 뜨지 않았다.
