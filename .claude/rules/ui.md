...
- 햄버거 메뉴 패널(#navPanel, 렌더: `renderNavMenu()` in dev/js/notifications.js)
  ...
  - 로그인 시: 계정 카드(아바타·이름·핸들·이메일) / 홈 / 캠페인 / 「마이페이지」 접기·펼치기 아코디언(기본 펼침, `toggleMypageAccordion`, 서브 7종 응모이력·기본정보·SNS·배송지·PayPal·비밀번호·메일수신설정 + 미입력 항목 「未登録」 배지) / 메시지(배지) / 알림(배지) / 로그아웃 / 회원탈퇴(작은 링크). 마이페이지 랜딩 화면은 제거되어 햄버거가 목차 역할 (2026-05-21)
