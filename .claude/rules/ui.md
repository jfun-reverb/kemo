...
- 햄버거 메뉴 패널(#navPanel, 렌더: `renderNavMenu()` in dev/js/notifications.js)
  ...
  - 로그인 시: 계정 카드(이름·핸들·이메일 + 우측 알림 벨 아이콘·미읽음 배지, 아바타 없음) / 홈 / 캠페인 / 「마이페이지」 접기·펼치기 아코디언(기본 펼침, `toggleMypageAccordion`, 서브 7종 응모이력·기본정보·SNS·배송지·PayPal·비밀번호·메일수신설정 각 `min-height:48px` + 미입력 항목 「未登録」 배지) / 메시지(배지) / 로그아웃 / 회원탈퇴(작은 링크, `margin-top` 간격). 알림은 별도 항목이 아니라 계정 카드 우측 벨로 통합. 폼 화면에는 백버튼 없음(햄버거/브라우저 뒤로가기). 마이페이지 랜딩 화면 제거로 햄버거가 목차 역할 (2026-05-22)
