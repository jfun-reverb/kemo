...
- `deliverable_events` — 결과물 상태 변경 이력. `action`(submit/resubmit/approve/reject/revert/**admin_proxy_submit**/**admin_proxy_revoke**), `from_status`, `to_status`. 트리거/RPC 만 INSERT. `admin_proxy_submit`/`admin_proxy_revoke`(마이그레이션 160)는 관리자 대리 등록·회수 RPC 가 직접 INSERT. **주의**: `deliverable_events.deliverable_id`에 `ON DELETE CASCADE` 가 걸려 있어 `admin_revoke_proxy_deliverable` 가 결과물 행을 DELETE 할 때 `admin_proxy_revoke` audit 도 함께 삭제됨(영구 감사 필요 시 별도 audit 테이블 분리 권장)
