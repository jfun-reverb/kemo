...
- **명단 관리 페인**(`/admin#outbound`, `dev/js/admin-outbound.js`): 영업팀이 직접 컨택하는 아웃바운드 시딩·타이업 인플루언서 명단(globalreverb 미가입·별도 자산). 기존 `influencers`(auth 계정 1:1·정산/마스킹 결합)와 **분리한 신규 테이블**. 목록(대표이미지·한글명·계정·계열·세분·등급·주채널 팔로워·가용상태 배지·소속사, IntersectionObserver lazy-load) + 필터(계열/세분 다중·등급·가용상태·채널·검색) + 등록/편집 모달(채널4·가격5[원화 ₩·내부전용]·대표게시물 링크 최대5·대표이미지 업로드) + 삭제(네이티브 confirm — 편집 모달 열린 채라 z-index 회피). 세분→계열 자동채움(`OB_CATEGORY_SERIES` shared.js). 권한 `outbound.view`(campaign_admin=write/campaign_manager=hidden). 추천 알고리즘=2단계·성과 축적=3단계(미착수). 사양서 `docs/specs/2026-07-08-influencer-recommendation.md` + HANDOFF `2026-07-09-influencer-recommendation-stage1-handoff.md`
