...
- **관리자 계정**: 3단계 권한(super_admin > campaign_admin > campaign_manager)
  - **추가**(2026-07-20 개편): super_admin 이 이메일+이름+역할 입력 → `invite_admin()` RPC 로 계정 생성 → Edge Function `notify-admin-invite` 가 **서버에서** 비밀번호 설정 링크 발급(`auth.admin.generateLink`) + 관리자 전용 **한국어** 메일 Brevo 발송 → 받은 사람이 자립형 페이지 `/admin-setpw.html` 에서 직접 설정. 기존 인플루언서 계정도 같은 이메일로 호출 시 자동 관리자 승격. 재발송은 목록 「비밀번호」 모달 → 같은 경로(`mode=reset`) 재사용. `storage.js` `sendAdminInviteMail(email, mode)`
    ...
    - **랜딩이 `/admin/` 밖 자립형 페이지인 이유**: 관리자 앱은 세션 없으면 인플루언서 앱으로 리다이렉트(`dev/admin/app.js`)라 로그인 전 화면을 둘 수 없음. `dev/admin-setpw.html` → `build.sh` 가 루트로 복사. `persistSession:false` 로 임시 세션이 두 앱과 저장소를 공유하지 않게 격리 + 완료 후 `signOut()`. 관리자 전용 로그인·비밀번호 찾기 모드는 같은 파일에 후속 추가 예정. 사양서 `docs/specs/2026-07-20-admin-invite-mail-and-setpw.md`
