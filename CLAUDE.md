...
- `campaigns_yearly_counter` — 연도별(JST) 캠페인 번호 채번 카운터. SECURITY DEFINER 트리거 `generate_campaign_no()` 전용 (직접 UPDATE 금지). 088~090 도입 후 `campaigns.brand_id IS NULL` 폴백 경로에서만 사용
- **계층 채번 시스템(088~090, 2026-05-04)** — `B{brand_seq}-A{app_seq}-C{camp_seq}` 형식. brands → brand_applications → campaigns 부모-자식 계층을 번호에 직접 표현. 외부 캠페인(부모 신청 없음)은 `B{brand_seq}-C{ext_seq}` (A 세그먼트 생략). 자릿수: brand 9999/신청 999/캠 999. 기존 `JFUN-R-260504-NNN`/`CAMP-2026-NNNN`은 `legacy_no` 컬럼에 보존, `numbering_legacy_map` 테이블에서 양방향 매핑. 신규 컬럼: `brands.brand_seq`(UNIQUE), `campaigns.brand_id`/`source_application_id` FK ON DELETE SET NULL, `campaigns.legacy_no`, `brand_applications.legacy_no`. 신규 카운터: `brand_seq_counter`(싱글톤), `brand_application_counter`, `application_campaign_counter`, `brand_external_campaign_counter` — 모두 SECURITY DEFINER 트리거 전용 (직접 UPDATE 금지). 트리거 `generate_brand_application_no`/`generate_campaign_no` 재설계, advisory lock(hashtext(brand_id::text)::bigint)으로 동시 INSERT 직렬화. 캠페인 등록 폼은 brands 드롭다운 + 신청 cascade + 신규 brand 인라인 모달 패턴
...
- `campaign_caution_history` (migration 077, 2026-04-30, Phase 2) — 캠페인 주의사항/참여방법 변경 audit. `campaign_id` FK CASCADE + `changed_by`(auth.uid)·`changed_by_name`(스냅샷)·`changed_at` + caution(set_id/items) prev·next + participation(set_id/steps) prev·next + `app_count_at_change`(변경 시점 pending+approved 신청자 수) + `bypass_warning_ack`(showSensitiveChangeConfirm 모달 통과 여부). RLS SELECT super_admin 전용, INSERT/UPDATE/DELETE 정책 미정의 — `record_caution_history()` SECURITY DEFINER RPC 경유 INSERT 만 허용(campaign_admin 이상 가드, `SET search_path = ''`). 인덱스: (campaign_id, changed_at DESC). 캠페인 더보기 메뉴 「변경 이력」(super_admin 한정) → 타임라인 모달, 인플루언서 응모이력 「現在の文言と比較」 토글에서 v2 스냅샷↔현재 비교
- `brand_applications` — 광고주(브랜드) 신청 폼 제출 데이터 (form_type: reviewer|seeding, brand_name, contact_name, phone, email, billing_email, products jsonb, total_jpy/total_qty, estimated_krw, final_quote_krw, quote_sent_at, status **10단계 파이프라인**, admin_memo, **`request_note text NULL` (migration 068) — 신청자 자유 입력 기타/요청사항**, version 낙관적 락). 상태: `new→reviewing→quoted→paid→kakao_room_created→orient_sheet_sent→schedule_sent→campaign_registered→done`(최종완료) / `rejected` (migration 064=8단계 schedule_sent+campaign_registered, 065=9단계 orient_sheet_sent, 2026-04-30 d860685=10단계 `kakao_room_created` 추가, 기존 데이터 보존). 서버 트리거가 `JFUN-{R|S}-YYMMDD-NNN` 채번(2026-05-04 migration 078: reviewer=R, seeding=S, 연도 2자리. v1 `JFUN-{Q|N}-YYYYMMDD-NNN` 데이터는 일괄 변환됨) + products 재계산. 관리자만 SELECT/UPDATE/DELETE. **익명 INSERT는 `submit_brand_application()` RPC(SECURITY DEFINER, BYPASSRLS) 경유 필수** — 직접 INSERT는 42501 RLS 오류 (migration 056, 068에서 `p_request_note` 파라미터 추가). 사업자등록증(`business_license_path`)·`brand-docs` 버킷은 migration 057에서 제거됨
