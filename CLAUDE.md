...
- **동적 권한 관리 (PR1 인프라, 마이그레이션 207~210 — 화면·실차단은 후속 PR2·PR3)**: `role_permissions` — 등급×기능별 접근수준. `(role, feature_key)` PK, `access_level CHECK(write|read|hidden)`, `role CHECK(campaign_admin|campaign_manager)`(⚠️super_admin 행 없음 = 잠금 방지). RLS SELECT `is_admin()` / CUD `is_super_admin()`. 시드 72행 = **현행 권한 그대로**(도입 즉시 동작 변화 0). / `role_permission_history` — 권한 변경 이력 골격(PR2가 채움). RLS SELECT `is_super_admin()`. / `has_permission(p_feature, p_min default 'read')` RPC (SECURITY DEFINER + `search_path=''`) — super_admin 무조건 통과·미등록 feature_key 는 안전측 거부(false)·서열 write>read>hidden. **아직 어떤 화면·서버 가드도 호출 안 함(순수 인프라)**. 권한 카탈로그 = 코드 상수 `ADMIN_PERMISSION_CATALOG`(shared.js, 화면 19 + 주요기능 17 = 36개). 사양서 `docs/specs/2026-06-15-admin-permission-management.md`
