...
- **동적 권한 관리 (PR1~PR3 운영 완료 · 마이그레이션 207~215 · 「기본값 복원」 포함)**: super_admin 이 `/admin#permissions` 「권한 관리」 화면에서 등급(campaign_admin/campaign_manager)×기능별 접근수준(쓰기/읽기/숨김)을 설정 → 사이드바 메뉴 노출이 바뀌고, **민감정보(전화·주소·PayPal 등)는 서버 가림막 뷰로 실제 차단**(PR3, 마이그212·213). 화면 메뉴 숨김은 클라 표시 제어(fail-open), 민감정보·`has_permission` 서버 판정은 fail-closed(미등록 거부). 「기본값 복원」 버튼으로 baseline(default_level) 되돌리기 가능(복원해도 민감정보 차단 유지).
  ...
  - 권한 카탈로그 = 코드 상수 `ADMIN_PERMISSION_CATALOG`(shared.js, 화면 20[menu.*] + 주요기능 19 — 정산 `settlement.view`/`settlement.pay` 2종 포함[마이그레이션 220 시드, campaign_admin=write·campaign_manager=hidden]). 클라 헬퍼 `permLevel/canWrite/canRead/isHidden`(super=write·미로드=fail-open), 부팅 시 `fetchRolePermissions` 로드, `applyLookupMenuVisibility` 가 menu.* 순회로 사이드바 숨김. 설정 화면 `dev/js/admin-permissions.js`(super 전용, 그리드·경고배너·denylist 행 잠금·super 열 고정). `switchAdminPane` 진입 가드(permissions=super 전용·hidden 페인 대시보드 리다이렉트). 사양서 `docs/specs/2026-06-15-admin-permission-management.md`
...
### 정산 (인플루언서 리워드, PR1 — 마이그레이션 217~220, 화면은 PR2·3 예정)
...
- `storage.js`: `fetchSettlements(opts)`(관리자·1000행 페이지네이션)·`fetchMySettlements()`(인플 본인)·`backfillSettlements()`·`markSettlementPaid()`(PR2 RPC 시그니처 선정의)
