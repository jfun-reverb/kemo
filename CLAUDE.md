...
- `companies` (migration 118, 2026-05-13) — 회사 마스터 (1개 회사 = N개 brands, 4단 계층: 회사 > 브랜드 > 신청 > 캠페인). name_ko(NOT NULL)·name_ja·name_en + `name_normalized text UNIQUE NOT NULL` (자동 `lower(trim(name_ko))` 트리거 `trg_companies_name_normalized`) + business_no(사업자등록번호, NULL 허용)·address·homepage_url·contact_* 3종·billing_email·billing_address·memo + `status text NOT NULL DEFAULT 'active' CHECK(active|archived)` + `total_brands integer NOT NULL DEFAULT 0` (brands.company_id 변경 시 `trg_brands_company_total_brands` 트리거 자동 재계산) + updated_at 자동 갱신. 행 단위 보안 정책: SELECT는 `is_admin()`(모든 관리자), INSERT/UPDATE/DELETE는 `is_campaign_admin()` 이상(분리 정책 3종). 트리거 함수 3종 모두 `SECURITY DEFINER + SET search_path=''`. `brands.company_id uuid FK ON DELETE SET NULL` 외래 키 추가. 백필은 migration 119(예정)에서 이름 유사도 기반(2개 이상 동명만 자동 회사 생성, 나머지 NULL 유지 — 운영자 수동 정리). 「운영 현황」 페인 회사 필터 기반 (사양서 docs/specs/2026-05-13-brand-ops-redesign.md)
