...
- 작성 폼 `dev/sales/orient.html`(+ 빌드 복사본 `sales/orient.html`) — sales 정적 배포(storage.js 미사용·RPC 인라인 호출). **카드형**(공통 브랜드 1회 + 카드 N개 아코디언, 카드마다 형식 가구매/리뷰어/시딩 선택 → §15-12대로 입력 분기): 가구매·리뷰어=**판매처 Qoo10 고정**(드롭다운 제거, 2026-06-25)·판매 URL(ttp:// 등 자동 보정 + Qoo10 아니면 경고만)·**가격은 상시가만**(세일가·대형할인 제거)·리뷰어=리뷰가이드 / 시딩=등급(나노 IG한정·미들메가 다채널)·채널별 가이드·소구·해시태그·촬영가이드(미들메가). 제출 시 항상 읽기전용 미리보기 1회. 자동저장·낙관적 락·만료/소비 진입 분기.
