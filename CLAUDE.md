...
- **오리엔시트 발급·조회 페인**(`/admin#orient-sheets`, `dev/js/admin-orient.js`, PR3+PR5): 브랜드에게 작성 토큰 링크를 발급(`create_orient_sheet`, `is_admin()` 가드 — campaign_manager 포함 전체 관리자)하고 제출 내용을 확인. 목록(브랜드·타입·상태·발급일·작성기한·링크복사) + 발급 모달(브랜드 드롭다운·선택적 신청 연결[타입 자동 승계]·타입 선택[비우면 브랜드가 0단계에서 선택]·서베이 연결 시 제품 선택 행) + 상세 모달(data jsonb를 §11 스키마대로 한국어 렌더, 타입별 분기, 카테고리 code→라벨 변환, 이미지 링크 http/https만 허용). 링크 도메인 hostname 분기(운영 sales.globalreverb.com / 개발 sales-dev). 모달은 동적 생성. 만료는 클라 판정(조회 함수가 status 미전환).
  ...
  - brand-applications 신청 목록 「셀프 오리엔시트」 열(연결 건수·최근 상태, `renderSelfOrientCell`, 만료 클라 판정 — 기존 「오리엔트 전달」 외부 URL 열과 별개) — PR5 단계⑤ 구현 완료. 목록 로드 시 `fetchOrientSheets` 1회 조회 후 `_orientByApp` 그룹(N+1 회피). 사양서 `docs/specs/2026-06-18-brand-self-orient-sheet.md` §7·§14
