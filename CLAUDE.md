...
### 브랜드 셀프 오리엔시트 (마이그레이션 186~188)
> 브랜드가 로그인 없이 토큰 링크로 캠페인 콘텐츠(오리엔시트)를 한국어로 작성·제출 → 관리자가 자동 채움으로 캠페인 발행. 사양서 `docs/specs/2026-06-18-brand-self-orient-sheet.md`. PR1·PR2 dev 배포(운영 보류).
- `orient_sheets` — `brand_id`(NOT NULL, ON DELETE RESTRICT) · `application_id`(NULL, SET NULL — 신청 연결 선택) · `token uuid UNIQUE`(작성 링크 식별자) · `form_type`(reviewer|seeding, NULL 허용 — NULL이면 작성 폼 0단계에서 브랜드가 선택) · `data jsonb`(작성 내용, 스키마는 사양서 §11) · `status CHECK(draft|submitted|consumed|expired)` · `campaign_id`(발행 캠페인 역참조, SET NULL) · `token_expires_at`(발급+30일) · `version`(낙관적 락) · `created_by`. **익명 직접 접근 0**(RLS — 토큰 함수로만), `updated_at` 자동 갱신 트리거. ⚠️ `brand_id` RESTRICT → PR3 `delete_brand` RPC에 `orient_sheets` 카운트 체크 추가 필요
- 익명 함수 4종 (GRANT anon, SECURITY DEFINER + `search_path=''`, 토큰 미매칭/만료/consumed 차단, 140·187 패턴): `get_orient_sheet(token)`(조회 + 연결 신청 모집 희망값 `initial_values` 승계, 읽기 전용) · `save_orient_draft(token, data, version)`(임시저장·낙관적 락·jsonb 100KB·status 미변경) · `submit_orient_sheet(token, data, version)`(draft/submitted→submitted, 재제출 허용) · `set_orient_form_type(token, form_type)`(타입 확정 — **draft면 자유 변경, submitted 후 잠금**[다른 값=type_locked], 마이그레이션 188 신규+189 잠금완화)
- 작성 폼 `dev/sales/orient.html`(+ 빌드 복사본 `sales/orient.html`) — sales 정적 배포(storage.js 미사용·RPC 인라인 호출). 4단계 위저드(모집·브랜드/제품/콘텐츠/이미지·안내) + 타입별 분기(리뷰어=구매기간·가격·판매URL / 시딩=제공·배송) + `form_type` 미지정 시 0단계 타입 선택 화면. 자동저장·낙관적 락·만료/소비 진입 분기. 관리자 발급(PR3)·자동 채움 발행(PR4)은 미착수
