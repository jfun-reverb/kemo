...
- **주의사항 동의**(2026-04-22 migration 067): 캠페인에 `caution_lookup_codes`(lookup `kind='caution'` 코드 배열) 또는 `caution_custom_html`(Quill 리치)가 있으면 신청 모달 상단에 빨간 박스로 표시 + 하단에 "全ての注意事項を確認しました" 단일 체크박스. 미체크 시 신청 차단(`apply.cautionRequired`). 동의 시 `applications.caution_agreed_at`(시각) + `caution_snapshot`(jsonb: lookup_codes/labels ko·ja, custom_html, agreed_lang, snapshot_at) 저장 — 광고주가 나중에 주의사항을 수정해도 신청 당시 내용이 보존됨. 응모이력 항목에 동의 시각 작은 배지 노출(`appHistory.cautionAgreed`). race 방지를 위해 lookup 라벨 비동기 fetch 전에도 동의 행 즉시 표시
...
- **캠페인별 주의사항 동의**(2026-04-22 migration 067): 캠페인 등록/편집 폼 콘텐츠 가이드 섹션 하단에 "동의 받을 주의사항" 영역 — 표준 lookup(`kind='caution'`) 다중 체크 + Quill 자유 메모(`caution_custom_html`). 인플루언서 신청 시 박스 노출 + 동의 체크 강제. 신청 행 메시지 셀에 "주의사항 동의 ✓ {시각}" 작은 배지(클릭 시 스냅샷 모달) — 캠페인 신청 관리/캠페인별 신청자/대시보드 최근 신청 3개 페인 모두 자동 적용 (msgCell 헬퍼 확장)
...
- `applications` — 캠페인 신청 (user_id, campaign_id, message, address, status, reviewed_by, reviewed_at, oriented_at (OT 발송 체크 수동 토글), reviewed_version (낙관적 락), **`caution_agreed_at timestamptz NULL`·`caution_snapshot jsonb NULL` (migration 067)** — 주의사항 동의 시각 + 동의 시점 lookup 코드/라벨/자유메모/언어 통째 보존)
...
- `brand_applications` — 광고주(브랜드) 신청 폼 제출 데이터 (form_type: reviewer|seeding, brand_name, contact_name, phone, email, billing_email, products jsonb, total_jpy/total_qty, estimated_krw, final_quote_krw, quote_sent_at, status **9단계 파이프라인**, admin_memo, version 낙관적 락). 상태: `new→reviewing→schedule_sent→quoted→orient_sheet_sent→campaign_registered→paid→done`(최종완료) / `rejected` (migration 064=8단계 schedule_sent+campaign_registered, 065=9단계 orient_sheet_sent 추가, 기존 데이터 보존). 서버 트리거가 `JFUN-Q|N-YYYYMMDD-NNN` 채번 + products 재계산. 관리자만 SELECT/UPDATE/DELETE. **익명 INSERT는 `submit_brand_application()` RPC(SECURITY DEFINER, BYPASSRLS) 경유 필수** — 직접 INSERT는 42501 RLS 오류 (migration 056). 사업자등록증(`business_license_path`)·`brand-docs` 버킷은 migration 057에서 제거됨
- `admin_notices` (migration 063) — 관리자 공지사항. category(system_update/release/warning/general), pin(bool), title, body_html(Quill rich), created_by, created_at, updated_at. 관리자 SELECT/CUD. 리치 텍스트 XSS 방어는 저장+렌더 이중 sanitize
