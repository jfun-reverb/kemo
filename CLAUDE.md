...
- `settlement_settings` — 정산 도입일(컷오프) 싱글톤 설정(id=1 고정, `cutoff_at timestamptz NULL`, 마이그레이션 230). `backfill_settlements()`가 이 값을 읽어 도입일 이후 인증성공만 자동 백필. NULL=미설정(자동 0건 — 값 세팅 전 폭주 차단). RLS SELECT `has_permission('settlement.view','read')` / UPDATE `is_super_admin()`(도입일은 과거분 전체를 좌우하는 스위치라 최고 관리자 전용). 도입일 이전 과거 인증성공분은 관리자가 정산 화면에서 수동 처리(지급완료 기록/정산대기 추가, **무알림** — PR2 예정). 사양서 `docs/specs/2026-07-09-settlement-cutoff-past-handling.md`
