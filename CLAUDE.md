...
- **오리엔시트 제출 알림** (`notify-orient-submitted`, 마이그202): 브랜드가 작성 폼에서 제출하면 관리자에게 즉시 1통(1인 1통 분리). 트리거 = `orient_sheets` UPDATE 데이터베이스 웹훅(이벤트 UPDATE, 함수 내부 가드로 `status='submitted'` 만 처리 — 임시저장 제외, 재제출은 `last_submitted_at` 변경으로 UPDATE 발생해 포착, 발행 consumed 전환 제외). 신규/재제출 구분(웹훅 `old_record.submitted_at IS NULL`=신규), 제목 「[신규 제출]/[수정 재제출] {브랜드명}」. 본문=브랜드명·형식·제출시각·연결 신청번호·관리자 링크(`data jsonb` 본문 미탑재). 수신자 = `get_subscribed_admin_emails('brand_notify')` + env `NOTIFY_ADMIN_EMAILS`. 메일 best-effort(제출 트랜잭션과 분리). 웹훅은 Dashboard 수동 등록(개발/운영 각각). 사양서 `docs/specs/2026-06-30-orient-submit-notification.md`
