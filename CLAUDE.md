...
- `deliverables` — 결과물 통합. `kind`('receipt'|'review_image'|'post'), `status`, `receipt_url`/`purchase_date`/`purchase_amount`/`order_number`(receipt), `post_url`/`post_channel`/`post_submissions`(post), `reject_reason`, `reviewed_by`/`reviewed_at`, `version` 낙관적 락. **관리자 대리 등록 4종(마이그레이션 160)**: `submitted_by_admin uuid REFERENCES admins(id) ON DELETE SET NULL`(NULL=인플 본인 제출, NOT NULL=대리 등록 식별 기준) + `submitted_by_admin_reason_code text`(`lookup_values.code` 참조, FK 강제 없음) + `submitted_by_admin_reason text`(자유 메모) + `submitted_by_admin_at timestamptz`. 부분 인덱스 `idx_deliverables_proxy ON (submitted_by_admin) WHERE submitted_by_admin IS NOT NULL`
- `deliverable_events` — 결과물 상태 변경 이력. `action`(submit/resubmit/approve/reject/revert/**admin_proxy_submit**/**admin_proxy_revoke**/**channel_assign**/**channel_unassign**), `from_status`, `to_status`. 트리거/RPC 만 INSERT. `admin_proxy_submit`/`admin_proxy_revoke`(마이그레이션 160)는 관리자 대리 등록·회수 RPC 가 직접 INSERT. `channel_assign`/`channel_unassign`(마이그레이션 162)는 `assign_review_image_channel` RPC 가 채널 미지정 review_image 에 채널 지정·해제 시 INSERT(status 불변 → 알림 트리거 미발화). **주의**: `deliverable_events.deliverable_id`에 `ON DELETE CASCADE` 가 걸려 있어 `admin_revoke_proxy_deliverable`/`delete_legacy_review_image` 가 결과물 행을 DELETE 할 때 audit 도 함께 삭제됨(영구 감사 필요 시 별도 audit 테이블 분리 권장)
