...
### 브랜드 서베이 (광고주 신청 관리)
- **오리엔시트 발급·조회 페인**(`/admin#orient-sheets`, `dev/js/admin-orient.js`, PR3): 브랜드에게 작성 토큰 링크를 발급(`create_orient_sheet` 마이그레이션 190, `is_admin()` 가드 — campaign_manager 포함 전체 관리자)하고 제출 내용을 확인. 목록(브랜드·타입·상태·발급일·작성기한·링크복사) + 발급 모달(브랜드 드롭다운·선택적 신청 연결[타입 자동 승계]·타입 선택[비우면 브랜드가 0단계에서 선택]) + 상세 모달(data jsonb를 §11 스키마대로 한국어 렌더, 타입별 분기, 이미지 링크 http/https만 허용). 링크 도메인 hostname 분기(운영 sales.globalreverb.com / 개발 sales-dev). 모달은 동적 생성. 만료는 클라 판정(조회 함수가 status 미전환). brand-applications 상세 연결 링크는 후속. 사양서 `docs/specs/2026-06-18-brand-self-orient-sheet.md` §7
