...
- **GNB**: 비로그인 시 Log In/Sign Up 버튼, 로그인 시 우측 햄버거 메뉴 (계정 카드 + 홈/캠페인/마이페이지 아코디언/메시지/알림/로그아웃/회원탈퇴), 관리자는 Admin 버튼
...
- **마이페이지**: 랜딩(목차) 화면 없이 입력 폼 화면 7종(応募履歴/基本情報/SNSアカウント/配送先/PayPal/パスワード変更/メール受信設定)의 컨테이너(`#mypage-list` 제거, 햄버거 메뉴로 흡수). 진입은 GNB 햄버거 「マイページ」 아코디언 서브항목. `navigate('mypage')` 기본 화면·서브 화면 뒤로가기·`#mypage` popstate 는 모두 応募履歴(`closeMypageSub`)로 복귀. 대표SNS 선택 가능, 필수 미입력 항목 "未登録" 배지(햄버거 서브항목에 표시, `computeProfileBadges` 헬퍼 공용)
...
- **GNB 햄버거 메뉴**: 상단 우측 ☰ 버튼에 미읽음 알림 배지(9+), 클릭 시 우측 슬라이드 패널. 로그인 시 구성 — 최상단 계정 카드(아바타·이름·SNS핸들·이메일) → 홈/캠페인 → 「マイページ」 접기/펼치기 아코디언(기본 펼침, 클릭 시 토글만·화면 이동 없음, 서브 7종 + 기본정보/SNS/배송지/PayPal 「未登録」 배지) → メッセージ(미읽음 배지)/通知 → ログアウト → 하단 작은 退会する 링크. 열 때 프로필 비동기 새로고침으로 계정 카드·배지 최신화. 비로그인은 로그인/회원가입, 인증 페이지에선 햄버거 숨김. 마이페이지 랜딩 화면 흡수(2026-05-21)
