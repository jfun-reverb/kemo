...
- **별도 입구 URL**(`/admin/?app=outbound`, `dev/admin/app.js`): 영업팀 전용 진입 경로. 이 쿼리로 접속하면 「아웃바운드 전용 모드」 — 사이드바에서 아웃바운드·내계정 외 항목·그룹 라벨 숨기고 첫 화면을 명단으로, 상단에 「전체 관리자 화면으로」 복귀 링크(`#adminBackToFull`) 노출. 같은 앱·같은 로그인(기존 관리자가 두 모드 오감). `window._adminAppMode`+`applyOutboundModeUI()`+`exitOutboundMode()`. **보안 경계 아님**(데이터는 `outbound.view` RLS 서버 강제) — 화면 배치 제어. 권한 없는 campaign_manager 진입 시 모드 해제+대시보드(빈 화면 함정 방지). DB 변경 없음. 페인 전환 시 쿼리 유지(`pushState('#'+pane)` 프래그먼트 상대참조).
