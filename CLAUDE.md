...
- **동적 권한 관리 (PR1~PR2 = 화면 표시 제어 단계 완료 · 마이그레이션 207~211 · 실차단=후속 PR3)**: super_admin 이 `/admin#permissions` 「권한 관리」 화면에서 등급(campaign_admin/campaign_manager)×기능별 접근수준(쓰기/읽기/숨김)을 설정 → 사이드바 메뉴 노출이 바뀜. ⚠️**1단계는 화면 표시 제어일 뿐 실제 데이터 차단이 아님**(서버 RLS/`has_permission` 이 방어선, 민감정보 갭1 서버 차단은 PR3). 클라 메뉴 숨김은 fail-open(로드 실패 시 전부 표시), 서버 `has_permission` 은 fail-closed(미등록 거부).
  - `role_permissions` — 등급×기능별 접근수준. `(role, feature_key)` PK, `access_level CHECK(write|read|hidden)`, `role CHECK(campaign_admin|campaign_manager)`(⚠️super_admin 행 없음 = 잠금 방지). RLS SELECT `is_admin()` / CUD `is_super_admin()`. 시드 72행 = **현행 권한 그대로**(도입 즉시 동작 변화 0).
