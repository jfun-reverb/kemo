...
- **엑셀 내보내기**(`exportInfluencersExcel`, admin-excel.js): 현재 필터 결과(화면에 보이는 대상)를 .xlsx 로. 기본 16열(이름 한자/가나·이메일·대표SNS·4채널 핸들+팔로워·합계·도도부현·시군구·등록일). 민감정보 6열(전화·LINE·PayPal·우편번호·건물·상세주소)은 campaign_admin 이상 + 「민감정보 포함」 체크 시만(`isCampaignAdminOrAbove`). ✅ **민감정보 서버 실차단(PR3, 마이그레이션 212·213)**: `fetchInfluencers`/`fetchInfluencersByIds` 가 가림막 뷰 `influencers_admin_view`(security_invoker) 경유로 바뀌어, `has_permission('influencer.sensitive_pii','read')` 가 false 인 등급(마이그213으로 campaign_manager=hidden)에게 6종을 **서버에서 NULL 마스킹**(네트워크 응답 자체에서 제거) + 존재 여부 불리언 `has_phone`/`has_line`/`has_paypal` 로 배지·필터 동작. 화면 마스킹 칸은 「열람 권한 없음」. 레거시 `pw`·`bank_*` 6컬럼은 뷰에서 제외. 신청자·결과물 엑셀도 뷰 경유로 자동 마스킹. 백로그: 엑셀 「민감정보 포함」 게이트를 `isCampaignAdminOrAbove` → `canRead('influencer.sensitive_pii')` 로 일원화(PR3-D). 기존 `_excel*` 헬퍼 + ExcelJS lazy-load 재사용
...
- **동적 권한 관리 (PR1~PR2 = 화면 표시 제어 단계 완료 · 마이그레이션 207~211 · 실차단=후속 PR3)**: super_admin 이 `/admin#permissions` 「권한 관리」 화면에서 등급(campaign_admin/campaign_manager)×기능별 접근수준(쓰기/읽기/숨김)을 설정 → 사이드바 메뉴 노출이 바뀜. ⚠️**1단계는 화면 표시 제어일 뿐 실제 데이터 차단이 아님**(서버 RLS/`has_permission` 이 방어선, 민감정보 갭1 서버 차단은 PR3). 클라 메뉴 숨김은 fail-open(로드 실패 시 전부 표시), 서버 `has_permission` 은 fail-closed(미등록 거부).
  - `role_permissions` — 등급×기능별 접근수준. `(role, feature_key)` PK, `access_level CHECK(write|read|hidden)`, `role CHECK(campaign_admin|campaign_manager)`(⚠️super_admin 행 없음 = 잠금 방지). RLS SELECT `is_admin()` / CUD `is_super_admin()`. 시드 74행 = **현행 권한 그대로**(도입 즉시 동작 변화 0).
