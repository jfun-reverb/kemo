...
- `orient_sheets` — `brand_id`(NOT NULL, ON DELETE RESTRICT) · `application_id`(NULL, SET NULL — 신청 연결 선택) · `token uuid UNIQUE`(작성 링크 식별자) · `form_type`(reviewer|seeding|proxy_purchase, NULL 허용 — **마이그194로 3종 확장**. §15-11 카드별 형식 구조라 컬럼 자체는 대표값/미사용, 형식은 `data.cards[].form_type`) · `data jsonb`(작성 내용 — **cards 배열**, 스키마는 사양서 §15-A) · `status CHECK(draft|submitted|consumed|expired)` · `campaign_id`(발행 캠페인 역참조, SET NULL) · `token_expires_at`(발급+30일) · `version`(낙관적 락) · `created_by`. **익명 직접 접근 0**(RLS — 토큰 함수로만), `updated_at` 자동 갱신 트리거. ⚠️ `brand_id` RESTRICT → `delete_brand` RPC(마이그191)에 `orient_sheets` 카운트 체크
