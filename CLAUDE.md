...
- **오리엔시트 제출 알림** (`notify-orient-submitted`, 마이그202): 브랜드가 작성 폼에서 제출하면 관리자에게 즉시 1통(1인 1통 분리). 트리거 = `orient_sheets` UPDATE 데이터베이스 웹훅(Row filter `status='submitted'` — 임시저장 제외, 재제출은 `last_submitted_at` 변경으로 UPDATE 발생해 포착, 발행 consumed 전환은 제외). 신규/재제출 구분(웹훅 `old_record.submitted_at IS NULL`=신규), 제목 「[신규 제출]/[수정 재제출] {브랜드명}」. 본문=브랜드명·형식·제출시각·연결 신청번호·관리자 링크(`data jsonb` 본문 미탑재). 수신자 = `get_subscribed_admin_emails('brand_notify')` + env `NOTIFY_ADMIN_EMAILS`. 메일 best-effort(제출 트랜잭션과 분리). 웹훅·Edge Function 배포는 Dashboard 수동(개발/운영 각각). 사양서 `docs/specs/2026-06-30-orient-submit-notification.md`
...
- `orient_sheets` — `brand_id`(NOT NULL, ON DELETE RESTRICT) · `application_id`(NULL, SET NULL — 신청 연결 선택) · `token uuid UNIQUE`(작성 링크 식별자) · `form_type`(reviewer|seeding|proxy_purchase, NULL 허용 — **마이그194로 3종 확장**. §15-11 카드별 형식 구조라 컬럼 자체는 대표값/미사용, 형식은 `data.cards[].form_type`) · `data jsonb`(작성 내용 — **cards 배열**, 스키마는 사양서 §15-A) · `status CHECK(draft|submitted|consumed|expired)` · `campaign_id`(발행 캠페인 역참조, SET NULL) · `token_expires_at`(발급+30일) · `submitted_at`(최초 제출 시각·불변) · `last_submitted_at`(마지막 제출 시각·매 제출 갱신 — 마이그202, 제출 알림 신규/재제출 판정용) · `version`(낙관적 락) · `created_by`. **익명 직접 접근 0**(RLS — 토큰 함수로만), `updated_at` 자동 갱신 트리거. ⚠️ `brand_id` RESTRICT → `delete_brand` RPC(마이그191)에 `orient_sheets` 카운트 체크
